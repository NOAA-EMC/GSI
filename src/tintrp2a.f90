...
!-----------------------------------------------------------------------
!RY: for single observation test, write out the interpolator's values
!    purpose: the comparison of interpolatees vs the interpolator
!-----------------------------------------------------------------------
subroutine tintrp2so(f,g,dx,dy,obstime,gridtime, mype,nflds)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    tintrp2a11
!   prgmmr: parrish          org: np22                date: 2013-01-26
...
! Declare local variables
  ...
  g=(f(ix,iy,itime)*delxp*delyp+f(ixp,iy,itime)*delx*delyp &
        ...
        + f(ix,iyp,itimep)*delxp*dely +f(ixp,iyp,itimep)*delx*dely)*deltp
!  printing the values
   write (6,*) 'interpolator value at f(ix,iy,itime)', f(ix,iy,itime)
   write (6,*) 'interpolator value at f(ixp,iy,itime)', f(ixp,iy,itime)

   write (6,*) 'interpolator value at f(ix,iyp,itime)', f(ix,iyp,itime)
   write (6,*) 'interpolator value at f(ixp,iyp,itime)', f(ixp,iyp,itime)

   write (6,*) 'interpolator value at f(ix,iy,itimep)', f(ix,iy,itimep)
   write (6,*) 'interpolator value at f(ixp,iy,itimep)', f(ixp,iy,itimep)

   write (6,*) 'interpolator value at f(ix,iyp,itimep)', f(ix,iyp,itimep)
   write (6,*) 'interpolator value at f(ixp,iyp,itimep)', f(ixp,iyp,itimep)

   write (6,*) 'FINAL INTERPOLATEE=', g
 

  return
end subroutine tintrp2so
