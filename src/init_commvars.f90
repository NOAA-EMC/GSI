...
!   2010-05-23  todling - nvarbal_id no longer wired to 1,2,3,4
!   2010-05-31  todling - only defined nvarbal_id when strong Jc is on and
...
! First find indexes of required balance variables
! u,v(st,vp),ps,and t among control variables (nrf_var)
!_RTod: it's unfortunate to use CV indexes to handle SV
  ivars=-1
  do i=1,size(nrf_var)
     select case (trim(nrf_var(i)))
       case('sf');ivars(1)=i
       case('vp');ivars(2)=i
       case('ps');ivars(3)=i
       case('t' );ivars(4)=i
  ...
  if (do_balid_set) then !_NEW

! Next take care of u,v and st,vp
  do n=1,nsig
     if(mype == icount)then
        nvarbal_id(npcount)=ivars(1)
        nvarbal_id(npcount+1)=ivars(2)
        npcount=npcount+2
     end if
