module gsi_wspd10mOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_wspd10mOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for wspd10mNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: wspd10mOper      ! data stracture

  type,extends(obOper):: wspd10mOper
  contains
    ...
    procedure:: intjo1_
    procedure:: stpjo1_
  end type wspd10mOper

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_wspd10mOper'
  type(wspd10mNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    use m_rhs  , only: iwork => i_wspd10m
    ...
    implicit none
    class(wspd10mOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intwspd10mmod, only: intjo => intwspd10m
    ...
    implicit none
    class(wspd10mOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stpwspd10mmod, only: stpjo => stpwspd10m
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
