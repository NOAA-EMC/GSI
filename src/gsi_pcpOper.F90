...
!                .      .    .                                       .
! subprogram:	 module gsi_pcpOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for pcpNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: pcpOper      ! data stracture

  type,extends(obOper):: pcpOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_pcpOper'
  type(pcpNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(pcpOper ), intent(inout):: self
    ...
    diagsave  = write_diag(jiter) .and. diag_pcp

    call setup(self%obsLL(:), self%odiagLL(:), &
        lunin,mype,aivals,nele,nobs,obstype,isis,is,diagsave,init_pass)

  end subroutine setup_

  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intpcpmod, only: intjo => intpcp
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stppcpmod, only: stpjo => stppcp
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
