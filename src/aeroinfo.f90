...
!   def jpch_aero        - number of (levels+1) * number of satellites
...
!   def nulev            - integer level of aerosol observation
...
!      Task mype_aero writes information to aerosol runtime file
       ...
       do k=1,nlines
          ...
          read(crecord,*) nusis_aero(j),&
               nulev(j),iuse_aero(j),pob_aero(j),gross_aero(j),error_aero(j), &
               b_aero(j),pg_aero(j)
          if (mype==mype_aero) write(iout_aero,130) j,nusis_aero(j),nulev(j),&
