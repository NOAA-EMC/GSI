subroutine tune_gsdpbl_height(mype,station_id,dx,dy,prestsfc,thisPBL_height,diffsfc)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    tune_gsdpbl_height
...
!$$$
  ...
  use guess_grids, only: gsdpbl_height, ges_ps
  implicit none
...
!           -4 C - full reduction
  ...
  gsdpbl_height(ix,iy,itime)  = ges_ps(ix,iy,itime)*ten - max(zero, &
                             pblfact_cool*(ges_ps(ix,iy,itime)*ten-gsdpbl_height(ix,iy,itime)))
  gsdpbl_height(ixp,iy,itime) = ges_ps(ixp,iy,itime)*ten - max(zero,& 
                             pblfact_cool*(ges_ps(ixp,iy,itime)*ten-gsdpbl_height(ixp,iy,itime)))
  gsdpbl_height(ix,iyp,itime) = ges_ps(ix,iyp,itime)*ten - max(zero,&
                             pblfact_cool*(ges_ps(ix,iyp,itime)*ten-gsdpbl_height(ix,iyp,itime)))
  gsdpbl_height(ixp,iyp,itime)= ges_ps(ixp,iyp,itime)*ten - max(zero,&
                             pblfact_cool*(ges_ps(ixp,iyp,itime)*ten-gsdpbl_height(ixp,iyp,itime)))
  write(*,'(a,a8,10f10.2)') 'tuned PBL height=', station_id,diffsfc,pblfact_cool,prestsfc,thisPBL_height,oldPBL_height 

end subroutine tune_gsdpbl_height
