...
!   2017-09-28 li & Whitaker : change stc to tmp amd smc to solw due to the change in fv3gfs surface history file
! 
!
...
!            smc (soilw)
             ...
             if (iret /= 0) call error_msg(trim(my_name),trim(filename),'soilw','read',istop,iret)
             work(:,:)=reshape(rwork2d(:),(/size(work,1),size(work,2)/))
             call tran_gfssfc(work,soil_moi(1,1,it),lonb,latb)
