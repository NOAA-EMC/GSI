...
!   2017-08-30 li     - tic659: add subroutine (read_nems_sfcnst) to read sfc history file in nemsio Gaussin grids
!                               nemsio Gaussin grids generated by regriding from FV3 native grid
! 
!
...
!$$$ end documentation block
  ...
  interface read_nems_sfcnst
     module procedure read_nems_sfcnst_
  end interface
...
!   End of loop over time levels
  ...
  subroutine read_sfcnst_(sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                           tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                           veg_type,soil_type,terrain,isli,use_sfc_any)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    read_sfcnst_     read nems hist file
...
!   Declare passed variables
    logical,                                               intent(in   ) :: use_sfc_any
    real(r_single),  dimension(nlat_sfc,nlon_sfc,nfldsfc), intent(  out) :: sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough
    real(r_single),  dimension(nlat_sfc,nlon_sfc,nfldsfc), intent(  out) :: tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d
    real(r_single),  dimension(nlat_sfc,nlon_sfc),         intent(  out) :: veg_type,soil_type,terrain
    integer(i_kind), dimension(nlat_sfc,nlon_sfc),         intent(  out) :: isli
!   Declare local parameters
    integer(i_kind),parameter   :: nsfc_all=20
    integer(i_kind),dimension(7):: idate
    integer(i_kind),dimension(4):: odate
...
!            zorl
          ...
          elseif(n == 8) then                            ! tref

             write(*,*) 'read tref in read_sfcnst'
             ...
             write(*,*) 'read tref in read_sfcnst done'

          elseif(n == 9) then                            ! dt_cool

             write(*,*) 'read dtcool in read_sfcnst'

             call nemsio_readrecv(gfile, 'dt_cool','sfc', 1, rwork2d, iret=iret)
             ...
             write(*,*) 'read dtcool in read_sfcnst done'

          elseif(n == 10) then                            ! z_c

!            z_c
             call nemsio_readrecv(gfile, 'z_c',  'sfc', 1, rwork2d,iret=iret)
...
!            vtype
          ...
          elseif(n == 18 .and. use_sfc_any) then          ! soil type
...
!            orog
          ...
          elseif(n == 20) then                           ! sea/land/ice flag
...
!     Load onto all processors
  ...
  subroutine read_nems_sfcnst_(iope,sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                               tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                               veg_type,soil_type,terrain,isli,use_sfc_any)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    read_nems_sfcnst_     read nems hist file
...
!     dt_warm   - diurnal warming amount at sea surface 
!     z_w       - depth of diurnal warming layer
!     c_0       - coefficient to calculate d(Tz)/d(tf) 
!     c_d       - coefficient to calculate d(Tz)/d(tf)
!     w_0       - coefficient to calculate d(Tz)/d(tf) 
!     w_d       - coefficient to calculate d(Tz)/d(tf) 
!     veg_type  - vegetation type
!     soil_type - soil type
!     terrain   - terrain height
!     isli      - sea/land/ice mask
...
!   Read surface history file on processor iope
    if(mype == iope)then
       write(*,*) 'read_sfcnst nemsio'
       call read_sfcnst_(sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                          tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                          veg_type,soil_type,terrain,isli,use_sfc_any)
    end if
...
!     Load onto all processors
  ...
  end subroutine read_nems_sfcnst_
