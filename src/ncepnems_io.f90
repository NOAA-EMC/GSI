...
!   2017-08-30 li     - tic659: add subroutine (read_nems_sfchist) to read sfc history file in nemsio Gaussin grids
...
!   sub read_nems_sfchist  - read sfc hist file, including sfc and nst vars, scatter on grid to analysis subdomains
!
...
!$$$ end documentation block
  ...
  interface read_nems_sfchist
     module procedure read_nems_sfchist_
  end interface
...
!   End of loop over time levels
  ...
  subroutine read_sfchist_(sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                           tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                           veg_type,soil_type,terrain,isli,use_sfc_any)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    read_sfchist_     read nems hist file
!   prgmmr: Li            org: np23                date: 2017-08-30
!
! abstract: read nems hist file
!
! program history log:
!  
!   input argument list:
...
!   Declare local variables
    ...
    character(len=24)  :: filename
    character(len=120) :: my_name = 'READ_NEMS_SFCHIST'
    character(len=1)   :: null = ' '
    integer(i_kind) :: i,j,it,n,nsfc
    integer(i_kind) :: iret, nframe, lonb, latb
    integer(i_kind) :: nfhour, nfminute, nfsecondn, nfsecondd
...
!
!      Read the surface history records (lonb, latb)  and convert to GSI array pattern (nlat_sfc,nlon_sfc)
!      Follow the read order sfcio in ncepgfs_io
!
       allocate(work(lonb,latb))
       allocate(rwork2d(size(work,1)*size(work,2)))
...
!     Load onto all processors
  ...
  subroutine read_nems_sfchist_(iope,sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                               tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                               veg_type,soil_type,terrain,isli,use_sfc_any)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    read_nems_sfchist_     read nems hist file
...
!   Read surface history file on processor iope
    if(mype == iope)then
       write(*,*) 'read_sfchist nemsio'
       call read_sfchist_(sfct,soil_moi,sno,soil_temp,veg_frac,fact10,sfc_rough, &
                          tref,dt_cool,z_c,dt_warm,z_w,c_0,c_d,w_0,w_d, &
                          veg_type,soil_type,terrain,isli,use_sfc_any)
    end if
...
!     Load onto all processors
  ...
  end subroutine read_nems_sfchist_
