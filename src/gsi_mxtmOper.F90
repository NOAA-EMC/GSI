...
!     date:	 2018-08-10
!
! abstract: an obOper extension for mxtmNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: mxtmOper      ! data stracture

  type,extends(obOper):: mxtmOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_mxtmOper'
  type(mxtmNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(mxtmOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intmxtmmod, only: intjo => intmxtm
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stpmxtmmod, only: stpjo => stpmxtm
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
