...
! Declare local variables
  ...
  real(r_kind),pointer,dimension(:) :: beta1 => NULL()
  real(r_kind),pointer,dimension(:) :: beta2 => NULL()
...
!$omp parallel do schedule(dynamic,1) private(nn,k,j,i,ii)
  ! next multiply by beta2
  do j=1,grd_ens%lon2
     do ii=1,nsubwin
        do nn=1,n_ens
           do k=1,grd_ens%nsig
              do i=1,grd_ens%lat2
                 grady%aens(ii,nn)%r3(1)%q(i,j,k) = beta2(k)*grady%aens(ii,nn)%r3(1)%q(i,j,k)
              enddo
           enddo
        enddo
     enddo
...
!   2015-09-19  mahajan - adapt pwgt for global
!
!   input argument list:
!
!   output argument list:
