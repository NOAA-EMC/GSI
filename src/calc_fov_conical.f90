...
! Declare local variables.
 ...
 if(instr > 24_i_kind .and. instr < 30_i_kind) mchan = nchan
...
! These are the flat earth variables
 ...
 integer(i_kind) :: mchan         ! storage for channel number for future use with instruments other than ssmis
 integer(i_kind) :: minstr        ! storage for instrument number permitting instr 25 default circular

 real(r_kind), target, dimension(0:7,2,nchan) :: ssmiscoeff_26 = reshape( (/    &
 ...
 real(r_kind), target, dimension(0:7,2,nchan) :: ssmiscoeff_27 = reshape( (/  &
 ...
 real(r_kind), target, dimension(0:7,2,nchan) :: ssmiscoeff_28 = reshape( (/ &
 ...
 real(r_kind), target, dimension(0:7,2,nchan) :: ssmiscoeff_29 = reshape( (/  &
 ...
 real(r_kind), target, dimension(0:7,2,nchan) :: ssmiscoeff_30 = reshape( (/  &
 ...
 real(r_kind), pointer, dimension(:,:,:) :: ssmiscoeff 
