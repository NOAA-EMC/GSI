...
do
  if (fn+t*fpn.lt.fn+0.9*t*fpn) exit
  t=2.*t
enddo
...
!
    ...
    if (logic.eq.7) write (io,1006) td
    exit
  endif
...
if (abs(z1).le.1.) then
    ...
    discri=discri*fpa
    discri=z1-discri
    if (z1.ge.0.0 .and. discri.ge.0.0) then
        discri=sqrt(z1)*sqrt(discri)
        go to 120
    endif
    if (z1.le.0.0 .and. discri.le.0.0) then
        discri=sqrt(-z1)*sqrt(-discri)
        go to 120
    endif
    discri=-one
