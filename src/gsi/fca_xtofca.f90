...
!$$$ end documentation block
    ...
    integer(i_kind) :: status, Nx, Ny, uv_zlevel

    integer(i_kind) :: i, j, unit_fcauv
    ...
    if (status /= 0) write(*,*) '*** xtofca: failed to allocate disp ***, status=',status
    ...
    if (.not. flag_linear) then
       ...
       if (status /= 0) write(*,*) '*** xtofca: sval_to_disp_grid failed for disp ***, status=',status
       ...
       if (status /= 0) write(*,*) '*** xtofca: failed to allocate disp_tl ***, status=',status
       call sval_to_disp_grid(sval,disp_tl,uv_zlevel,status)
       if (status /= 0) write(*,*) '*** xtofca: sval_to_disp_grid failed for disp_tl ***, status=',status
    ...
    if (status /= 0) write(*,*) '*** xtofca: fca_state_to_sval failed ***, status=',status

    ! deallocate: disp, disp_tl, bg_state, inc_state
    call deallocate_disp(disp,status)
    if (status /= 0) write(*,*) '*** xtofca: failed to deallocate disp ***, status=',status

    if (flag_linear) then
       call deallocate_disp(disp_tl,status)
       if (status /= 0) write(*,*) '*** xtofca: failed to deallocate disp_tl ***, status=',status
    end if
    call fca_deallocate_wrf_grid(bg_state,status)
    if (status /= 0) write(*,*) '*** xtofca: failed to deallocate bg_state ***, status=',status

    call fca_deallocate_wrf_grid(inc_state,status)
    if (status /= 0) write(*,*) '*** xtofca: failed to deallocate inc_state ***, status=',status
