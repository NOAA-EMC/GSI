...
!    directory names (hardwired for now)
  type type_fv3regfilenameg
      character(len=max_varname_length) :: grid_spec !='fv3_grid_spec'            
      character(len=max_varname_length) :: ak_bk     !='fv3_akbk'
      character(len=max_varname_length) :: dynvars   !='fv3_dynvars'
      character(len=max_varname_length) :: tracers   !='fv3_tracer'
      character(len=max_varname_length) :: sfcdata   !='fv3_sfcdata'
      character(len=max_varname_length) :: couplerres!='coupler.res'
      contains
...
! set subroutines to public
  ...
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_io_dynmetvars3d_nouv  ! copy of cvars3d excluding uv 3-d fields   CV
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_io_tracermetvars3d_nouv  ! copy of cvars3d excluding uv 3-d fields   CV
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_io_dynmetvars2d_nouv  ! copy of cvars3d excluding uv 3-d fields   CV
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_io_tracermetvars2d_nouv  ! copy of cvars3d excluding uv 3-d fields   CV
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_names_gsibundle_dynvar_nouv  !to define names in gsibundle 
  character(len=max_varname_length),allocatable,dimension(:) :: fv3lam_names_gsibundle_tracer_nouv  !to define names in gsibundle 
...
!clt    use gsi_bundlemod, only : gsi_bundle
...
!clt this block is still maintained for they would be needed for a certain 2d fields IO 
    ...
    enddo
      ...
      do i=1,GSI_MetGuess_Bundle(it)%n2d
        write(6,*)'metvardeb333-2d nmae ', trim(name_metvars2d(i))
      enddo
...
! Inquire about guess fields
  ...
  if (nguess>0) then
     allocate(guessname(nguess))
     call gsi_metguess_get('gsinames',guessname,istatus)
  endif
  
      do i=1,nguess
        write(6,*)'metvardeb333 guessname', trim(guessname(i))
      enddo
...
!maybe more flexibile procedure would relax it
      ...
      if (fv3sar_bg_opt == 0.and.ifindstrloc(name_metvars3d,'delp').le.0)then
         ndynvario3d=ndynvario3d+1  ! for delp  
      endif
      if (fv3sar_bg_opt == 1.and.ifindstrloc(name_metvars3d,'delp').gt.0)then
      ...
      do i=1,size(name_metvars3d)
        ...
        if(.not.(trim(vartem)=='u'.or.trim(vartem)=='v'.or.trim(vartem)=='iqr')) then
         ...
         else if (ifindstrloc(vardynvars,trim(vartem)).gt.0) then
      ...
      enddo
      if(fv3sar_bg_opt == 0.and.ifindstrloc( fv3lam_io_dynmetvars3d_nouv(1:jdynvar),"delp").le.0)  then
            jdynvar=jdynvar+1
      ...
      if(jdynvar.ne.ndynvario3d.or.jtracer.ne.ntracerio3d  ) then
      ...
      if (ntracerio2d.gt.0) allocate(fv3lam_io_tracermetvars2d_nouv(ntracerio2d))
      jdynvar=0
      jtracer=0
      do i=1,size(name_metvars2d)
        vartem=trim(name_metvars2d(i))
      ...
      if(mype.eq.0) then
        if (allocated(fv3lam_io_dynmetvars2d_nouv)) write(6,*)' fv3lam_io_dynmetvars2d_nouv is ',(trim(fv3lam_io_dynmetvars2d_nouv(i)), i=1,ndynvario2d)
        if (allocated(fv3lam_io_tracermetvars2d_nouv)) write(6,*)'fv3lam_io_dynmetvars2d_nouv is ',(trim(fv3lam_io_dynmetvars2d_nouv(i)),i=1,ntracerio3d)
      endif      
...
!clt      call GSI_BundleGetPointer ( gsibundle_fv3lam_dynvar_nouv, 'delp'  ,ges_delp ,istatus );ier=ier+istatus
!clt      ges_delp=ges_delp*0.001_r_kind
...
!$$$  end documentation block
    ...
    do ilevtot=kbgn,kend
       ...
       name=trim(varname)
       if(trim(filenamein).ne.trim(filenamein2)) then
...
!   write out
    if(fv3sar_bg_opt == 0) then
       call gsi_fv3ncdf_write(grd_fv3lam_dynvar_ionouv,gsibundle_fv3lam_dynvar_nouv,add_saved,fv3filenamegin%dynvars,fv3filenamegin)
       call gsi_fv3ncdf_write(grd_fv3lam_tracer_ionouv,gsibundle_fv3lam_tracer_nouv,add_saved,fv3filenamegin%tracers,fv3filenamegin)
       call gsi_fv3ncdf_writeuv(grd_fv3lam_uv,ges_u,ges_v,add_saved,fv3filenamegin)


    else
       call gsi_fv3ncdf_write_v1(grd_fv3lam_dynvar_ionouv,gsibundle_fv3lam_dynvar_nouv,add_saved,fv3filenamegin%dynvars,fv3filenamegin)
       call gsi_fv3ncdf_write_v1(grd_fv3lam_tracer_ionouv,gsibundle_fv3lam_tracer_nouv,add_saved,fv3filenamegin%tracers,fv3filenamegin)
...
!$$$ end documentation block
    ...
    integer(i_kind) nxcase,nycase
    integer(i_kind) u_countloc(3),u_startloc(3),v_countloc(3),v_startloc(3)
    character(len=124) filenamein ,varname
    real(r_kind),allocatable,dimension(:,:,:,:):: worksub
...
!!!!!!!!  add analysis_inc to readin work_b !!!!!!!!!!!!!!!!
       ...
       print *,'write out u/v to ',trim(filenamein )
...
!ct   use gsi_bundlemod, only : gsi_bundle
   implicit none  
...
! !INPUT/OUTPUT PARAMETERS:
  ...
  type(gsi_bundle), intent(inout) :: bundo
    ...
    real(r_kind),dimension(:,:,:),pointer:: pvar3d
    real(r_kind),dimension(:,:,:),pointer:: pvar2d
    ...
    do ivar=1,src_nc3d
       ...
       do jvar=1,target_nc3d
         if(index(target_name_vars3d(jvar),varname).gt.0)  then
         call GSI_BundleGetPointer (bundi,varname,pvar3d,istatus)
         call gsi_bundleputvar (bundo,varname,pvar3d,istatus)
         write(6,*)varname ,' is copied from bundi to bundo in  gsi_copy_bundle'
         exit
         endif
      enddo
    enddo
    do ivar=1,src_nc2d
       ...
       do jvar=1,target_nc2d
         if(index(target_name_vars2d(jvar),varname).gt.0)  then
         call GSI_BundleGetPointer (bundi,varname,pvar2d,istatus)
         call gsi_bundleputvar (bundo,varname,pvar2d,istatus)
         write(6,*)'varname 2d',trim(varname),' is copied from bundi to bundo in  gsi_copy_bundle'
         exit
         endif
      enddo
    enddo
    ...
    character(len=max_varname_length):: vgsinamein
    character(len=max_varname_length),intent(out):: vname
    character(len=max_varname_length),intent(out):: filenameout
    if (ifindstrloc(vgsiname,vgsinamein).le.0) then
      write(6,*)'the name ',vgsinamein ,'cannot be treated correctly in getfv3lamfilevname,stop'
      call stop2(333)
    endif
    if(ifindstrloc(vardynvars,vgsinamein).gt.0)  then 
        filenameout=fv3filenamegref%dynvars
    else if(ifindstrloc(vartracers,vgsinamein).gt.0 )  then 
