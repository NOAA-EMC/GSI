...
!maybe more flexibile procedure would relax it
     ...
     do i=1,size(name_metvars3d)
      ...
      if (.not. allocated(fv3lam_io_dynmetvars3d_nouv)) then
        allocate(fv3lam_io_dynmetvars3d_nouv(ndynvario3d))
      end if
      if (.not. allocated(fv3lam_io_tracermetvars3d_nouv)) then
...
! for io_layout > 1
    ...
    do ilevtot=kbgn,kend
      ...
      if(fv3_io_layout_y > 1) then
        do nio=0,fv3_io_layout_y-1
         if (ensgrid) then
          countloc=(/nxcase,ny_layout_lenens(nio),1/)
          allocate(uu2d_layout(nxcase,ny_layout_lenens(nio)))
