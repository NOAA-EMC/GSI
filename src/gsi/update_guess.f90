...
!$$$
  ...
  use jfunc, only: iout_iter,bcoption,tsensible,clip_supersaturation,superfact,use_2m_obs
...
! update surface and soil    
     ...
     if ( (i_use_2mt4b > 0.or. use_2m_obs) .and. is_t>0) then
        do j=1,lon2
           do i=1,lat2
              tinc_1st(i,j)=p_tv(i,j,1)
           end do
     ...
     if ( (i_use_2mq4b > 0.or. use_2m_obs) .and. is_q>0) then
        do j=1,lon2
           do i=1,lat2
              qinc_1st(i,j)=p_q(i,j,1)
           end do
