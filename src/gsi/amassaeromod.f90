...
real(r_kind),pointer,dimension(:,:,:) :: cv_amassi,cv_amassj,cv_amassk
    ...
    real(r_kind),dimension(:,:,:),pointer::ges_aero=>NULL()
    ...
    if (ier/=0) then
       print*,"get first guess amassi-k failed! Stop"
       stop
    end if
...
do ic=1,naerosols-1
  ...
  print*,"amas2aero_tl:wg.ge.1 ",trim(aerosols(ic)),ges_aero(i,j,k),ges_amassj(i,j,k)
   ...
   !print*,"amas2aero_tl: ",trim(aerosols(ic)),sv_rank3(1,1,1)
end do
...
do ic=1,naerosols-1
   ...
   !print*,"amass2aero_ad: cvad= ",trim(aerosols(ic)),cv_amassi(1,1,1),cv_amassj(1,1,1),cv_amassk(1,1,1)
   !print*,"amass2aero_ad: coe= ",ges_aero(1,1,1)/ges_amassi(1,1,1),ges_aero(1,1,1)/ges_amassj(1,1,1),ges_aero(1,1,1)/ges_amassk(1,1,1) 
end do
