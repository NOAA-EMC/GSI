...
!    Link obs to diagnostics structure
     if(luse_obsdiag)then
        if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_tcamt_ob_type,ibin)%tail)) then
              call die(myname,'.not.associated(obsdiags(i_tcamt_ob_type,ibin)%tail)')
           end if
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           if (lobsdiagsave) then
              ...
              call nc_diag_data2d("ObsDiagSave_nldepart", obsdiags(i_tcamt_ob_type,ibin)%tail%nldepart )
              call nc_diag_data2d("ObsDiagSave_tldepart", obsdiags(i_tcamt_ob_type,ibin)%tail%tldepart )
              call nc_diag_data2d("ObsDiagSave_obssen",   obsdiags(i_tcamt_ob_type,ibin)%tail%obssen   )             
           endif
