module gsi_uwnd10mOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_uwnd10mOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for uwnd10mNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: uwnd10mOper      ! data stracture

  type,extends(obOper):: uwnd10mOper
  contains
    ...
    procedure:: intjo1_
    procedure:: stpjo1_
  end type uwnd10mOper

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_uwnd10mOper'
  type(uwnd10mNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    use m_rhs  , only: iwork => i_uwnd10m
    ...
    implicit none
    class(uwnd10mOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intuwnd10mmod, only: intjo => intuwnd10m
    ...
    implicit none
    class(uwnd10mOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stpuwnd10mmod, only: stpjo => stpuwnd10m
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
