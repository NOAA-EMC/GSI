module gsi_tcamtOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_tcamtOper
...
! module interface:
  ...
  implicit none
  public:: tcamtOper      ! data stracture

  type,extends(obOper):: tcamtOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_tcamtOper'
  type(tcamtNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(tcamtOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use inttcamtmod, only: intjo => inttcamt
    ...
    implicit none
    class(tcamtOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stptcamtmod, only: stpjo => stptcamt
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
