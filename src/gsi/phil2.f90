...
! any integer up to 30, or 100, or 300, and uses precomputed pseudo-random
...
real(dp),dimension(nob),intent(inout):: dob
!-----------------------------------------------------------------------------
real(dp),parameter:: eps=1.e-13,shrink=1-eps
real(dp)          :: sa,sb,sc,pa0,pc0,sbold,ds,soba,sobc,spansi
integer           :: ia,ib,ic,jb,La,Lc,pa1,pb1,pc1
!=============================================================================
spansi=u1/span**2
ia=1
ic=1
...
integer           :: ia,ib,ic,La,Lc,pa1,pb1,pc1
!=============================================================================
spansi=u1/span**2
ia=1
ic=1
...
! the Hilbert curves, and round up to the next integer power, ngen4, of 1/2.
...
integer,                intent(in ):: nob,nrand,nor
...
!-----------------------------------------------------------------------------
integer,parameter         :: ngen=14
real(dp),dimension(3,nob) :: xob
real(dp),dimension(nob)   :: rob,sob
...
integer                   :: i,irand,j,k,L,ngen4,ntri,skip
!=============================================================================
uh=sqrt(pi/6)*re
uv=(uh*vscale)/hscale
vh=(vmax-vmin)/uv
...
do ngen4=0,ngen-1
   ...
   v=v/2
enddo
v=v*2 ! <- Shell thickness in Hilbert units
...
integer,dimension(nob)    :: rank
integer,dimension(0:ngen) :: hilr
integer                   :: i,irand,j,L,ngen4,ntri,skip
!=============================================================================
uh=sqrt(pi/6)*re
uv=(uh*vscale)/hscale
vh=(vmax-vmin)/uv
...
integer,dimension(ngen)  :: hilr
integer,dimension(nob)   :: rank
integer                  :: irand,L
!==============================================================================
vo3=(/o3,o3/)! Tile-relative axis of rotation is at (1/3, 1/3)
xcen=(xmin+xmax)/2; ycen=(ymin+ymax)/2
rad=eps+sqrt((xmax-xmin)**2+(ymax-ymin)**2)/2
rad3=rad*3 ! Size of the edge of the Hilbert curve square tile
...
!-----------------------------------------------------------------------------
real(dp),parameter:: u8=8,or8=u1/sqrt(u8),sig=u1/phi,chi=r2-u1
...
case(5)
   ce=u1/(2*sig+chi-sqrt(4*sig+2*chi-u3))
   cf=chi*ce
   cg=-u1+(-u1+r2*phi)*ce
   ch=-phi+(u1+r2*sig)*ce
...
! the initial standard cube by integer multiples of the angle pi*2/7 about
...
k2=2*cos(2*pio7)
k3=2*cos(3*pio7)
a=4*(5-2*k3-4*k3**2-r2*(k3-1))
a=4*(6*k1-5-2*k2+r2*(k1-k2-2))
b=(u1/k3+r2*k3)
w=(b+sqrt(b**2-a))/a
x=r2*w-k2/2
y=(k1-2)*r2*w+k2/2
...
!-----------------------------------------------------------------------------
real(dp),parameter:: term1=29*r2-u1,term2=(11+9*r2+2*sqrt(term1))/41,&
                     term3=(30-9*r2-2*sqrt(term1))/82,&
