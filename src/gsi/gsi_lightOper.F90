module gsi_lightOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_lightOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for lightNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: lightOper      ! data stracture
  public:: lightOper_config
    interface lightOper_config; module procedure config_; end interface

  type,extends(obOper):: lightOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_lightOper'
  type(lightNode),save,target:: myNodeMold_
...
!>   >    call lightOper_config(nsig_save=nsig_save)
!>   >  endif
!>
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    use m_rhs  , only: iwork => i_light

    use obsmod  , only: write_diag
    use lightinfo,only: diag_light
    use jfunc   , only: jiter

    use mpeu_util, only: die
    implicit none
    class(lightOper ), intent(inout):: self
    ...
    diagsave  = write_diag(jiter) .and. diag_light
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intlightmod, only: intjo => intlight
    ...
    implicit none
    class(lightOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stplightmod, only: stpjo => stplight
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
