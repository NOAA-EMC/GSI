...
!       2022-11-16  Y. Yang, Y. Wang, and X. Wang - add convective-static BEC calculated with 
!                                                   the reflectivity-dependent binning method
...
! !REVISION HISTORY:
!      2022-11-16  Y. Yang, Y. Wang, and X. Wang - changed from berror_get_dims_reg for 
...
!   Load agv wgv bv
    ...
    subroutine berror_read_bal_reg_extra(msig,mlat,num_bins2d,agvi,bvi,wgvi,evi2,evi3,mype,unit)
      ...
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: wgvi
      real(r_kind),dimension(0:mlat+1,nsig)     ,intent(  out) :: bvi
      real(r_kind),dimension(extra_var2num,0:mlat+1,num_bins2d,nsig)     ,intent(  out),optional :: evi2
      real(r_kind),dimension(extra_var3num,0:mlat+1,num_bins2d,nsig,nsig),intent(out) :: evi3
      integer(i_kind)                              ,intent(in   ) :: mype  !"my" processor ID
...
!   Read header.
    ...
    if(extra_var2num > 0) allocate (extravi2(extra_var2num,0:mlat+1,num_bins2d,1:msig) )
    allocate ( extravi3(extra_var3num,0:mlat+1,num_bins2d,1:msig,1:msig) )

    if(extra_var2num > 0) extravi2 = 0.0
    extravi3 = 0.0
...
!   Read background error file to get balance variables
    ...
    if(extra_var2num > 0) then
       ...
       read(inerr)agv_avn,bv_avn,wgv_avn ,extravi3
    end if
    close(inerr)
...
!   Load agv wgv bv
    ...
    deallocate (agv_avn,bv_avn,wgv_avn,clat_avn,sigma_avn,rlsigo)
    deallocate (extravi3)
    if(extra_var2num > 0) deallocate (extravi2)
    return
...
! !REVISION HISTORY:
!      2022-11-16  Y. Yang, Y. Wang, and X. Wang - changed from berror_read_wgt_reg for 
!                                                  the use of covective-scale static BEC
!                                                  poc: xuguang.wang@ou.edu
!
!EOP ___________________________________________________________________
...
!  workspace variables not returned
  ...
  if(extra_var2num>0) allocate(extravi2(extra_var2num,0:mlat+1,num_bins2d,msig))
...
! Read background error file to get balance variables
  ...
  if(extra_var2num>0) then
     read(inerr)agv_avn,bv_avn,wgv_avn,&
               extravi2(1:extra_var2num,0:mlat+1,num_bins2d,1:msig),&
               extravi3(1:extra_var3num,0:mlat+1,num_bins2d,1:msig,1:msig)
  else
     read(inerr)agv_avn,bv_avn,wgv_avn,&
               extravi3(1:extra_var3num,0:mlat+1,num_bins2d,1:msig,1:msig)
  end if
