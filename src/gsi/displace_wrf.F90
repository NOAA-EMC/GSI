...
!$$$ end documentation block
  ...
  real(r_kind), parameter, public :: t0 = 300.0_r_kind  !Basic state potential temperature (K)
  real(r_kind), parameter, public :: p0 = 1E5_r_kind  !basic state dry surface pressure
  real(r_kind), parameter, public :: gas_constant = 287.0_r_kind     !dry air gas constant
  real(r_kind), parameter, public :: gas_constant_v = 461.6_r_kind      !water vapor gas constant
  real(r_kind), parameter, public :: gravity = 9.81_r_kind
  ! Copy to variable names used here
  real(r_kind), parameter, public :: grav=gravity !dry air gas constant 
  real(r_kind), parameter, public :: Rd=gas_constant !dry air gas constant 
  real(r_kind), parameter, public :: Rv=gas_constant_v !dry air gas constant 
  real(r_kind), parameter, public :: cp = 7.0_r_kind*Rd/2.0_r_kind !dry air heat capacity at constant pressure
  real(r_kind), parameter, public :: kappa = Rd / cp !exponent used in computation of theta
  real(r_kind), parameter, public :: base_pres = p0 !equal to p0
  real(r_kind), parameter, public :: gamma=0.0065_r_kind    !lapse rate used for SLP reduction
...
!$$$ end documentation block
    ...
    integer(i_kind), intent(in) :: th_compute				! control parameter for theta computation/displacemnt
    integer(i_kind), intent(in) :: istep				! control parameter for theta computation/displacemnt
    integer(i_kind), intent(in) :: qv_ind				! index for qvapor
    integer(i_kind), intent(in) :: nmoist ! number of moisture fields
    real(r_kind), intent(INOUT), dimension(ims:ime,jms:jme,kms:kme) :: p, pb, t, u, v, w
    real(r_kind), intent(inout), dimension(ims:ime,jms:jme,kms:kme) ::  ph, phb
    real(r_kind), intent(INOUT), dimension(ims:ime,jms:jme,kms:kme,1:nmoist) :: moist
    real(r_kind), intent(IN), dimension(ims:ime,jms:jme) :: hgt, mub
    real(r_kind), intent(INOUT), dimension(ims:ime,jms:jme) :: mu, psfc
    real(r_kind), intent(IN), dimension(kms:kme) :: znu, znw, c1h, c2h, c3h, c4h, c3f, c4f
    ...
    real(r_kind), dimension(ims:ime,jms:jme,kms:kme) :: ph_hyd, ph_nl	! hydrostatic component for geopotential
    real(r_kind), dimension(ims:ime,jms:jme,2) :: work2d		! work space for 2d original/displaced fields
...
!
!$$$ end documentation block
    real(r_kind), intent(IN), dimension(ims:ime,jms:jme,kms:kme) :: qvapor, p, pb, t
    real(r_kind), intent(IN), dimension(ims:ime,jms:jme) :: hgt, mu, mub
    real(r_kind), intent(IN), dimension(kms:kme) :: c3h,c4h,c3f,c4f
    real(r_kind), intent(IN) :: ptop
    real(r_kind), dimension(ims:ime,jms:jme,kms:kme), intent(out) :: ph_hyd		! nonhydrostatic component for geopotential
...
!$$$ end documentation block
    ...
    implicit none
    real(r_kind), intent(INOUT), dimension(ims:ime, jms:jme, kms:kme) :: qvapor, p, pb, t, ph
    real(r_kind), intent(IN), dimension(ims:ime, jms:jme, kms:kme) :: ph_nl, phb
    real(r_kind), intent(IN), dimension(ims:ime, jms:jme) :: hgt, mub
    real(r_kind), intent(INOUT), dimension(ims:ime, jms:jme) :: mu, psfc
    real(r_kind), intent(IN), dimension(kms:kme) :: znu, znw, c1h, c2h, c3h, c4h, c3f, c4f
    real(r_kind), intent(IN) :: ptop
...
!$$$ end documentation block
    ...
    implicit none
    real(r_kind), intent(INOUT), dimension(ims:ime,jms:jme,kms:kme) :: t
    real(r_kind), intent(IN), dimension(ims:ime,jms:jme,kms:(kme+1)) :: ph, phb
    type (fca_gridded_disp), intent(in) :: fcadisp			! displacement field generated by FCA routine
    integer(i_kind), intent(in) :: num_glob_needed, needed_ij_in(:,:), needed_ij_out(:,:), index_in(:)
    real(r_kind), intent(in) :: needed_wgts_out(:,:)
    integer(i_kind), intent(in) :: interp_order
...
!$$$ end documentation block
    ...
    real(r_kind), dimension(ims:ime, jms:jme,2) :: T_bot_top ! theta above and below level for interpolation
...
!$$$ end documentation block
    ...
    do j= jms, jme
       do i= ims, ime
          ...
          t1 = t(i,j) - tf
          if ( ( t1 >= t_ref ) .and. ( t1 >= const_47) ) then    ! liq phase eslo
             ew = a0 + t1 * (a1 + t1 * (a2 + t1 * (a3 + t1 * (a4 + t1 * (a5 + t1 * a6)))))
          else if ( ( t1 >= t_ref ) .and. ( t1 < const_47 ) ) then !liq phas poor es
