module gsi_cldchOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_cldchOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for cldchNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: cldchOper      ! data stracture

  type,extends(obOper):: cldchOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_cldchOper'
  type(cldchNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(cldchOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intcldchmod, only: intjo => intcldch
    ...
    implicit none
    class(cldchOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stpcldchmod, only: stpjo => stpcldch
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
