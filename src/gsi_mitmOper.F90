...
!     date:	 2018-08-10
!
! abstract: an obOper extension for mitmNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: mitmOper      ! data stracture

  type,extends(obOper):: mitmOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_mitmOper'
  type(mitmNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(mitmOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intmitmmod, only: intjo => intmitm
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stpmitmmod, only: stpjo => stpmitm
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
