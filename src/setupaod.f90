...
!$$$
  ...
  use aod_mod, only: laeroana_gocart, l_aoderr_table
...
! Declare local variables
  ...
  integer(i_kind) mm1
  integer(i_kind) kk,n,kk2,ibin,ioff,iii
...
!    first outer iteration.
     if (init_pass .and. mype==mype_diaghdr(is)) then
        write(4) isis,dplat(is),obstype,jiter,nchanl,ianldate,ireal,ipchan,nsig
