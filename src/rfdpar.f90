...
!$$$ end documentation block
  ...
  real(r_kind),intent(out),dimension(*):: be,rate
...
!$$$
  ...
  integer(i_kind),intent(in)::m
  real(r_kind),intent(in),dimension(m):: be,rate
  real(r_kind),intent(out),dimension(m,m):: turn
...
!$$$
  ...
  real(r_kind),intent(in),dimension(n):: dsh
  real(r_kind),intent(in),dimension(m):: rate
  real(r_kind),intent(out),dimension(n,m):: al
...
!$$$
  ...
  real(r_kind),intent(in),dimension(na,*):: a
  real(r_kind),intent(inout),dimension(nb,*):: b
...
!$$$
  ...
  integer(i_kind),intent(out),dimension(m):: ipiv
  real(r_kind),intent(inout),dimension(na,*):: a
...
!$$$
  ...
  integer(i_kind),intent(in),dimension(m):: ipiv
  real(r_kind),intent(in),dimension(na,*):: a
  real(r_kind),intent(inout),dimension(nb,*):: b
...
!$$$
  ...
  real(r_kind),intent(in),dimension(m):: d
  real(r_kind),intent(in),dimension(na,*):: a
...
!$$$
  ...
  integer(i_kind),intent(in):: m
  complex(r_kind),intent(in),dimension(*):: a
  complex(r_kind),intent(out),dimension(m):: roots
  ...
  do j=2,m
     x=roots(j)
     do i=j-1,1,-1
        if(dreal(roots(i)).le.dreal(x))go to 10
        roots(i+1)=roots(i)
     end do
     i=0
10   continue
