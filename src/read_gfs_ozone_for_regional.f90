...
!!   allocate necessary space on global grid
  ...
  if(use_gfs_nemsio)then
     if(hires) then
        call read_nemsatm(grd_gfst,filename,mype,sp_gfs,sp_b,uv_hyb_ens,.false.,.false.,z,ps,vor,div,u,v,tv,q,cwmr,oz)
     else
        call read_nemsatm(grd_gfst,filename,mype,sp_gfs,sp_gfs,uv_hyb_ens,.false.,.false.,z,ps,vor,div,u,v,tv,q,cwmr,oz)
     end if
  else
     if (hires) then
