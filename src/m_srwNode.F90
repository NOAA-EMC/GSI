...
!                .      .    .                                       .
! subprogram:	 module m_srwNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type srwNode (radar wind superobs)
!
! program history log:
...
! module interface:
  ...
  public:: srwNode_typecast
  public:: srwNode_nextcast
        interface srwNode_typecast; module procedure typecast_ ; end interface
        interface srwNode_nextcast; module procedure nextcast_ ; end interface

  character(len=*),parameter:: MYNAME="m_srwNode"
...
!-- cast a class(obsNode) to a type(srwNode)
  use m_obsNode, only: obsNode
  implicit none
  type(srwNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(srwNode)
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(srwNode),intent(inout):: aNode
...
_ENTRY_(myname_)
  ...
  if(skip_) then
    ...
    read(iunit,iostat=istat)    aNode%res1   , &
                                aNode%res2   , &
                                aNode%err2   , &
                                aNode%raterr2, &
                                aNode%b      , &
    ...
    aNode%diagv => obsdiagLookup_locate(diagLookup,aNode%idv,aNode%iob,2_i_kind)
                ...
                if(.not. (associated(aNode%diagu) .and. &
                  ...
                  call perr(myname_,'obsdiagLookup_locate(u), %idv =',aNode%idv)
...
_ENTRY_(myname_)
  ...
  jstat=0
  write(junit,iostat=jstat)     aNode%res1   , &
                                aNode%res2   , &
                                aNode%err2   , &
                                aNode%raterr2, &
                                aNode%b      , &
