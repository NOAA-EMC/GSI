...
!  end function compare_priority
  ...
  subroutine heapify(a,i_in)
    ...
    real(r_single)    :: pri_i, pri_l, pri_r, pri_largest
...
bigloop:  do
  ...
  real(r_single) function pq_maxpri(a)
    type(pq), pointer :: a
  ...
  real(r_single) function pq_replace_max(a,dis,idx) 
    !
...
!    type(kdtree2_result), intent(in) :: e
    ...
    integer :: parent, child, N
    real(r_single)    :: prichild, prichildp1
...
module kdtree2_module
  ...
  type :: tree_node
      ...
      real(r_single) :: cut_val
      ! where to cut the dimension
      real(r_single) :: cut_val_left, cut_val_right  
  ...
  type :: kdtree2
      ...
      real(r_single), pointer :: the_data(:,:) => null()
      ...
      real(r_single), pointer :: rearranged_data(:,:) => null()
  ...
  type :: tree_search_record
      ...
      logical           :: overflow
      real(r_single), pointer :: qv(:)  ! query vector
      type(kdtree2_result), pointer :: results(:) ! results
      type(pq) :: pq
      real(r_single), pointer :: data(:,:)  ! temp pointer to data
...
contains
  ...
  function kdtree2_create(input_data,dim,sort,rearrange) result (mr)
    ...
    ! ..
    ! .. Array Arguments ..
    real(r_single), target :: input_data(:,:)
    !
    integer :: i
    ! ..
    allocate (mr)
      ...
      if ((u-l)<=bucket_size) then
         ...
         if (.true.) then
            ...
            if (.true.) then
               ...
               average = sum(tp%the_data(c,tp%ind(l:u))) / real(u-l+1,r_single)
            else
...
!         res%cut_val = tp%the_data(c,tp%ind(m))
   ...
   subroutine spread_in_coordinate(tp,c,l,u,interv) 
      ...
      ! ..
      ! .. Local Scalars ..
      real(r_single) :: last, lmax, lmin, t, smin,smax
      integer :: i, ulocal
      ! ..
      ! .. Local Arrays ..
      real(r_single), pointer :: v(:,:)
      integer, pointer :: ind(:)
      ! ..
      v => tp%the_data(1:,1:)
      ind => tp%ind(1:)
  ...
  subroutine kdtree2_n_nearest(tp,qv,nn,results)
    ...
    type (kdtree2), pointer      :: tp
    real(r_single), target, intent (In)    :: qv(:)
...
!    deallocate(sr%pqp)
  ...
  subroutine kdtree2_n_nearest_around_point(tp,idxin,correltime,nn,results)
    ...
    sr%ballsize = huge(1.0)       ! the largest real(r_single) number
    sr%centeridx = idxin
  ...
  recursive subroutine search(node)
    ...
    integer                            :: cut_dim, i
    ! ..
    real(r_single)                               :: qval, dis
    real(r_single)                               :: ballsize
    real(r_single), pointer           :: qv(:)
...
!          dis=extra**2
  ...
  real(r_single) function dis2_from_bnd(x,amin,amax) result (res)
    real(r_single), intent(in) :: x, amin,amax
  ...
  subroutine process_terminal_node(node)
    ...
    real(r_single), pointer          :: qv(:)
    integer, pointer       :: ind(:)
    real(r_single), pointer          :: data(:,:)
    !
    integer                :: dimen, i, indexofi, k, centeridx, correltime
    real(r_single)                   :: ballsize, sd, newpri
    logical                :: rearrange
    type(pq), pointer      :: pqp 
    !
  ...
  subroutine kdtree2_n_nearest_brute_force(tp,qv,nn,results) 
    ...
    ! does.
    type (kdtree2), pointer :: tp
    real(r_single), intent (In)       :: qv(:)
    integer, intent (In)    :: nn
    type(kdtree2_result)    :: results(:) 

    integer :: i, j, k
    real(r_single), allocatable :: all_distances(:)
    ! ..
  ...
  subroutine heapsort(a,ind,n)
    ...
    !
    !
    real(r_single)        :: value   ! temporary for a value from a()
