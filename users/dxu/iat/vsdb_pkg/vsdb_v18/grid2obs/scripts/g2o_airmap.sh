...
*------------------------
  ...
  xmin=1.0; xmax=xmin+xwd; ymin=4.0; ymax=ymin+ywd
...
*--find maximum and minmum values
   ...
   i=i+1
   endwhile
   dist=cmax-cmin; cmin=cmin-0.01*dist; cmax=1.05*cmax
   ...
   while (i <= $nmd)
    ...
    if ( b>0 )
      ...
      'set xlint $xtick'
      'set cstyle 'cst.i; 'set cthick 2'; 'set cmark 0'; 'set ccolor 'cco.i
      if ( $var = bias )
       'd ${coeff}*(fcst(x='%i')-obs(x='%i'))'
      else
       'd ${coeff}*${var}(x='%i')'
      ...
      if ( $var = bias )
       ...
       'd tloop(ave(${coeff}*${var}(x='%i'),t-15,t+15))'
      endif
