...
export exedir=${exedir:-/stmp/$LOGNAME/g2osfc/plot/T}
...
## verification variable parameters: e.g. T, RH
export vnam=${1:-T}
...
while [ $cdate -le $edate ]; do
 ...
 while [ $fhour -lt $vlength ]; do
   for reg in $reglist; do
    mycheck=$( grep ${mdl} $vsdbname |grep " $fhour $cdate" |grep "ONLYSF G104/$reg SL1L2 $vnam $lev " )
    if [ $? -ne 0 ]; then
     echo "missing" >>$outname.txt
    else
      grep ${mdl} $vsdbname |grep " $fhour $cdate" |grep "ONLYSF G104/$reg SL1L2 $vnam $lev " |cat >>$outname.txt
    fi
   done
...
! for universal applications. 
       ...
       real*8             :: vsdb0(nreg,5,fnum,nday)
       real*4             :: points(fnum,nday,nmd)
       real*8             :: vsdb(5,fnum,nday)
...
! accumulate over all sub-regions
           do m=1,nreg
            if(nhead(m,i,j,n).eq.0) then
              ...
              read(11,*)points0(m,i,j),(vsdb0(m,k,i,j),k=1,5)
           ...
           if(points(i,j,n).eq.0) then
             ...
             enddo
             fcst(i,j,n)=vsdb(1,i,j)
             obs(i,j,n)=vsdb(2,i,j)
             cor(i,j,n)=(vsdb(3,i,j)-vsdb(1,i,j)*vsdb(2,i,j))/  &
                      sqrt((vsdb(4,i,j)-vsdb(1,i,j)**2)*(vsdb(5,i,j)-vsdb(2,i,j)**2))
             rms(i,j,n)=sqrt(max(0.0d0,vsdb(4,i,j)+vsdb(5,i,j)-2*vsdb(3,i,j)))
