...
if [ $MAKEVSDBDATA = YES ] ; then
   ...
   export expdlist="$myarch $myarch $myarch"  ;#exp directories, can be different
   export expnlist="cntrl 3polar 2polar"          ;#experiment names 
   export fcyclist="00"               ;#forecast cycles to be verified
   export complist="$chost  $chost $chost"  ;#computer names, can be different if passwordless ftp works 
   export dumplist=".gfs. .gfs. .gfs."      ;#file format pgb${asub}${fhr}${dump}${yyyymmdd}${cyc}
   export vhrlist="00 "               ;#verification hours for each day             
   export DATEST=20140525            ;#verification starting date
   export DATEND=20140715             ;#verification ending date
   export vlength=192                 ;#forecast length in hour
...
if [ $MAKEMAPS = YES ] ; then
   ...
   export mdlist="cntrl 3polar 2polar"              ;#experiment names, up to 10, to compare on maps
   export vsdblist="$vsdbsave $vsdbsave $vsdbsave";#vsdb stats directories 
   export vhrlist="00 "                 ;#verification hours for each day to show on map
   export DATEST=20140525               ;#verification starting date to show on map
   export DATEND=20140715               ;#verification ending date to show on map
   export vlength=192                   ;#forecast length in hour to show on map
