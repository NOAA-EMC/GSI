...
## 1. Compute precip skill scores over CONUS and performance Monte Carlo 
##    signoficance tests.
## 2. Use GrADS to make plots including significance bars. Maps will be 
##    sent to a web server for display. 
...
## 1. expand verification beyond 84 hours to any given forecast length. Add 
##    contour plots showing scores for all forecast hours in one map.
##---------------------------------------------------------------------------

#  ------------------------------------------
myhost=`echo $(hostname) |cut -c 1-1 `
export scrdir=${scrdir:-/global/save/Fanglin.Yang/VRFY/vsdb/precip_test}    

export expnlist=${expnlist:-"gfs prhw14"}                                  ;#experiment names, up to 10
export expdlist=${expdlist:-"/global/noscrub/emc.glopara/global/wgne /global/noscrub/Fanglin.Yang/archive"}  ;#archive directory
export complist=${complist:-"tide tide"}                               ;#computer name     
export DATEST=${DATEST:-20140108}                                          ;#forecast starting date
export DATEND=${DATEND:-20140501}                                          ;#forecast ending date
export cyclist=${cyclist:-"00"}                                            ;#forecast cycles for verify, 00Z and/or 12Z      
export vhour=${vhour:-"84"}                                                ;#forecast length for verification                
...
export gstat=${gstat:-/global/noscrub/emc.glopara/global}                  ;#global operational GFS stats directory   
...
export tmpdir=${rundir:-/stmpd2/$LOGNAME/pvrfy/${expname[1]}}
...
while [ $((nn+=1)) -lt $nexp ] ; do
 ...
 while [ $cdate -le ${DATEND} ]; do
   ...
   if [ $exp = "gfs" ]; then 
     filein=$gstat/wgne/$exp/${exp}_rain_${cdate}${affix}
     if [ ! -s $filein ]; then
