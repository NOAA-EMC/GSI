...
! arrays used for monte carlo test
      ...
      real, allocatable :: bisd(:,:,:,:),   etsd(:,:,:,:)                  !difference of scores
...
!--------------------------------------------------------
      ...
      read(argument,*) cycle       
      if (nargs.ge.6) then
       call getarg(6,argument)     !number of 24-h verification length
       read(argument,*) nfhr        
      else
       nfhr=3
      endif
      write(6,*) "nday=",nday," nexp=",nexp, " nreg=",nreg, " ntest=",ntest, " cycle=",cycle," nfhr=",nfhr
...
!
!-------------------------------------------------------------------
!--write out scores, all fcst hours in one file                           
      do id = 1, nday
       do ih = 1, nfhr
         write(20)((dbis(ic,ih,id,np),ic=1,ncat),np=1,nexp)
       enddo
       do ih = 1, nfhr
         write(20)((dets(ic,ih,id,np),ic=1,ncat),np=1,nexp)
       enddo
       do ih = 1, nfhr
         write(20)((float(obs(ic,ih,id,np)),ic=1,ncat),np=1,nexp)
       enddo
      enddo
      close(20)
...
!      enddo
      ...
      do ih = 1, nfhr
       write(30) ((bis(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((ets(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((obcount(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((mbis(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((mets(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((vbis(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((vets(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      do ih = 1, nfhr
       write(30) ((ngood(ic,ih,np),ic=1,ncat),np=1,nexp)
      enddo
      close(30)
      do np=1,nexp
...
!-------------------------------------------------------------------
      ...
      if(nexp .ge. 2) then
       ...
       allocate ( bisd(ncat,nfhr,ntest,nexp), etsd(ncat,nfhr,ntest,nexp) )
...
!!     write(6,'(31i3)') int(sign(1.0,randx-0.5))
 ...
 145     continue
         bisd(ic,ih,nr,np)=bis2(ic,ih,nr)-bis1(ic,ih,nr)
         etsd(ic,ih,nr,np)=ets2(ic,ih,nr)-ets1(ic,ih,nr)
 150    continue
 110  continue
      ...
      !derive standard deviaiton of score differences from monte carlo runs!
        ...
        bisdv=0.0; etsdv=0.0
         do nr=1,ntest
          bisdm=bisdm+bisd(ic,ih,nr,np)/ntest             
          etsdm=etsdm+etsd(ic,ih,nr,np)/ntest             
         enddo
         do nr=1,ntest
          bisdv=bisdv+(bisd(ic,ih,nr,np)-bisdm)**2         
          etsdv=etsdv+(etsd(ic,ih,nr,np)-etsdm)**2         
         enddo
...
!        do nr=1,ntest
!         write(6,'(i7, 9e12.4)') nr,(bisd(ic,ih,nr,np),ic=1,ncat)
!        enddo
!        write(6,*)"--- Monte Carlo differences of ETS:  model ",np, " fcst hour ", 12+ih*24
!        do nr=1,ntest
!         write(6,'(i7, 9e12.4)') nr,(etsd(ic,ih,nr,np),ic=1,ncat)
!        enddo
 ...
 100   continue
       ...
       do ih = 1, nfhr
         write(40) ((std_bis(ic,ih,np),ic=1,ncat),np=1,nexp)
       enddo
       do ih = 1, nfhr
         write(40) ((std_ets(ic,ih,np),ic=1,ncat),np=1,nexp)
       enddo

       do ih = 1, nfhr
        do nr=1,ntest
         write(50) ((bisd(ic,ih,nr,np),ic=1,ncat),np=1,nexp)
        enddo
        do nr=1,ntest
         write(50) ((etsd(ic,ih,nr,np),ic=1,ncat),np=1,nexp)
        enddo
       enddo
