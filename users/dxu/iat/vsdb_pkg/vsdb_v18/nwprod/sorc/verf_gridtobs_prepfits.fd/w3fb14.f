...
C 2003-06-21  GILBERT     MODIFIED FROM W3FB12 AND ADDED SUPPORT FOR
C                         SECANT CONE AS WELL AS TANGENTIAL.
C
C USAGE:  CALL W3FB14(XI,XJ,ALAT1,ELON1,DX,ELONV,ALATAN1,ALATAN2,ALAT,ELON,IERR,
C                                   IERR)
...
C                THE CONE FLAT.
C     ALATAN1  - THE 1ST LATITUDE AT WHICH THE LAMBERT CONE IS TANGENT TO
...
C
           IF (NEWMAP) THEN
             ANINV = 1./AN
             ANINV2 = ANINV/2.
C             THING = ((AN/REBYDX) ** ANINV)/
C     &         ((COSLTN**((1.-AN)*ANINV))*(1.+ AN))
           ENDIF
           
           STH=sqrt(r2)
c           ALAT = H*(PIBY2 - 2.*ATAN(THING*(R2**ANINV2)))*DEGPRD
