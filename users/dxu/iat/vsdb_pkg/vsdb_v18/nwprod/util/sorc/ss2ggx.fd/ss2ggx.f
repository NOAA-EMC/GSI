...
!  Feb 2015, Fanglin Yang - reduce number of output to save CPU
!
  use sigio_module
  implicit none
  integer narg,iargc
...
!fy call sptez(0,head%jcap,idrt,imax,jmax,data%hs,f1,1)
!fy call wryte(luggg,4*imax*jmax,f1)
...
!fy  call wryte(luggg,4*imax*jmax*head%levs,pm)
!fy  call wryte(luggg,4*imax*jmax*head%levs,g2)
! call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,data%t,g1,1)
...
!
  ...
  call wryte(luggg,4*imax*jmax*head%levs,g1)
  call wryte(luggg,4*imax*jmax*head%levs,g2)
!fy  call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,data%d,g1,1)
!fy  call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,data%z,g2,1)
!fy  call wryte(luggg,4*imax*jmax*head%levs,g1)
!fy  call wryte(luggg,4*imax*jmax*head%levs,g2)

!fy  do k=1,head%levs
!fy    vp(:,k)=data%d(:,k)
!fy    sf(:,k)=data%z(:,k)
!fy    call splaplac(0,head%jcap,enn1,vp(1,k),vp(1,k),-1)
!fy    call splaplac(0,head%jcap,enn1,sf(1,k),sf(1,k),-1)
!fy    vp(1:2,k)=0.
!fy    sf(1:2,k)=0.
!fy  enddo
!fy  call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,vp,g1,1)
!fy  call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,sf,g2,1)
!fy  call wryte(luggg,4*imax*jmax*head%levs,g1)
!fy  call wryte(luggg,4*imax*jmax*head%levs,g2)
...
!fy  write(luctl,'("vars",i6)') 12+head(1)%ntrac
  write(luctl,'("vars",i6)') 5+head(1)%ntrac
!fy  write(luctl,'("HS  ",i3," 99 surface orography (m)")') 1
  write(luctl,'("PS  ",i3," 99 surface pressure (Pa)")') 1
!fy  write(luctl,'("P   ",i3," 99 pressure (Pa)")') head(1)%levs
!fy  write(luctl,'("DP  ",i3," 99 delta pressure (Pa)")') head(1)%levs
...
!fy  write(luctl,'("DIV ",i3," 99 divergence (m/s**2)")') head(1)%levs
!fy  write(luctl,'("VOR ",i3," 99 vorticity (m/s**2)")') head(1)%levs
!fy  write(luctl,'("VP  ",i3," 99 potential (m)")') head%levs
!fy  write(luctl,'("SF  ",i3," 99 streamfcn (m)")') head%levs
