...
!
      ...
      integer, parameter ::  n3d=12          !number of 3D variable   
...
!--define common GRIB header information 
      ...
      IBMS=1     !BITMAP FLAG (0 FOR NO BITMAP)
...
!  note the north-south direction is resversed.
      do n=1,ns  
        ...
        do i=1,nlon
          ...
          do j=2,nlat-1
           if(vtmp(i,j-1).ne.bad .and. vtmp(i,j).ne.bad) then
...
!  convert surface and sea-level pressure unit from hPa to Pa
      do j=1,nlat
      do i=1,nlon
       if(var(i,j,n3d*nlev+1).ne.bad) 
     +    var(i,j,n3d*nlev+1)=100*var(i,j,n3d*nlev+1)
       if(var(i,j,n3d*nlev+2).ne.bad) 
     +    var(i,j,n3d*nlev+2)=100*var(i,j,n3d*nlev+2)
       if(var(i,j,n3d*nlev+4).ne.bad) 
     +    var(i,j,n3d*nlev+4)=var(i,j,n3d*nlev+4)+var(i,j,n3d*nlev+3)
      enddo
      enddo


      do k=5*nlev+1,6*nlev
      do j=1,nlat
      do i=1,nlon
      if(var(i,j,k).ne.bad .and. var(i,j,k+2*nlev).ne.bad) then
       var(i,j,k)=var(i,j,k)+var(i,j,k+2*nlev)     
      endif
      enddo
      enddo
      enddo
...
!--write 2D fields
      do 300 nv=1,n2d
       ...
       IL2=IL1               !SECOND LEVEL VALUE
       ITR=10                !TIME RANGE INDICATOR (10 FOR SINGLE PERIOD)
      if(nv.ge.3 .and. nv.le.4) then   !for time-averaged fluxes
        ITR=4    
        IFHR=0       !assume flux bucket is 6 hours            
      endif
