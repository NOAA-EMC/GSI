...
#------------------------------------------------------------
#--for vector variable correlation and rms on multiple layer  
#------------------------------------------------------------
#------------------------------------------------------------
...
export exedir=${exedir:-/stmp/$LOGNAME/g2oair/plot/VWND}
...
export vnam=${2:-VWND}         ;#variable: e.g. VWND
 ...
 while [ $fhour -lt $vlength ]; do
  for lev in $levlist ; do
   for reg in $reglist; do
    ...
    mycheck=$( grep ${mdl} $vsdbname |grep " $fhour $cdate" |grep "$obtype $reg VL1L2 $vnam $levid" )
    if [ $? -ne 0 ]; then
     echo "missing" >>$outname.txt
    else
     grep ${mdl} $vsdbname |grep " $fhour $cdate" |grep "$obtype $reg VL1L2 $vnam $levid" |cat >>$outname.txt
    fi
   done
  done
...
! for universal applications. 
       ...
       real*8             :: vsdb0(ns,nreg,nlev,fnum,nday)
       real*4             :: points(nlev,fnum,nday,nmd)
       real*8             :: vsdb(ns,nlev,fnum,nday)
...
! accumulate over all sub-regions
           do m=1,nreg
            if(nhead(m,l,i,j,n).eq.0) then
              ...
              read(11,*)points0(m,l,i,j),(vsdb0(k,m,l,i,j),k=1,ns)
           ...
           if(points(l,i,j,n).eq.0) then
             ...
             enddo
             fcst(l,i,j,n)=sqrt(vsdb(6,l,i,j))
             obs(l,i,j,n)=sqrt(vsdb(7,l,i,j))
             cor(l,i,j,n)=vsdb(5,l,i,j)/sqrt(vsdb(6,l,i,j)*vsdb(7,l,i,j))
             rms(l,i,j,n)=sqrt(max(0.0d0,vsdb(6,l,i,j)+vsdb(7,l,i,j)-2*vsdb(5,l,i,j)))
             bias(l,i,j,n)=sqrt(vsdb(6,l,i,j))-sqrt(vsdb(7,l,i,j))
