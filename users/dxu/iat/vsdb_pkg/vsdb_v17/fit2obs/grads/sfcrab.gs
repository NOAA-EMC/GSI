...
'set string 2 tl 6'
'set strsiz 0.15'
'draw string 1 10.8 'vartit' RMSE in 'units
'draw string 2 10.6 from 'ts'-'te
*
*grfile=pdir'/'var'.all.'titreg'.rmse.gr'
*say grfile
*'enable print 'grfile
giffile=pdir'/'var'.all.'titreg'.rmse.gif'
say giffile
*
fcsx=1
while (fcsx <= 6)
...
*
'set gxout stat'
'd maskout(p'f'e.'fl1'(time='tsx'),pc.'fl1'(time='tsx')-'minday')'
say result
...
word=subwrd(line,4)
sfcrms=digs(word,2)
'd maskout(p'f'e.'fl2'(time='tsx'),pc.'fl2'(time='tsx')-'minday')'
say result
line=sublin(result,7)
...
'd maskout(oacres(grid.1(t=1),maskout(p'f'e.'fl1'(time='tsx'),pc.'fl1'(time='tsx')-'minday')),mask.2(t=1)-0.9)'
*
** over ocean
'set clevs .4 .8 1.2 1.6  2 2.4 2.8 3.2 3.6  4'
'set ccols 71 72  73  74 75  21  22  23  24 25 26'
'd maskout(oacres(grid.1(t=1),maskout(p'f'e.'fl2'(time='tsx'),pc.'fl2'(time='tsx')-'minday')),0.9-mask.2(t=1))'
*
'run 'gdir'/cbarnew.gs'
'draw title 'fcstit' 'titfcs' RMSE LAND : 'sfcrms' OCEAN 'shprms 
*
if(fcsx=1)
'set gxout contour'
** over land
