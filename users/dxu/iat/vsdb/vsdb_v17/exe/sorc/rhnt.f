        subroutine RHNT(nvsdb,Nmodel,ensname,ist,numfcst,
     +    numvfyobs,numarea,numvarbl,numlevel,ngrid,levels,
     +    hasdata,fcstmdl,obsvdata,ufcstmdl,vfcstmdl,
     +    uobsv,vobsv)

c    This program is to generate RHNT partial sum for ensemble forecast
...
c          March 5 2007
     ...
     +          obsvdata,uobsv,vobsv
      ...
      real*8,  allocatable, dimension(:,:,:,:,:) :: weigh
      real*8,   allocatable, dimension(:,:,:,:,:,:) :: rhn
      real*8   dif(Nmodel)               
...
C
     ...
     +     afhomrk,afho,nchrafho,afhothr,nchrafhothr,rafhothr
      ...
      write(*,*) 'In RHNT:',Nmodel,ist,numfcst,numvfyobs,numarea,
     +  numvarbl,numlevel,ngrid
      ...
      do i=1,numfcst
       ...
       do j=1,numvarbl
        ...
        allocate (rhn(numfcst,numvarbl,numlevel,numarea,numvfyobs,
     +                Nmodel))
...
CCCCCCCCCCC       RHNT  CCCCCCCCCCCCCCCCCCCCCCCCCC 

            if(vectormrk(ivr).eq.0) then        
              do imodel=1,Nmodel                                 
               dif(imodel) = abs( fcstmdl(imodel,ifh,ivr,ilv,i) - 
     +               obsvdata(ifh,ivr,ilv,i))*area_factor(i)
              end do
            else
              do imodel=1,Nmodel
               wf = sqrt( ufcstmdl(imodel,ifh,ivr,ilv,i)**2 +
     +                    vfcstmdl(imodel,ifh,ivr,ilv,i)**2)
               wo = sqrt( uobsv(ifh,ivr,ilv,i)**2 +
     +                    vobsv(ifh,ivr,ilv,i)**2)
               dif(imodel)=abs(wf-wo)*area_factor(i)
              end do
            end if
         
              ibest_model=minloc(dif, dim=1, mask=dif.ne.9999. )
              rhn(ifh,ivr,ilv,iar,iob,ibest_model) =
     +           rhn(ifh,ivr,ilv,iar,iob,ibest_model) +
        ...
        do 71 iar = 1, numarea
         do 61 ivr = 1, numvarbl
          ...
          do 51 ilv = 1, levels(ivr)
           ...
           do  imodel = 1, Nmodel
            rhn(ifh,ivr,ilv,iar,iob,imodel)= 
     +      rhn(ifh,ivr,ilv,iar,iob,imodel)/
     +      weigh(ifh,ivr,ilv,iar,iob)
           end do
...
c     +                           CNmodel(:numCNmodel)
     ...
     +     (rhn(ifh,ivr,ilv,iar,iob,imodel), imodel=1,Nmodel-1)      

 1000                   FORMAT (A,F7.0,<Nmodel-1>(1x,F7.5))
                       end if
                    END IF
150            continue
160           continue
