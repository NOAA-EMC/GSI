      subroutine VAL1L2(nvsdb,imodel,ist,numfcst,numvfyobs,numarea,
     +  numvarbl,numlevel,ngrid,levels,plevel,hasdata,
     +  ufcstx,vfcstx, uobsvx,vobsvx, uclim,vclim,tendencymrk)
...
C
     ...
     +     afhomrk,afho,nchrafho,afhothr,nchrafhothr,rafhothr
      ...
      write(*,*) 'In VAL1L2:',imodel,ist,numfcst,numvfyobs,numarea,
     +  numvarbl,numlevel,ngrid
      ...
      do ifh=1,numfcst
       do ivr = 1, numvarbl
        if(anomly_mrk(ivr).eq.1.and.
     +    vectormrk(ivr).eq.1.and.tendencymrk(ivr).eq.0) then
          do ilv=1,levels(ivr)
            if(anomlylev(ivr,ilv).eq.1) then
             do i=1, ngrid
                ufcst(ifh,ivr,ilv,i)=ufcstx(ifh,ivr,ilv,i)
     +                        -uclim(ifh,ivr,ilv,i)
                vfcst(ifh,ivr,ilv,i)=vfcstx(ifh,ivr,ilv,i)
     +                        -vclim(ifh,ivr,ilv,i)
                uobsv(ifh,ivr,ilv,i)=uobsvx(ifh,ivr,ilv,i)
     +                        -uclim(ifh,ivr,ilv,i)
                vobsv(ifh,ivr,ilv,i)=vobsvx(ifh,ivr,ilv,i)
     +                        -vclim(ifh,ivr,ilv,i)
             end do
            end if
          end do
      ...
      write(*,*)'fcst:', ifh
       do ivr = 1, numvarbl
       write(*,*) 'ivr:',ivr,':anomly_mrk=',
     +   anomly_mrk(ivr),
     +  ' vectormrk=', anomly_mrk(ivr)
        if(anomly_mrk(ivr).eq.1.and.
     +    vectormrk(ivr).eq.1) then
          do ilv=1,levels(ivr)
           write(*,*) '   ilv=',ilv,
     +      ':anomlylev=',anomlylev(ivr,ilv)
            if(anomlylev(ivr,ilv).eq.1) then
              write(*,*) 'levels=',levels(ivr)
               write(*,*) 'ufcstx,vfcstx=',
     +           (ufcstx(ifh,ivr,ilv,i),i=1,5),
     +           (vfcstx(ifh,ivr,ilv,i),i=1,5)
               write(*,*) 'uobsvx,vobsvx=',
     +           (uobsvx(ifh,ivr,ilv,i),i=1,5),
     +           (vobsvx(ifh,ivr,ilv,i),i=1,5)
               write(*,*) 'uclim,vclim=',
     +           (uclim(ifh,ivr,ilv,i),i=1,5),
     +           (vclim(ifh,ivr,ilv,i),i=1,5)
            end if
          end do
        end if
       end do
        ...
        do 80 iob = 1, numvfyobs
          do 70 iar = 1, numarea
           do 60 ivr = 1, numvarbl
             if(vectormrk(ivr).eq.0) goto 60      !scaler in SAL1L2
             if(tendencymrk(ivr).ne.0) goto 60    !tendency has no anomly computation
