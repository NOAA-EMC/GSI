       parameter(nvar=1,nlev=1,nstcs=6,nstcw=9)
...
c
       ...
       write(*,*) "incr= ",incr
c
       iname = trim(mdldir)//'/f'//trim(fcshr)//'.sfc.' 
       write(*,*) "iname= ",iname
c
...
c
      do ivar=1,nvar
      do ilev=1,nlev
      do isub=1,1   ! this is for land surface only
      do ireg=1,nreg; bincor=0
      do nt=1,ntime
      ...
      if(ivar==3) then
         ...
         xcnt(nt) = cnt
         xrms(nt) = sqrt(max(uvf+uvo-2*uv,0.))
         xavg(nt) = spd 
      else
