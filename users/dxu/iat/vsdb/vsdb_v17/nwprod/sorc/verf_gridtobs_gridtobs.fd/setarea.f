...
C-------------------------------------------------------------------------
      nam24 = namarea(1:nchr)
      mode(iar) = 0
      iqstn = INDEX ( nam24, '.STNS' )
      print*,'iar=',iar,' nam24(1:1)=',nam24(1:1),' iqstn =',iqstn
      IF (nam24(1:1).eq.'G' .and. iqstn .eq. 0 ) THEN
        igrid=-999
        READ (nam24(2:4),'(I3)',iostat=iost) igrid
         print*,nam24(1:nchr), ' iostat=',iost,' igrid in setarea ',igrid
         if(iost.ne.0) then
          print*,'reset igrid to 3 '
          igrid=3   !if nam24(2:4) are not numbers, Fanglin Yang
         endif
!!      READ (nam24(2:4),'(I3)') igrid
        print*,'igrid=',igrid
