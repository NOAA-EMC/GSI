...
C   PRGMMR: HENRICHSEN       ORG: W/NP12PH       DATE:1999-08-20
C
...
C
C     (1.) MAP -   INTEGER*4   MAP(15) IS A LIST CONTAINING:
...
C          . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
C     (2.) LABEL - INTEGER*4   LABEL(2048)
...
ckumar
        print*,'In CNTR Before MERGES - ITAPE ',ITAPE
ckumar
...
C     ... OTHERWISE, INITIALIZE FOR NEW OUTPUT,
      ...
      IF(IFID(1) .EQ. 0) GO TO 19
        CALL GBYTES(IFID,KFID,0,32,0,12)
        IOUT(1)=IOR(IAND(MSK,KFID(1)),IOUT(1))
        LCKPT = 2137
        PRINT  888, LCKPT
        DO  L=2,12
          IOUT(L)=KFID(L)
