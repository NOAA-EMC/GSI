...
C     ...   so convert it into CRAY REAL 
       ...
       iret = ibm2cray(2,nitems,loneibm,nbitoffset,crayreal)
C                 type=2; for IBM REAL*4

       if(iret .LT. 0) then
         write(6,315)iret
  315    format(1h ,'FP02_LN2::ibm2cray FAILED with return code=',I8)
C        ... TO LET IT GO ON ANYWAY,
         crayreal = 0.0
       endif
       if (lcheckout) then
         write(6,325)loneibm, crayreal
  325    format(1h ,'FP02_LN2::ibm2cray: R*4  ibm = HEX',Z17.16,
     1         /1h ,'                    R*8 cray = DEC',E13.5)
       endif
