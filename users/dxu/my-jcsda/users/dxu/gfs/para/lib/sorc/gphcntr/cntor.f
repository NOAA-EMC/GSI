...
C   97-05-06  LUKE LIN      MODIFY FOR TDL MOS TSTM/SVR WX LAND-SEA FIELD.
C   97-07-11  LUKE LIN      MODIFY FOR TDL 4-PANEL TRAJ AND ETA TYPE 5 POLE CLIP.
C 1999-06-01  KRISHNA KUMAR CONVERT THIS CODE TO IBM RS/6000 SYSTEM
C                           A LOGICAL ARRAY OVERWRITE WAS FIXED ON IBM RS/6000
C                           ASSIGNED PROPER VALUES FOR INDEF/XINDEF USING RANGE
C                           FUNCTION FOR IBM RS/6000
C
...
C          ... FOR TDL MOS THUNDERSTORM/SEVERE WX CHART
           CALL INDFSEA(    Z, IMZIN, JMZIN,NDIV, IMZ, JMZ,XINDEF)
           FINDEF = .TRUE.
         ELSE IF (KEYIDX.EQ.51.OR.KEYIDX.EQ.48.OR.KEYIDX.EQ.47) THEN
C          ... FOR TRAJECTORY 4-PANEL CHART
           CALL INDFTRAJ(  Z, IMZIN, JMZIN,NDIV, IMZ, JMZ,XINDEF)
           FINDEF = .TRUE.
         ENDIF
      ENDIF
      IF (PCLPFG) THEN
         IF (GDTYPE.EQ.05) THEN
C          ... FOR GRID TYPE 5 TO CLIP THE POLE AREA
           CALL INDFSLFM(  Z, IMZIN, JMZIN,NDIV, IMZ, JMZ,XINDEF)
           FINDEF = .TRUE.
         ENDIF
      ENDIF
C
         ...
         DO J=1,JMZ
            DO I=1,IMZ
              IF(Z(I,J) .GT. ZC) THEN
               ...
               ENDIF
            ENDDO
C           IF (GDTYPE .EQ.27 .OR. ECLEAR) THEN
            IF (FINDEF) THEN
               DO  I=1,IMZ-1
