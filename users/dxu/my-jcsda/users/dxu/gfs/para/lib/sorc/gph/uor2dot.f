      SUBROUTINE UOR2DOT(XUORS,YUORS,IDOT,JDOT,IUOPTN)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    UOR2DOT      CONVERT UORS COORDINATES
...
C              
C
C USAGE:    CALL UOR2DOT(XUORS,YUORS,IDOT,JDOT,IUOPTN,LISNHAWIP)
...
C                =5 - NH4004
C     LISNHAWIP   - LOGICAL*1 SWITCH FOR DESTINATION= N. HEMI. AWIPS
C
...
C
       DATA     T1         / 54.0,  37.5, 37.5, 37.5, 37.5 /
       DATA     XPO        / 17.0,  33.0, 27.0, 33.0, 33.0 /
       DATA     YPO        / 46.0,  33.0, 46.0, 45.0, 33.0 /
       DATA     XADJUS     / -1.0,  -8.0,  0.0,  0.0, -9.0 /
       DATA     YADJUS     / -4.0,  -0.0,  0.0,  0.0, -7.0 /
...
C        END IF
C
C        print *, ' x/yuors=', xuors, yuors 
         ISS = IUOPTN
         IF((ISS .LT. 1) .OR. (ISS .GT. 5)) ISS = 2
         CUORGR = CU2GI(ISS)
         XPOLE = XPO(ISS)
         YPOLE = YPO(ISS)
         DOTSGI = T1(ISS)
