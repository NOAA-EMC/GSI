...
export FNOROG=${FNOROG_ENKF:-$FIXGLOBAL/global_orography.t$JCAP.grb}
export FNMASK=${FNMASK_ENKF:-$FIXGLOBAL/global_slmask.t$JCAP.grb}
export OROGRAPHY=${OROGRAPHY_ENKF:-$FIXGLOBAL/global_orography.t$JCAP.grb}
export OROGRAPHY_UF=${OROGRAPHY_UF_ENKF:-$FIXGLOBAL/global_orography.t$JCAP.$LONB.$LATB.grb}
export SLMASK=${SLMASK_ENKF:-$FIXGLOBAL/global_slmask.t$JCAP.grb}
export MTNVAR=${MTNVAR_ENKF:-$FIXGLOBAL/global_mtnvar.t$JCAP.f77}
...
if [[ -s $EFMNSTAT ]]; then
   ...
   if [[ $size -ne $NMEM_ENKF ]]; then
      echo "***WARNING*** $size entries in $EFMNSTAT instead of $END_NUM_ENKF"
      exit 98
   fi
else
