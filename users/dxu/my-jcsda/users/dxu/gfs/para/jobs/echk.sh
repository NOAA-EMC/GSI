...
SIGANL_HIRES=${SIGANL_HIRES:-$COMROT/siganl.$CDFNL.$CDATE}
...
if [[ $ECHKTYPE = siganl ]]; then
  nsleep=0
  tsleep=10
  msleep=30
  until [[ -s $SIGANL_HIRES || $((nsleep+=1)) -gt $msleep ]];do sleep $tsleep;done
  if [[ $nsleep -gt $msleep ]]; then
    echo "$SIGANL_HIRES does not exist"
    export SIGANLEX="NO"
