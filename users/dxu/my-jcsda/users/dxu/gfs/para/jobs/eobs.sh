...
export INSITUINFO=${INSITUINFO:-${FIXGLOBAL}/global_insituinfo.txt}
...
export BERROR=${BERROR_ENKF:-${FIXGLOBAL}/global_berror.l${LEVS}y${NLAT_A}.f77}
...
export SHIPSBF=${SHIPSBF:-$COMIN_DUMP/ships.$CDUMP.$CDATE}
...
export SETUP="miter=0,niter=1,lread_obs_save=.true.,lread_obs_skip=.false.,lwrite_predterms=.true.,lwrite_peakwt=.true.,reduce_diag=.true.,passive_bc=.false.,newpc4pred=.true.,adp_anglebc=.true.,angord=4,use_edges=.false.,diag_precon=.true.,step_start=1.e-3,emiss_bc=.true.,"
