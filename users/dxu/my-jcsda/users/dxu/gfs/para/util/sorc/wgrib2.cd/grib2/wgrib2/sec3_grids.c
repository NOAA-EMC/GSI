...
/* create the grib2 sec3 and latitudes and longitudes for lat-lon grid */
...
unsigned char *sec3_lola(int nx, double x0, double dx, int ny, double y0, double dy, 
    ...
    int_char((int) (y0*ANGLE_FACTOR), gds+46);        /* lat1 */
    uint_char((int) (x0*ANGLE_FACTOR), gds+50);        /* lon1 */

    gds[54] = 32+16;			/* flag table 3.3 */

    r = y0 + (ny-1)*dy;
    int_char((int) (r*ANGLE_FACTOR), gds+55);        /* lat1 */
    r = x0 + (nx-1)*dx;
    if (r >= 360.0) r -= 360.0;
    if (r < 0.0) r += 360.0;
    uint_char((int) (r*ANGLE_FACTOR), gds+59);        /* lon1 */

    int_char((int) (fabs(dx) * ANGLE_FACTOR), gds+63);        /* Di i direction increment */
    int_char((int) (fabs(dy) * ANGLE_FACTOR), gds+67);        /* Dj i direction increment */

    gds[71] = 0;                                /* scanning mode */
    if (dx < 0.0) gds[71] |= 128;
    if (dy > 0.0) gds[71] |= 64;

    /* now for the lat and lon values */

    if (lon == NULL || lat == NULL) printf(" no lat-lon ");

    if (lon == NULL || lat == NULL) return gds;
    *lon = (double *) malloc(nx * ny * sizeof(double));
    *lat = (double *) malloc(nx * ny * sizeof(double));
    if (*lon == NULL || *lat == NULL) fatal_error("sec3_lola: memory allocation","");
...
unsigned char *sec3_gaussian(int nx, double x0, double dx, int ny, double y0, 
    ...
    r = -y0;
    int_char((int) (r*ANGLE_FACTOR), gds+55);        /* lat2 */
    r = x0 + (nx-1)*dx;
    if (r >= 360.0) r -= 360.0;
    if (r < 0.0) r += 360.0;
    uint_char((int) (r*ANGLE_FACTOR), gds+59);        /* lon2 */
...
/*
 * create the grib2 sec3 and latitudes and longitudes for mercator
 */
...
unsigned char *sec3_mercator(double lad, int nx, double x0, double dx, int ny, double y0, double dy,
    ...
    uint_char(nx, gds+30);
    uint_char(ny, gds+34);
    int_char((int) (y0*ANGLE_FACTOR), gds+38);        /* lat1 */
    uint_char((int) (x0*ANGLE_FACTOR), gds+42);        /* lon1 */
    int_char((int) (lad*ANGLE_FACTOR), gds+47);
    r = y0 + (ny-1)*dy;
    int_char((int) (r*ANGLE_FACTOR), gds+51);        /* lat2 */
    r = x0 + (nx-1)*dx;
    if (r >= 360.0) r -= 360.0;
    if (r < 0.0) r += 360.0;
    uint_char((int) (r*ANGLE_FACTOR), gds+55);        /* lon2 */
    int_char((int) (0*ANGLE_FACTOR), gds+60);        /* only handle 0 orient */

    int_char((int) (fabs(dx) * ANGLE_FACTOR), gds+64);        /* Di i direction increment */
    int_char((int) (fabs(dy) * ANGLE_FACTOR), gds+68);        /* Dj i direction increment */
...
/*
 * create the grib2 sec3 and latitudes and longitudes for lambert conformal (conic) grid
 */
...
unsigned char *sec3_lc(double lov, double lad, double latin1, double latin2, int proj,
    ...
    int_char((int) (lad*ANGLE_FACTOR), gds+47);
    uint_char((int) (lov*ANGLE_FACTOR), gds+51);
    int_char((int) fabs(dx*1000), gds+55);        /* Di 1/1000 of a meter */
    int_char((int) fabs(dy*1000), gds+59);        /* Dj 1/1000 of a meter */
    gds[63] = proj;
    gds[64] = 0;                                /* scanning mode */
    if (dx < 0.0) gds[64] |= 128;
    if (dy > 0.0) gds[64] |= 64;
    int_char((int) (latin1*ANGLE_FACTOR), gds+65);
    int_char((int) (latin2*ANGLE_FACTOR), gds+69);
...
/*
 * create the grib2 sec3 and latitudes and longitudes for polar stereographic
 */
