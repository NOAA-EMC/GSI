...
int f_grib_ieee(ARG1) {
	...
	strcat(filename, ".grb");
	if (( save->grib = (void *) ffopen(filename, "wb") ) == NULL)
		fatal_error("Could not open %s", filename);

	strcpy(filename, arg1);
	strcat(filename, ".head");
	if ((save->head = (void *) ffopen(filename, "wb") ) == NULL)
		fatal_error("Could not open %s", filename);

	strcpy(filename, arg1);
	strcat(filename, ".tail");
	if ((save->tail = (void *) ffopen(filename, "wb") ) == NULL)
		fatal_error("Could not open %s", filename);

	strcpy(filename, arg1);
	strcat(filename, ".h");
	if ((save->c = (void *) ffopen(filename, "wb") ) == NULL)
		fatal_error("Could not open %s", filename);
    }
    else if (mode >= 0) {
        save = *local;
