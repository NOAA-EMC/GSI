...
struct function functions[] = {
   ...
   {"code_table_4.11",f_code_table_4_11, inv, 0, "code table 4.11 type of time intervals", -1},
   {"code_table_4.15",f_code_table_4_15, inv, 0, "code table 4.11 type of areal statistical processing", -1},
   ...
   {"ctl_ens",f_ctl_ens, inv, 0, "ens info for g2ctl/GrADS", 100},
   {"ctl_inv",f_ctl_inv, inv, 0, "ctl inventory dump for g2ctl/GrADS", 100},
   {"cyclic",f_cyclic, inv, 0, "is grid cyclic? (not for gaussian)", -1},
   ...
   {"get_byte",f_get_byte, inv, 3, "get bytes in Section X, location Y (1..N), number of bytes Z", 100},
   {"get_int",f_get_int, inv, 3, "get ints in Section X, location Y (byte), number of ints Z", 100},
   ...
   {"lev0",f_lev0, inv, 0, "level for g2ctl/GrADS", 200},
   ...
   {"misc",f_misc, inv, 0, "-ens -prob", 100},
   ...
   {"new_grid_winds",f_new_grid_winds, misc, 1, "new_grid wind rotation: X = grid, N/S (default) -- alpha", 111},
   ...
   {"set_byte",f_set_byte, misc, 3, "set bytes in Section X, location Y (1..N), bytes Z (a|a:b:c)", 100},
   ...
   {"lola",f_lola, output, 4, "lon-lat grid values X=lon0:nlon:dlon Y=lat0:nlat:dlat Z=file A=[bin|text|spread]", 100},
   {"merge_fcst",f_merge_fcst, output, 2, "normalize NCEP-type ave/acc X=number Y=output grib file", 100},
   ...
   {"g2clib",f_g2clib, setup, 1, "X=0/1 g2clib for decoding grib", 100},
