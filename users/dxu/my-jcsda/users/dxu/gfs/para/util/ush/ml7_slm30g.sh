...
# Check arguments
if [[ $# -ne 10 ]] ; then
 echo Usage: $0 slmgb orogb mtnvar14 IM JM NM filter1 filter2 MTNRES orogb_uf >&2
 exit 1
fi
#
...
#
export WRKDIR=${WRKDIR:-$ptmp/$LOGNAME/terr_wrkdir$$}

export lin=${lin:-""}
export HOMEDIR=${UTILDIR:-/global/save/emc.glopara/svn/gfs/trunk/para/util}
export NWPROD=${NWPROD:-/nwprod}
export FIXDIR=${FIXDIR:-$NWPROD/fix}
export LONSPERLAT=${LONSPERLAT:-$FIXDIR/global_lonsperlat.t${jcap}$lin.txt}
export TERRAINEXEC=${TERRAINEXEC:-$HOMEDIR/exec/terrain.x}
################################################################################
