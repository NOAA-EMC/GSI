...
c
     ...
     &                 ,iggret)
      ...
      call tracker (imax,jmax,grdspc,inp,maxstorm,numtcv
...
c
c---------------------------------------------------------------------
      subroutine tracker (imax,jmax,grdspc,inp,maxstorm,numtcv
     &          ,ifhours,igetpds,igetgds,trkrinfo,itret)
c
...
c             from the model forecast data itself, not the obs.
     ...
     &              fixlat(ist,ifh-1) > -990.0) then
                  ...
                  ! time level.
                  call get_phase (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
...
!GPL                else
  ...
  617               format (1x,'+++ RPT_STORM_MOTION: istmspd= ',i5
     &                     ,' istmdir= ',i5,' rcc= ',i3)
                    ...
                    imeanzeta = -99
                    igridzeta = -99
                    call get_zeta_values (fixlon,fixlat,imax,jmax,grdspc
...
c
c-----------------------------------------------------------------------
      subroutine get_phase (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
     &                    ,fixlon,fixlat,valid_pt,maxstorm
     &                    ,cps_vals,igpret)
c
...
c
 ...
 615  format(1x,'#---------------------------------------------------#')
      ...
      if (phasescheme == 'cps') then
        ...
        call get_cps_paramb (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
     &               ,fixlon,fixlat,valid_pt,paramb,maxstorm,igcpret)

        call get_cps_vth (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
...
c
c-----------------------------------------------------------------------
      subroutine get_cps_paramb (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
     &                 ,fixlon,fixlat,valid_pt,paramb,maxstorm,igcpret)
c
...
c
c-----------------------------------------------------------------------
      subroutine get_cps_vth (imax,jmax,inp,grdspc,ist,ifh,trkrinfo
...
c-----------------------------------------------------------------------
     ...
     &          ,grdspc,modelid,valid_pt,readflag,maxstorm,istmspd
...
c
c-----------------------------------------------------------------------
      subroutine get_zeta_values (fixlon,fixlat,imax,jmax,grdspc
     &                     ,trkrinfo,imeanzeta,igridzeta,readflag
     &                     ,valid_pt,ist,ifh,maxstorm,igzvret)
c
...
c     the atcfunix file.
      ...
      real     fixlon(maxstorm,maxtime),fixlat(maxstorm,maxtime)
      real     gridpoint_maxmin,xmeanzeta,grdspc,re,ri,parmlon,parmlat
...
c     These (i,j) coordinates are returned as ilonfix and jlatfix.
     ...
     &     ,grdspc,glatmax,glatmin,glonmax,glonmin
 ...
 621    format (1x,'+++ RPT_MEAN_ZETA: n= ',i2,' lev= ',i4
     &         ,' xmeanzeta= ',f9.7,' imeanzeta= ',i4,' iuret= ',i3)
        ...
        call fix_latlon_to_ij (imax,jmax,grdspc
 ...
 623    format (1x,'+++ RPT_GRID_ZETA: n= ',i2,' lev= ',i4
     &         ,' grid zeta= ',f9.7,' igrid zeta= ',i4,' ifilret= ',i3)
...
c         return to calling subroutine without trying to get max/min
     ...
     &        rlont >= (glonmax + grdspc) .or. rlont < glonmin) then
            print *,' '
...
c
      ...
      print *,' +++ grdspc= ',grdspc,' nhalf= ',nhalf
      print *,' +++ npts= ',npts
...
c
      ...
      print *,' +++ ibmaxlonpts= ',ibmaxlonpts,' grdspc= ',grdspc
...
c
      parameter (jf=1000*750,lugb=11,lugi=31) 
      real      f(jf)
...
c     search for and come up empty for.  For GFDL, we have 35m winds.
      ...
      if (inp%model == 1 .or. inp%model == 2 .or. inp%model == 16 .or.
     &    inp%model == 5 .or. inp%model == 6 .or. inp%model == 8 .or.
     &    inp%model == 10 .or. inp%model == 13 )
     &then
        iglevtyp(12) = 105
        iglevtyp(13) = 105
        iglev(12)    = 10
        iglev(13)    = 10
      else if (inp%model == 3) then    
        ! UKMET: "surface" winds  
        iglevtyp(12) = 1
        iglevtyp(13) = 1
        iglev(12)    = 0
        ...
        iglev(12)    = 10000
        iglev(13)    = 10000
      else if (inp%model == 21) then   ! ECMWF Ensemble - pmsl level ID
        iglevtyp(9)  = 1
      endif
...
c
      ...
      parameter (lugb=11,lugi=31,jf=1000*750)
      logical(1), allocatable :: lb(:)
      real      grdspc,xhold,xlondiff,xlatdiff,xgrdspc,ygrdspc
      real, allocatable    :: f(:)
...
c     For tracking Isidoro's stuff, the first files of his that 
c     we're dealing with don't have an x- or y-grid spacing 
c     indicated, so we need to calculate it on the fly here.

      if (grdspc == 0.0) then
        xlondiff = glonmax - glonmin
        xlatdiff = glatmax - glatmin
        xgrdspc = xlondiff / float(imax)
        ygrdspc = xlatdiff / float(jmax)
        grdspc = 0.5 * (xgrdspc + ygrdspc)
      endif
  ...
  83  format (' Max Lat: ',f8.3,'  Max Lon: ',f8.3)
      write (6,85) grdspc
  85  format (' Model grid-spacing (from grib file): ',f8.5)
      print *,' '
...
c     someone running this tracker on the NAM may have forgotten
...
c
      ...
      real      xlondiff,xlatdiff
      integer   z,iscanflag,nlat,nlon
      integer   igetgds(200)
...
c     indicated, so we need to calculate it on the fly here.
      ...
      if (dlon == 0.0 .or. dlat == 0.0) then
        xlondiff = abs(float(igetgds(8))/1000. 
     &               - float(igetgds(5))/1000.)
        xlatdiff = abs(float(igetgds(7))/1000. 
     &               - float(igetgds(4))/1000.)
        nlon = igetgds(2)
        nlat = igetgds(3)
        dlon = xlondiff / float(nlon)
        dlat = xlatdiff / float(nlat)
      endif
...
c     &      trkrinfo%westbd <   0.0 .or.
     ...
     &          trkrinfo%eastbd >= 0.0)then
              ...
              jbeg = int(((glatmax + grdspc - trkrinfo%northbd) 
     &               / grdspc) + 0.5)
              jend = int(((glatmax + grdspc - trkrinfo%southbd) 
     &               / grdspc) + 0.5)
              ibeg = int(((trkrinfo%westbd - glonmin + grdspc)  
     &               / grdspc) + 0.5)
c              iend = int(((trkrinfo%eastbd - glonmin + grdspc)  
c     &               / grdspc) + 0.5)
              iend = int(((trkrinfo%eastbd - glonmin + grdspc)  
     &               / grdspc) + 0.5) + imax
 ...
 377      continue
          ...
          jbeg = int(((glatmax + grdspc - trkrinfo%northbd) / grdspc)
     &              + 0.5)
          jend = int(((glatmax + grdspc - trkrinfo%southbd) / grdspc)
     &              + 0.5)
          ibeg = int(((trkrinfo%westbd - glonmin + grdspc)  / grdspc)
     &              + 0.5)
          iend = int(((trkrinfo%eastbd - glonmin + grdspc)  / grdspc)
     &              + 0.5)
        endif
      endif
