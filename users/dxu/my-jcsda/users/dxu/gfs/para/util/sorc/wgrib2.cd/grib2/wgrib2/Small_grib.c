...
int f_ijsmall_grib(ARG3) {
	...
	if (sscanf(arg2,"%d:%d", &(save->iy0), &(save->iy1)) != 2) 
        ...
        if ((save->out = (void *) ffopen(arg3, "wb")) == NULL) 
...
int small_grib(unsigned char **sec, int mode, float *data, double *lon, double *lat, unsigned int ndata,
    ...
    if (can_subset) {
        ...
        if ((grid_template == 0 && sec3_len == 72)) {
	...
	new_data = malloc(new_ndata * sizeof(float));
	k = 0;
	for(j = iy0; j <= iy1; j++) {
	    for(i = ix0; i <= ix1; i++) {
	  	new_data[k++] = data[ idx(i,j,nx,ny,cyclic_grid) ];
	    }
	}
    }
    else {
...
/*
    if (grib_type == simple) grib_out(new_sec, new_data, new_ndata, out);
    else if (grib_type == jpeg) jpeg_grib_out(new_sec, new_data, new_ndata, 
	new_nx, new_ny, use_scale, dec_scale, bin_scale, out);
    else if (grib_type == ieee) ieee_grib_out(new_sec, new_data, new_ndata, out);
    else if (grib_type == complex1) complex_grib_out(new_sec, new_data, new_ndata,
             use_scale, dec_scale, bin_scale, wanted_bits, max_bits, 1, out);
    else if (grib_type == complex2) complex_grib_out(new_sec, new_data, new_ndata,
             use_scale, dec_scale, bin_scale, wanted_bits, max_bits, 2, out);
    else if (grib_type == complex3) complex_grib_out(new_sec, new_data, new_ndata,
             use_scale, dec_scale, bin_scale, wanted_bits, max_bits, 3, out);
    else fatal_error("small_grib: unsupported output grib type","");
*/
