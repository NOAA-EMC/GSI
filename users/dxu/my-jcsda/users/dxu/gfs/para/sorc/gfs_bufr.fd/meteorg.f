...
!
!   2014-03-27  DANA CARLIS   CORRECT VIRTUAL TEMPERATURE CALCULATION
...
!$$$
      ...
      real,dimension(npoint,levso) :: p1,p2,p3,pd1,pd2,pd3,tv,tvnew
...
!  obtain new interface-layer pressure for new levso
     ...
     &      p2,gridu, gridv, grids (1,3),grids (1,3+levso))
          ...
          do k = 1, levs
            ...
            enddo
          enddo
! Put topo(1),surf press(2),temp(3:66),and specifi hum(67:130) in grids
! for each station
          do k = 1, 2*levs+2
            do np = 1, npoint
...
!
        do k = 1, levso
          do np = 1, npoint
            pp  = p2(np,k)            
            ppn = p3(np,k)              
            tvnew(np,k) = tv(np,k) * (ppn/pp)**(con_rocp)
            call svp(qsn,esn,ppn,tvnew(np,k))
            call svp(qs,es,pp,tv(np,k))
...
!
     ...
     &      p3(np,1:levso),tvnew(np,1:levso),qnew(np,1:levso),
...
!       Use GFS routine calpreciptype.f to calculate precip type
     ...
     &      DOMR,DOMZR,DOMIP,DOMS)  ! Output vars
          ...
          data(nflx + 23) = DOMZR
          data(nflx + 24) = DOMR
          if(np.eq.1.or.np.eq.10.or.np.eq.100.or.np.eq.npoint) then
