...
!           wrtsfc predating the i/o task updates.
            ...
            call wrtflx_a( ioproc,noflx,zhour,fhour,idate,colat1,
...
!     ta = rtc()
      ...
      if (me == ioproc) then
        ...
        if (.not. gfsio_out) then           ! write using sigio
          ...
          call spect_write(nosig,ioproc,fhour,jdate,sl,si,pdryini,
     &                     ls_nodes,max_ls_nodes,global_lats_r,
...
!     t4 = rtc()
      if (.not. adiab) then
        if (liope) then                     !  write the fluxes
          if (me == ioproc) then
            call wrtflx_w( ioproc,noflx,zhour,fhour,idate,colat1,
     &                     secswr,seclwr,sfc_fld%slmsk,
     &                     global_lats_r,lonsperlar )
          endif
        endif
      endif
...
!  ---  set defalt decimal scaling factor array
      ...
      ids_uvb     = 0      ! used for iuvbf/iuvbfc due to conflict with ievcw
      call idsdef(1,ids)
