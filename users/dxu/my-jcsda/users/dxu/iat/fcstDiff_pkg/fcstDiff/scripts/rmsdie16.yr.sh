...
#  for standard deviation /error growth
#  same as ../../monthly/WTS16/rmsdie16.sh with slight changes
#------------------------------------------------------
...
# ----               -do 1000,500mb rmsz, mean z error
   for level in 500 1000
do
     for field in rmszn rmszs rmszcn rmszcs mzen mzes  
do
...
#  do rmszdie first
# ----------------------------------------
 rm changefield1 changefield2 out.gr1
...
# ----------------------------------------
 rm changefield3 changefield4 out.gr2
...
# now do msedie16.gs...
# ----------------------------------------
 rm changefield5 changefield6 out.gr3
...
#  do rmszcdie16.gs 
# ----------------------------------------
 rm changefield7 changefield8 out.gr4
 ...
 sed -f changefield8 okzero >ok8
  $gdir/grads -lbc "run  $sorcdir/rmszcdie16.gs \
   $sdate $edate $mdl1 $mdl2 $mdl3 $mdl4 $mdl5 $mdl6"
# ---------------------------------------
#  do meanzdie16.gs for 500 and 1000mb NH(ok9,10) SH(ok11,12) 
# ----------------------------------------
 rm changefield9 changefield10 out.gr10
 rm changefield11 changefield12 
 echo "s/mod_field/mzen.500/g" >>changefield9
 echo "s/mod_field/mzen.1000/g" >>changefield10
 echo "s/mod_field/mzes.500/g" >>changefield11
 echo "s/mod_field/mzes.1000/g" >>changefield12
 sed -f changefield9 okzero >ok9
 sed -f changefield10 okzero >ok10
 sed -f changefield11 okzero >ok11
 sed -f changefield12 okzero >ok12
  $gdir/grads -lbc "run  $sorcdir/meanzdie16.gs \
   $sdate $edate $mdl1 $mdl2 $mdl3 $mdl4 $mdl5 $mdl6"
# ---------------------------------------
...
cat << EOF >ftpin1
  ...
  put meanzdie.gif meanzdie.$yyyy.gif
