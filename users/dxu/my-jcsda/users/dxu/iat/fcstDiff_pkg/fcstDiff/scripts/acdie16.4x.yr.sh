...
#------------------------------------------------------
#  6-model version, order is s x e k n m; 
#  if a model is to be omitted, "no" is used for model name
#  anomalous correlation die-off curves; 4 times daily cycles
# same as  ../../monthly/WTS16/acdie16.4x.sh with slight changes 
#------------------------------------------------------

if [ $# -lt 9 ]; then
    echo " Usage: pass in beginning, ending dates,no. of days; model names, rundir "
    exit 9
fi
...
#-------------------------------------
 ...
 cp   $sorcdir/acdie16.f .
 xlf  $sorcdir/acdie16.f -o $tmpdir1/acdie16.x
...
do
   for field in acv
do
     rm $tmpdir1/${field}$level.$edate.sum 2>/dev/null
     rm stdin 2>/dev/null
     echo "$kbad1 $jbad1 $kbad2 $jbad2 $msel1 $msel2" >stdin
     echo "$nmdls $killt1" >>stdin
     cat stdin
...
cp $sorcdir/die5n1000.ctl ok5
cp $sorcdir/die5s1000.ctl ok6
cp $sorcdir/die5n500.ctl ok7
cp $sorcdir/die5s500.ctl ok8
cp $sorcdir/dieacv850.ctl ok9
cp $sorcdir/dieacv200.ctl ok10
#
cp $sorcdir/scat2pan16.gs .
...
# ----------------------------------------
 ...
 $gdir/grads -lbc "run  acdie2hem16.gs \
   $sdate $edate $mdl1 $mdl2 $mdl3 $mdl4 $mdl5 $mdl6"
# ---------------------------------------
#  do acv                
# ----------------------------------------
 rm out.gr9 out.ps9 out.gr10 out.ps10 2>/dev/null
 $gdir/grads -lbc "run  acvdie16.gs \
  $sdate $edate $mdl1 $mdl2 $mdl3 $mdl4 $mdl5 $mdl6"
# ---------------------------------------
# now do  scat2pan.gs (set for modl1 vs modl6 - 00 vs 12)
# ----------------------------------------
 rm out.gr23 out.ps23 out.gr24 out.ps24 2>/dev/null
$gdir/grads -lbc "run scat2pan16.gs \
 1 $ndays $sdate $edate $mdl1 $mdl2"
# ---------------------------------------
# now do ssplot.gs  - substituted for  aczd1216.gs (need "portrait")
#   can do ss4.gs to compare 4 models
# ----------------------------------------
#rm out.gr5 out.ps5 out.gr6 out.ps6
###$gdir/xgrads -pbc "run ssplot.gs 1 $ndays $mdl1 $mdl6"
$gdir/grads -lbc "run ss4.gs 1 $ndays $mdl1 $mdl2 $mdl6 $mdl5"
#
echo "finished grads"
