...
export fcst_day=${fcst_day:-1}                ;#forecast day-length to verify
...
for var in  \
 ...
 CFRZRsfc CICEPsfc CRAINsfc CSNOWsfc UFLXsfc VFLXsfc 
do
...
if [ $var = "CPRATEsfc" ]; then varname="Conv Precip Rate [mm/day]"     scal=24*3600 ; fi
...
if [ $var = "albedo" ];   then varname="Sfc Albedo"       scal=1 ; fi

if [ $var = "CFRZRsfc" ]; then varname="Cat. Frzn Rain"     scal=1 ; fi
if [ $var = "CICEPsfc" ]; then varname="Cat. Ice Pellets"       scal=1 ; fi
if [ $var = "CRAINsfc" ]; then varname="Cat. Rain"              scal=1 ; fi
if [ $var = "CSNOWsfc" ]; then varname="Cat. Snow"              scal=1 ; fi
if [ $var = "UFLXsfc" ]; then varname="Sfc U Mom flx [N/m2]"      scal=1 ; fi
if [ $var = "VFLXsfc" ]; then varname="Sfc V Mom flx [N/m2]"      scal=1 ; fi
...
*   'set mproj latlon'
    ...
    if(i=1); 'set rbcols 31    33   35    37    39    43    45   47     49    21     23    25   27  ';endif
      if( $var = "UGWDsfc" | $var = "VGWDsfc" | $var = "CFRZRsfc" | $var = "CICEPsfc" | $var = "CRAINsfc" | $var = "CSNOWsfc")
       'set clevs  -1 -0.5 -0.1 -0.05 -0.01 -0.001 0.001 0.01 0.05 0.1 0.5 1 '
       'set rbcols 49    46    42   39    36     32    0     22    26    29   73     76   79'
      endif
    'd sn'i 
*
...
for var in  \
 ...
 CWORKclm GUSTsfc LFTXsfc TMPsig995 UGRDsig995 VGRDsig995
do

if [ $var = "CWORKclm" ];    then varname="Cld work Func [J/kg]"        scal=1 ; fi
if [ $var = "GUSTsfc" ];     then varname="Sfc wind gust [m/s]"        scal=1 ; fi
if [ $var = "LFTXsfc" ];     then varname="Sfc lifted index [K]"        scal=1 ; fi
if [ $var = "TMPsig995" ];   then varname="s=.995  Temp. [K]"        scal=1 ; fi
if [ $var = "UGRDsig995" ];  then varname="s=.995 U Wind [m/s]"        scal=1 ; fi
if [ $var = "VGRDsig995" ];  then varname="s=.995 V Wind [m/s]"        scal=1 ; fi
...
*   'set mproj latlon'
    ...
    if( $var = "WATRsfc" ); 'set clevs  -20 -15 -10 -5 -2 -0.1 0.1 2 5 10 15 20 ';endif
    if( $var = "WEASDsfc" ); 'set clevs  -8  -6   -4 -2 -1 -0.1 0.1 1 2 4  6  8  ';endif
    if( $SOILTAG = "SOIL" ); 'set clevs  -20 -15 -10 -5 -3 -1 1 3 5 10 15 20 ';endif
    if( $var = "HGTsfc" ); 'set clevs  -400 -200 -100 -50 -20 -10 10 20 50 100 200 400';endif
    if( $var = "PWATclm" | $var = "CWATclm" ); 'set clevs  -20 -15 -10 -5 -2 -1 1 2 5 10 15 20';endif
    ...
    if(i=1)
      ...
      if( $var = "WEASDsfc" | $var = "WATRsfc" )
       'set clevs   0.1 1  2  4  6  8 10 15 20 40 60 80  100'
       'set rbcols 0  33 35 37 39 43 45 47 49 21 23 25 27  29'
      endif
     endif
    'd sn'i 
*
