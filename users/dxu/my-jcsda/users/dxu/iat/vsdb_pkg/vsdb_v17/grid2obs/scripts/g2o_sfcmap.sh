...
*  say 'cmin cmax cint 'cmin' 'cmax' 'cint
   ...
   while (i <= $nmd)
    ...
    ln=sublin(result,7); wd=subwrd(ln,8); count=substr(wd,1,3)
    if ( count>0 )
      if( i <=5); 
...
*------------------------
  ...
  xwd=7.0; ywd=4.0; yy=ywd/16
  xmin=1.0; xmax=xmin+xwd; ymin=6.0; ymax=ymin+ywd
  xt=xmin+0.3; xt1=xt+0.5; xt2=xt1+0.1; yt=ymin+0.30*ywd ;*for legend  
  zxt=xt2+3.0; zxt1=zxt+0.5; zxt2=zxt1+0.1; zyt=ymin+0.36*ywd
  titlx=xmin+0.5*xwd;  titly=ymax+0.20                  ;*for tytle
  xlabx=xmin+0.45*xwd;  xlaby=ymin-0.60
  'set parea 'xmin' 'xmax' 'ymin' 'ymax
...
*--find maximum and minmum values
   ...
   i=i+1
   endwhile
   dist=cmax-cmin; cmin=cmin-0.2*dist
...
*first plot observations (assume obs are the same for all models/experiments)
   i=1
    'set gxout stat'  ;*compute means and count good data numbers
    'd obs(x='%i')'
    ln=sublin(result,11); wd=subwrd(ln,2); omean=substr(wd,1,5)
    ln=sublin(result,7); wd=subwrd(ln,8); ocount=substr(wd,1,3)
    if ( ocount>0 )
      'set strsiz 0.13 0.13'; yt=yt
      'set line 'ccoo' 'csto' 11'; 'draw line 'xt' 'yt' 'xt1' 'yt
      'set string 'ccoo' bl 6';     'draw string 'xt2' 'yt' obs  'omean'  'ocount
