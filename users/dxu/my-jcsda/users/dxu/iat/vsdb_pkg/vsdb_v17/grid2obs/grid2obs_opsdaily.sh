...
sh +x /global/save/Fanglin.Yang/VRFY/vsdb/grid2obs/prhw14.sh

#--for running daily operational models
...
export vsdbsave=/global/save/Fanglin.Yang/vrfygfs/vsdb_data  ;#operational model grid-to-obs data base
...
export rundir=/stmpd2/$LOGNAME/g2o_ops                         ;#running directory
...
export NWPROD=$vsdbhome/nwprod                              ;#utilities in nwprod
export ACCOUNT=gm                                           ;#computer ACCOUNT task
...
export memory=2048; export share=N
if [ $CUE2RUN = dev_shared ]; then export memory=1024; export share=S; fi
mkdir -p $rundir


CDATE=${1:-$(date +%Y%m%d)}
CDATM1=`/nwprod/util/exec/ndate -24 ${CDATE}00 |cut -c 1-8 `
CDATM2=`/nwprod/util/exec/ndate -48 ${CDATE}00 |cut -c 1-8 `
chost=`echo $(hostname)|cut -c 1-1`

#---prepare prepbufr data
rm $rundir/g2o*.out
$SUBJOB -a $ACCOUNT  -q transfer -g $GROUP -p 1/1/S -r 512/1 -t 3:00:00 -j getbufr \
...
myarch="/global/noscrub/emc.glopara/global"
...
export batch=YES
listvar1=vsdbhome,vsdbsave,cyclist,expnlist,expdlist,dumplist,fhoutair,fhoutsfc,vsdbsfc,vsdbair,gdtype,vlength
listvar2=NWPROD,SUBJOB,ACCOUNT,CUE2RUN,GROUP,DATEST,DATEND,rundir,HPSSTAR,gdas_prepbufr_arch,batch
...
#===========================
dd=`echo $(date +%Y%m%d) |cut -c 8-8`
if [ $dd -eq 1 -o $dd -eq 6 ]; then
...
#--ops ensemble means
sleep 7200
export rundir=/stmpd2/$LOGNAME/g2o_ops/ensm                     ;#running directory
ndays=92
nhours=`expr $ndays \* 24 `
export DATEND=`/nwprod/util/exec/ndate -48 $(date +%Y%m%d)00 |cut -c 1-8`     ;#forecast ending date
export DATEST=`/nwprod/util/exec/ndate -$nhours ${DATEND}00 |cut -c 1-8 `     ;#forecast starting date
