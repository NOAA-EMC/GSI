...
fmType=3	#POES:0(low),1(high); DMSP:0(UAS-lowest),1(LAS),2(ENV),3(IMG-highest); TRMM:-1(coarse),0(low),1(high)
...
minLat=5	#Min latitude of data to be processed (only if GeogrLimits=1)
maxLat=35	#Max latitude of data to be processed (only if GeogrLimits=1)
minLon=110	#Min longitude of data to be processed (only if GeogrLimits=1)
maxLon=130	#Max longitude of data to be processed (only if GeogrLimits=1)
...
fwdCloudOffOrOn=1	#Set Hydrometeors to Zero or retain values (FWD step only).  0->Set to Zero, 1->Retain
biasComputeMethod=0	#Method for computing the bias. 0->Simple bias, 1->Histogram adjustment
