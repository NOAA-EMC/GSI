...
nargs=$#
sdate=`date`
echo "NWP start = $sdate"

cd /data/data007/pub/mirs_oper/scripts/
...
. /data/data007/pub/mirs_oper/scripts/script_functions.bash
. /data/data007/pub/mirs_oper/setup/metopB_pcf_daily.bash
...
nedtExt=${fileExt}
if [[ ${satId} == "f16"   || ${satId} == "aqua" || ${satId} == "f18" || \
      ${satId} == "fy3ri" || ${satId} == "trmm" || ${satId} == "gpm" || \
      ${satId} == "mtma"  || ${satId} == "mtsa" ]] ; then
...
if [[ ${satId} == "f16" || ${satId} == "f18"  || ${satId} == "trmm" || \
      ${satId} == "gpm" || ${satId} == "mtma" || ${satId} == "mtsa" ]] ; then
...
if [[ "${step_nwp}" -eq 1 ]] ; then
  ...
  if [[ ${checkGdas} == "1" ]] ; then
    ...
    ${spcCoeffFile} ${tauCoeffFile} ${cldOptPropFile} ${gdasData}
  else
    echo "NWP(Gdas) files needed are NOT complete"
  fi
fi
...
#--------------------------------------------------------------------------------
if [[ "${step_nwp}" -eq 1 ]] ; then
  checkEcmwf=`prepNWP ${fmsdrPath} ${rdirAnalysExt} "${extResol}" ${satId} ${ecmwfData}\
  ${nwpGenAnalysSrc} ${makeOrNot} ${binPath} ${controlDataPath} ${nwpEcmwfGridPath}\
  ${gridSfcNwpAnalysList} ${gridAtmNwpAnalysList}`
  if [[ ${checkEcmwf} == "1" ]] ; then
    echo "NWP(Ecmwf) files needed are complete"
    ...
    echo "NWP(Ecmwf) files needed are NOT complete"
  fi
fi
...
#--------------------------------------------------------------------------------
if [[ "${step_nwp}" -eq 1 ]] ; then
  checkGfs=`prepNWP ${fmsdrPath} ${rdirAnalysExt} "${extResol}" ${satId} ${gfsData}\
  ${nwpGenAnalysSrc} ${makeOrNot} ${binPath} ${controlDataPath} ${nwpGfsGridPath}\
  ${gridSfcNwpAnalysList} ${gridAtmNwpAnalysList}`
  if [[ ${checkGfs} == "1" ]] ; then
    echo "NWP(Gfs) files needed are complete"
    nwp ${nwpGfsGridPath} ${rdirAnalysExt} ${gridSfcNwpAnalysList} ${gridAtmNwpAnalysList}\
    ${fmsdrPath} "${extResol}" ${fmsdr4NwpList} ${nwpAnalysDir} ${topographyFile}\
    ${covBkgAtm1File} ${nOrbits2Process} ${logFile} ${nProfs2Fwd} ${grid2nwpControlFile}\
    ${nwpGenAnalysSrc} ${makeOrNot} ${binPath} ${sensorId} ${rdirNextAnalysExt}\
    ${spcCoeffFile} ${tauCoeffFile} ${cldOptPropFile} ${gfsData}
  else
    echo "NWP(Gfs) files needed are NOT complete"
  fi
fi
...
#--------------------------------------------------------------------------------
if [[ "${step_fwd}" -eq 1 && ${checkGdas} == "1" ]] ; then
  ...
  ${monitorFwd} ${logFile} ${fwdControlFile} ${binPath} ${satId} ${gdasData} ${fwdCloudOffOrOn} "${rdirExt}"
fi

#--------------------------------------------------------------------------------
#	step: Application of forward operator on NWP ECMWF analyses
#--------------------------------------------------------------------------------
if [[ "${step_fwd}" -eq 1 && ${checkEcmwf} == "1" ]] ; then
  fwd ${fwdSrc} ${makeOrNot} ${nwpAnalysDir} ${nwpAnalysList} ${nOrbits2Process}\
  ${fwdAnalysDir} ${spcCoeffFile} ${tauCoeffFile} ${cldOptPropFile}\
  ${instrumentSensor1Sensor2File} ${nProfs2Fwd} ${addDeviceNoise} ${nedtAftFMFile}\
...
#--------------------------------------------------------------------------------
if [[ "${step_fwd}" -eq 1 && ${checkGfs} == "1" ]] ; then
  fwd ${fwdSrc} ${makeOrNot} ${nwpAnalysDir} ${nwpAnalysList} ${nOrbits2Process}\
  ${fwdAnalysDir} ${spcCoeffFile} ${tauCoeffFile} ${cldOptPropFile}\
  ${instrumentSensor1Sensor2File} ${nProfs2Fwd} ${addDeviceNoise} ${nedtAftFMFile}\
  ${monitorFwd} ${logFile} ${fwdControlFile} ${binPath} ${satId} ${gfsData} ${fwdCloudOffOrOn} "${rdirExt}"
fi

#--------------------------------------------------------------------------------
#      step: Determination of the bias (by comparing FWD simul and meas)
#--------------------------------------------------------------------------------
if [[ "${step_biasGen}" -eq 1 && ${checkGdas} == "1" ]] ; then
  ...
  ${topographyFile} ${binPath} "${modelErrNominalFile}" ${controlDataPath} ${gdasData} \
...
#      step: Determination of the bias (by comparing FWD simul and meas)
#--------------------------------------------------------------------------------
if [[ "${step_biasGen}" -eq 1 && ${checkEcmwf} == "1" ]] ; then
  biasGen ${determineBiasSrc} ${nwpAnalysDir} "${extResol}" ${nwpAnalys4BiasList}\
  ${fwdAnalysDir} ${fwdAnalys4BiasList} ${fmsdr4BiasDir} ${fmsdr4BiasList}\
  ${biasCompuControlFile} ${biasComputeMethod} ${biasFile} ${modelErrFile}\
  ${figs4BiasDir} ${nOrbits2Process} ${satId} ${IDL} ${sensorId}\
  ${topographyFile} ${binPath} "${modelErrNominalFile}" ${controlDataPath} ${ecmwfData}\
  ${minLat} ${maxLat} ${minLon} ${maxLon}
fi
...
#      step: Determination of the bias (by comparing FWD simul and meas)
#--------------------------------------------------------------------------------
if [[ "${step_biasGen}" -eq 1 && ${checkGfs} == "1" ]] ; then
  biasGen ${determineBiasSrc} ${nwpAnalysDir} "${extResol}" ${nwpAnalys4BiasList}\
  ${fwdAnalysDir} ${fwdAnalys4BiasList} ${fmsdr4BiasDir} ${fmsdr4BiasList}\
  ${biasCompuControlFile} ${biasComputeMethod} ${biasFile} ${modelErrFile}\
  ${figs4BiasDir} ${nOrbits2Process} ${satId} ${IDL} ${sensorId}\
  ${topographyFile} ${binPath} "${modelErrNominalFile}" ${controlDataPath} ${gfsData} \
  ${minLat} ${maxLat} ${minLon} ${maxLon}
fi
...
#--------------------------------------------------------------------------------
if [[ "${step_biasFigsGen}" -eq 1 && ${checkGdas} == "1"  ]] ; then
...
#--------------------------------------------------------------------------------
if [[ "${step_biasFigsGen}" -eq 1 && ${checkEcmwf} == "1" ]] ; then
  biasFigsGen_parallel ${edr4BiasDir} ${nwpAnalysDir} ${fwdAnalysDir} ${edr4BiasList}\
  ${nwpAnalys4BiasList} ${fwdAnalys4BiasList} ${processMode} ${orbitInfo}\
  "${extResol}" ${satId} ${rdirAnalysExt} ${gridSrc} ${grid4BiasDir} ${makeOrNot}\
  ${binPath} ${nwpGridControlFile} ${fwdGridControlFile} ${biasGridControlFile}\
...
#--------------------------------------------------------------------------------
if [[ "${step_biasFigsGen}" -eq 1 && ${checkGfs} == "1" ]] ; then
  biasFigsGen_parallel ${edr4BiasDir} ${nwpAnalysDir} ${fwdAnalysDir} ${edr4BiasList}\
  ${nwpAnalys4BiasList} ${fwdAnalys4BiasList} ${processMode} ${orbitInfo}\
  "${extResol}" ${satId} ${rdirAnalysExt} ${gridSrc} ${grid4BiasDir} ${makeOrNot}\
  ${binPath} ${nwpGridControlFile} ${fwdGridControlFile} ${biasGridControlFile}\
...
dir_www=/net/www/www/smcd/mirs/img_hr/${satId}/${rdirAnalysExt}
mkdir -p ${dir_www}
find ${figs4BiasDir} -name "*_gdas_*.png"  -exec cp -f {} ${dir_www} \;
find ${figs4BiasDir} -name "*_gfs_*.png"   -exec cp -f {} ${dir_www} \;
find ${figs4BiasDir} -name "*_ecmw*.png"   -exec cp -f {} ${dir_www} \;
find ${figs4BiasDir} -name "*_mspps_*.png" -exec cp -f {} ${dir_www} \;


figs4BiasDir=`dirname ${figs4BiasDir}`
biasMonitor ${biasPath} ${figs4BiasDir} ${satId} ${determineBiasSrc} \
	    ${IDL} ${biasVerifControlFile} ${controlDataPath} ${gdasData} \
            ${version} ${fmType}
...
cp ${perfsMonitorPath}/*gdas*.png /net/www/www/smcd/mirs/images/${satId}/bias
#cp ${perfsMonitorPath}/*gfs*.png  /net/www/www/smcd/mirs/images/${satId}/bias
cp ${perfsMonitorPath}/*ecmw*.png /net/www/www/smcd/mirs/images/${satId}/bias


#---archive gridded data to free up space
cd ${grid4BiasDir}
dir_archive=/net/orbit272l/disk1/pub/archive/grid/${satId}_amsua_mhs/${rdirAnalysExt}
mkdir -p ${dir_archive}
cp ${grid4BiasDir}/GRID*gdas* ${dir_archive}
cp ${grid4BiasDir}/GRID*gfs*  ${dir_archive}
cp ${grid4BiasDir}/GRID*ecmw* ${dir_archive}

echo "NWP finish = `date`"
