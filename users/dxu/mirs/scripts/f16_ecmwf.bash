...
. /home/pub/mirs_operational/setup/f16_pcf_daily.bash
...
if [[ "${step_biasGen}" -eq 1 ]] ; then
  ...
  ${topographyFile} ${binPath} "${modelErrNominalFile}" ${controlDataPath} ${refData} ${minLat} ${maxLat} ${minLon} ${maxLon}
fi
...
if [[ "${step_biasFigsGen}" -eq 1 ]] ; then
  ...
  ${version} ${biasPath} ${minLat} ${maxLat} ${minLon} ${maxLon} ${refData}\
  ${fmType} "${nwpGridStr}" "${nwpP2PStr}" "${nedtExt}"
fi


find ${figs4BiasDir} -name "*_ecmwf_*.png" -exec cp -f {} /net/www/www/corp/scsb/mirs/images/${satId}/${rdirAnalysExt} \;

figs4BiasDir=`dirname ${figs4BiasDir}`
biasMonitor ${biasPath} ${figs4BiasDir} ${satId} ${determineBiasSrc}\
	    ${IDL} ${biasVerifControlFile} ${controlDataPath} ${refData} \
            ${version} ${fmType}
...
checkBias ${satId} ${biasPath} ${nedtBefFMFile} ${tune1File} ${rdirAnalysExt} \
	  ${determineBiasSrc} ${IDL} ${controlDataPath} ${refData} "Wanchun.Chen@noaa.gov, Sid.Boukabara@noaa.gov, Kevin.Garrett@noaa.gov"

#---remove gridded data to free up space
cd ${grid4BiasDir}
#mkdir -p /net/orbit082l/disk3/pub/mirs/grid/f16_ssmis/${rdirAnalysExt}
#cp GRID*ecmw* /net/orbit082l/disk3/pub/mirs/grid/f16_ssmis/${rdirAnalysExt}
#chmod 775 -R /net/orbit082l/disk3/pub/mirs/grid/f16_ssmis/${rdirAnalysExt}

mkdir -p /net/orbit272l/disk1/pub/archive/grid/f16_ssmis/${rdirAnalysExt}
cp GRID*ecmw* /net/orbit272l/disk1/pub/archive/grid/f16_ssmis/${rdirAnalysExt}
chmod 775 -R /net/orbit272l/disk1/pub/archive/grid/f16_ssmis/${rdirAnalysExt}
