...
depP2PStr="angle,clw,iwp,lat,lwp,rr,rwp,sfc2,swe,tpw"
...
if [[ ${satId} == "f16" || ${satId} == "f18" || ${satId} == "trmm" || ${satId} == "gpm" || ${satId} == "mtma" || ${satId} == "mtsa" ]] ; then
  ...
  depP2PStr="angle,clw,iwp,lat,lwp,rr,rwp,sfc2,swe,tpw,wspd"
...
www_dir_img=/net/www/www/smcd/mirs/img_hr/${satId}/${rdirExt}
mkdir -p ${www_dir_img}
find ${figsDir} -name "*.png" -exec cp {} ${www_dir_img} \;

# fm check stuff png
cp ${figs4BiasDir}/*${satId}_fm_*png ${www_dir_img}
...
#  "Kevin.Garrett@noaa.gov,Wanchun.Chen@noaa.gov" \
#  "http://www.star.nesdis.noaa.gov/smcd/mirs/dataquality.php" ${controlDataPath} \
#  ${satId} "${rdirExt}" ${binPath}

qcRetrieval ${depDir} ${depList} ${orbitMonPath} ${IDL} ${gridSrc} \
  "${controlDataPath}/qcRetrieval_abnormal_${satId}_${fileExt}" \
  "${controlDataPath}/qcRetrieval_namelist_${satId}_${fileExt}" \
  "Wanchun.Chen@noaa.gov" \
  "http://www.star.nesdis.noaa.gov/smcd/mirs/dataquality.php" ${controlDataPath} \
  ${satId} "${rdirExt}" ${binPath}
...
checkNEDT ${satId} ${nedtSensor1Sensor2Path} ${fileExt} ${tune1File} \
       ${nedtMonitorSrc} ${IDL} ${controlDataPath} \
       "Wanchun.Chen@noaa.gov"
#       "Wanchun.Chen@noaa.gov, Kevin.Garrett@noaa.gov, Limin.Zhao@noaa.gov, Jiang.Zhao@noaa.gov, Jiande.Wang@noaa.gov"

#checkQC ${perfsMonitorPath} ${satId} "Kevin.Garrett@noaa.gov, Sid.Boukabara@noaa.gov"
checkQC ${perfsMonitorPath} ${satId} "Wanchun.Chen@noaa.gov"


#copy images to web site
cp ${figsDir}/nedt*png  /net/www/www/smcd/mirs/images/${satId}/nedt/
...
satId2=m1
#copy edr and dep to web site
dir_www=/net/www/www/smcd/mirs/data/${satId2}
rm -f ${dir_www}/*
cp ${edrDir}/*  ${depDir}/* ${dir_www}/
cd ${dir_www}
tar -cf ${satId2}_mirs_edr_dep_${yyyymmdd}.tar EDR* DEP*
gzip ${satId2}_mirs_edr_dep_${yyyymmdd}.tar
mv ${satId2}_mirs_edr_dep_${yyyymmdd}.tar.gz ..

#create dep tar file for ftp site
tar -cf ${satId2}_mirs_dep_${yyyymmdd}.tar  DEP*
gzip ${satId2}_mirs_dep_${yyyymmdd}.tar

#copy dep to ftp site
dir_ftp=/net/www/aftp/pub/corp/scsb/mirs/rain/${rdirExt}
mkdir -p ${dir_ftp}
mv /net/www/www/smcd/mirs/data/${satId2}/${satId2}_mirs_dep_${yyyymmdd}.tar.gz ${dir_ftp}/
chmod -fR 775 ${dir_ftp}/

#copy edr and dep to archive
dir_edr=/net/orbit272l/disk1/pub/archive/edr/${satId}_amsua_mhs/${rdirExt}
dir_dep=/net/orbit272l/disk1/pub/archive/dep/${satId}_amsua_mhs/${rdirExt}
mkdir -p ${dir_edr}
mkdir -p ${dir_dep}
cp ${edrDir}/* ${dir_edr}
cp ${depDir}/* ${dir_dep}
chmod -fR 775  ${dir_edr}
chmod -fR 775  ${dir_edr}

#copy gridded data to archive
dir_grid=/net/orbit272l/disk1/pub/archive/grid/${satId}_amsua_mhs/${rdirExt}
mkdir -p ${dir_grid}
cp ${gridDir}/GRID* ${dir_grid}
#cp ${gridDir}/P2P*  ${dir_grid} 
chmod 775 -R ${dir_grid}
