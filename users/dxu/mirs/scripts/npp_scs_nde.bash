...
if [[ ${satId} == "npp" && ${rdrType} -eq 1 ]] ; then
  ...
  if [[ ${nfile_satms} -ge 1 && ${nfile_gatmo} -eq ${nfile_satms} ]] ; then
    ...
    #---- clean up ( only n1eed remove TDR to avoid conflict )
...
#--------------------------------------------------------------------------------
#      Define and write to PSF
#--------------------------------------------------------------------------------
ProdFilesGen='npp_scs_nde.PSF'
WriteListOfOutputs ${edrDir} ${depDir} ${ncDir} ${fmsdrDir} ${dataPath} ${ProdFilesGen}\
    ${nedtDir} ${edrList}
