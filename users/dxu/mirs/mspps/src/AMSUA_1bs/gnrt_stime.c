...
void gnrt_stime(short int numscan)
{
   ...
   long  int   lyflag, sday, eday, jday, hour, min, sec;
   long  int   itotsec, totsec, stime, stotsec, is_day_cross=0;
   short int   iscan, ifov, mon, domth, syear;
 ...
 *-----------------------------------------------------*/
   sday  = hblock1.start_day_of_year;
   eday  = hblock1.end_day_of_year;
   if(sday != eday) is_day_cross = 1;
   jday = sday;

   stime = hblock1.start_year;

   syear = stime;

   lyflag = leapyear(syear);

   stotsec = hblock1.start_milliseconds_of_day;

   for (iscan = 0; iscan < numscan; iscan++)
   {
      itotsec = scanline1[iscan].time_of_day_of_scan;
      totsec = itotsec / 1000;
      hour  = totsec / 3600;
