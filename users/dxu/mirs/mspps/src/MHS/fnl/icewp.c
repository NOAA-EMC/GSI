...
void icewp(short int iscan)
{
    ...
    float 	b89, b150, b176;
    float       a23,a31,a50,a52,a53,a53L, acoslza, A0,A1,A2;
    float       b180, b182; 
...
// Calculation of a53L is needed to check for rain by the new algorithm 
...
/* The new coastal extension to the existing algorithm starts here 
 ...
 of rain */
         ...
         {
            if ( ((a53L-b176) > -6.0) &&  ((a53L-b180) > 0.0) && ((b89-b150) > 3.0)  )
            { 
               mu = cos(blza * PI / 180.); 
               biwp = 0.01*(292.49289 - 1.83559*b176 + 0.002554378 *b176*b176 + 23.45537*mu);

/* remove confusiopn due to cold snow cover or ice effects              */

               if ( a53L < 244.5 && (a53L- b182) < 15.0) biwp = MISSING;

/* remove confusion of light or warm rain with clouds */
               if( b176 > 260.0 || b180 > 256.0) biwp = 0.0;
