 ...
 * returned values are saved in 
 * calrad[NUMCHAN_B][NUMSPOT_B]
 *------------------------------------*/

   cnt2rd(numchan, numspot);

/*------------------------------------*
 * Convert radiances to antenna 
 * temperatures
 *------------------------------------*/
   for(ichan = 0; ichan < NUMCHAN_B; ichan++)
   {
...
/*------------------------------------*
 * Save calculated AT (rat[NUMSPOT_B]) 
 * into a 3-D array at[]. Bad data is 
 * flagged as -99.
 *------------------------------------*/
     for (ispot=0; ispot< NUMSPOT_B; ispot++)
{
          at[iscan][ispot][ichan] = rat[ispot];
/*printf("Temp: %f\n",at[iscan][ispot][ichan]);*/
}
