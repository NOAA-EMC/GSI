 ...
 *----------------------------------------------------*/
   for(iscan = 0; iscan < numscan; iscan++)
   {
       ...
       new_flag = 0;
       if(scanline1[iscan].tx_newbias_flag!=1 && scanline1[iscan].tx_newbias_flag!=0) 
	 new_flag = -1;

/*----------------------------------------------------*
 * Toss off these scanlines that are bad and flaged as 
 * "do_not_use_scan" or "quality_indicator_bit_field".
 * Also toss off the scanlines that have constant ATs
 * for the whole scanline. 
 *----------------------------------------------------*/
       if(new_flag == 0 && iflag == 0 && scanline5[iscan].stx_3_power == 0)  /* good scanline */
       {
           qc_flag[iscan] = 0;
