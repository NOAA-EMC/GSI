...
!
!M-
! 12/08/06: fix the bug in calculating DTB_DL due to definition of real*4 (not larger enough)
! 03/?/2007 : fix the bug in removing RTM bias
!             add the correction in RTM simulation from two-strem to multiple streams
!
!--------------------------------------------------------------------------------
...
!  print *,'in ..._OPTION ',tpw,iwp,de,retrieval_status
  RETURN
...
!print *,'in loop ich before: icw , de', x(m+2),x(m+3)
   CALL CLOUD_OPTOUT(frequency,tc,x(m+2),x(m+3),tauL,ssalbL,gL)
!   print *,'in loop ich: icw , de', x(m+2),x(m+3)
...
!  !!print *,'tpw,icw,de in loop later:',x(7:9)
...
!data  kv/5.268469e-003,1.911984e-003,3.160225e-003,9.399432e-003,3.015978e-002/
data  kv/5.277882e-03,1.924828e-03,3.200650e-03,9.528114e-03,3.051224e-02,1.524409e-01/

!data  (ko2_coe(1,i),i=1,3)/2.016777e-002,  5.265939e-005, -2.361456e-007/
!data  (ko2_coe(2,i),i=1,3)/3.355675e-002,  8.587707e-005, -3.900665e-007/
!data  (ko2_coe(3,i),i=1,3)/4.174108e-001,  9.458187e-004, -4.380352e-006/
!data  (ko2_coe(4,i),i=1,3)/6.360679e-002,  1.084979e-004, -6.453394e-007/
!data  (ko2_coe(5,i),i=1,3)/2.296447e-002, -6.075383e-006, -1.325813e-007/

!data  (ko2_coe(1,i),i=1,3)/4.088259e-02, -9.147504e-05,  1.406028e-08/
!data  (ko2_coe(2,i),i=1,3)/6.813867e-02, -1.547323e-04,  2.758990e-08/
!data  (ko2_coe(3,i),i=1,3)/8.091954e-01, -1.780038e-03,  3.511531e-07/
!data  (ko2_coe(4,i),i=1,3)/1.312656e-01, -3.630794e-04,  1.747854e-07/
!data  (ko2_coe(5,i),i=1,3)/4.520401e-02, -1.612336e-04,  1.375581e-07/
!data  (ko2_coe(6,i),i=1,3)/2.271734e-02, -8.283299e-05,  7.084674e-08/  !use 183.31-7=176.31
...
! Initializations for various parameters
    ...
    dTB_dD = 0.0
!! print *,' from Amatrix_emissFreq',frequency,theta,em,i0,tc,ts,kv,gL,ssalbL,tauL,             &
!!
!!      dssalbL_dL,dssalbL_dD,dtauL_dL,dtauL_dD,dgL_dL,dgL_dD,       &
!!
!!       g,ssalb,tau,dTB_dV,dTB_dL,dTB_dD,dTB_dem,dTB_dTC,dTB_dTS
...
!  data (coe(1,k),k=1,6)/ -1.99975,  61.3539,  5.10721,  1.21363e-8,  0.00983835 , -1.98478e-5/
!  data (coe(2,k),k=1,6)/ -5.48215,  71.6091,  13.9380,  1.02585e-5, -0.0109019,    0.000189545/
!  data (coe(3,k),k=1,6)/ 14.9776,  92.2082,  44.5338, -1.75598 ,   -0.0768734 ,  -0.000734940/
!  data (coe(4,k),k=1,6)/ -1.88742, -9.45523,  19.7153,  1.68238,     0.00438582,  -0.000108717/
!  data (coe(5,k),k=1,6)/ 2.04982, -62.7181,  23.4092, -0.0566231,   0.0175071 ,  -9.82583e-05/

!  data (coe(1,k),k=1,ncoe)/ -2.05655, 7.86943,   21.1871,  1.91948,    0.00688158, 0.000116748/
!  data (coe(2,k),k=1,ncoe)/ -2.56348, 62.2753,   9.41001, 5.52973e-08,  -0.0218741, 0.000116748/
!  data (coe(3,k),k=1,ncoe)/ 0.389578, 50.3794,   7.70567, 7.13917e-08,   0.0145598, 0.000786876/
!  data (coe(4,k),k=1,ncoe)/ 3.25693,  37.2467,   17.8638, -0.370497, -0.00617641, 7.57529e-05/
!  data (coe(5,k),k=1,ncoe)/ 11.9649, -95.7599,   42.6737, -0.964835,   0.0714960, -0.000777726/
!  data (coe(6,k),k=1,ncoe)/ -1.27812, -62.0895, 15.0481, 0.000256925, 0.000692317, -2.77475e-05/
!  data (coe(6,k),k=1,ncoe)/ -1.75043,   -66.9206, 15.5577, 0.000168080, -0.00612779, 9.66958e-05/
  data (coe(1,k),k=1,ncoe)/ -3.72277, 4.11463, 29.6605, 3.68712, 0.00831045, -0.000840534/
  data (coe(2,k),k=1,ncoe)/ -3.42724, 66.4353, 11.8938, 4.99906e-07, -0.0169137, 0.000161781/
  data (coe(3,k),k=1,ncoe)/ 0.531498, 55.2623, 10.4928, -4.24334e-07, 0.0128095, 0.000840480/
  data (coe(4,k),k=1,ncoe)/ 2.87051, 35.1185, 17.8581, -0.415143, -0.00122094, 0.000214413/
  data (coe(5,k),k=1,ncoe)/ 10.9707, -92.5043, 39.3374, -0.690952, 0.0692855, -0.000704277/
  data (coe(6,k),k=1,ncoe)/ -3.231525, -70.7182, 20.62115, 0.009065965, -0.0093244, 0.000166729/ 
...
! data (coe(1,k),k=1,ncoe)/ -5.907940e+000, 3.311971e-002,-3.692662e-002, 3.397321e-002,-1.226470e-002, 5.405066e-003/
! data (coe(2,k),k=1,ncoe)/ -7.435709e+000, 4.742293e-002,-3.400790e-002, 2.573334e-002,-1.477616e-002, 3.701409e-003/
! data (coe(3,k),k=1,ncoe)/ -4.793554e+001, 2.365846e-001,-2.262365e-001, 2.259357e-001,-3.781223e-002,-1.874668e-002/
! data (coe(4,k),k=1,ncoe)/ -8.637854e+000, 7.447207e-002,-6.105726e-002, 1.888940e-002,-8.674696e-003, 8.431160e-003/
! data (coe(5,k),k=1,ncoe)/ -3.842273e+000, 5.970361e-002,-5.611888e-002, 2.785219e-002,-3.064824e-002, 1.461119e-002/

 data (coe(1,k),k=1,ncoe)/-4.886131e+00,  2.621341e-02, -2.182201e-02,  1.348439e-02, -7.357232e-03, -3.802168e-03,  1.206958e-02/
 data (coe(2,k),k=1,ncoe)/-6.405708e+00,  3.385949e-02, -9.574925e-03,  4.338091e-03, -1.434783e-02, -1.537727e-03,  1.084996e-02/
 data (coe(3,k),k=1,ncoe)/-4.792514e+01,  1.812091e-01, -1.527583e-01,  2.214428e-01, -6.232524e-02,  7.395509e-03, -1.500276e-02/
 data (coe(4,k),k=1,ncoe)/-7.667981e+00,  4.953698e-02, -2.381288e-02, -1.859874e-03, -1.124249e-02,  1.087237e-02,  4.650229e-03/
 data (coe(5,k),k=1,ncoe)/-3.050648e+00,  4.704005e-02, -3.659631e-02,  1.911762e-02, -3.635131e-02,  2.343352e-02, -4.541288e-03/
 data (coe(6,k),k=1,ncoe)/2.927140e+01, -2.717794e-02,  1.777877e-01, -3.237934e-01, -4.218274e-02, -2.726139e-01,  3.677596e-01/
...
! Initialize the ice water path
   ...
   real*4 omega,coef_iwp_a,coef_iwp_b, coef_iwp_c,biwp
   real*4 omegan
