...
!
     ...
     INTEGER :: ssmi_polstatus(7),amsre_polstatus(12)
     DATA ssmis_polstatus/5*4, 2*1, 5*5, 4, 4, 5, 4, 4, 5, 6*1/
     DATA amsu_polstatus/4*2, 2*3, 2, 7*3, 6*2/ 
     DATA ssmi_polstatus/4,5,4,4,5,4,5/
     DATA amsre_polstatus/4,5,4,5,4,5,4,5,4,5,4,5/ 
...
!  AMSU A+B
      ...
      ELSE IF(ChannelInfo%n_Channels == 15 ) THEN
!  AMSU A+B
      RTmodel%PolStatus = amsu_polstatus
      ELSE IF(ChannelInfo%n_Channels == 7 ) THEN
!  SSMI
      RTmodel%PolStatus = ssmi_polstatus
      ELSE IF(ChannelInfo%n_Channels == 12 ) THEN
!  AMSR-E 
      RTmodel%PolStatus = amsre_polstatus
      ELSE
