...
!                       -updated interface for CRTM 2.1.1
!
!===============================================================
...
PROGRAM retr1dvar
  ...
  REAL,                 DIMENSION(:),       ALLOCATABLE :: XbAtm,XbSfc,Xb,XbAtm_ext
  ...
  INTEGER :: nprofiles,nMeasurData,nExternData,nAtm,nSfc,nG,nR,nChan,nch
  INTEGER :: nchanModelErr,nprofsEff=0
  INTEGER :: i,iprof,iAtm,iSfc,stypeSfc,stypeSfcCRTM,AtmClass,SfcClass,iattempt,iAtm_Ext
  INTEGER :: nSfcTypes,nEDRs,nEDRs_atm,nEDRs_sfc,ierr,iTer,ichan,istat
  ...
  LOGICAL :: OkRetrvl,CvgceReached,useBkgExt
  ...
  call OpenLogFile(CntrlConfig%Logfile) 
  call LoadTunParams(CntrlConfig%nAttempts,CntrlConfig%TuningFile(1:CntrlConfig%nAttempts))
  call LoadGlobGeophyCovBkg()            !---Load the global/stratified cov matrix -Atm & Sfc- as well as  U
  call LoadNoise(CntrlConfig%NoiseFile)  !---Load Noise Info and related items (NRF, NEDT, etc)

  useBkgExt = CntrlConfig%useBkgExt .EQ. 1
  if (useBkgExt) call LoadBkgAtm_Ext()                  !---Load external atmospheric backgroud database
  ...
  IF (CntrlConfig%ExternDataAvailab .ne. 0) THEN
      CALL ReadHdrScene(iuExtern,CntrlConfig%ExternDataFile,Scene_Ext,nExternData)
  ENDIF
  call ReadBias(CntrlConfig%BiasFile,nchanBias,nposBias,cfreq_bias,Bias,Slope,Intercept,errReadingBias)
  ...
  CALL GetSensorInfo(CntrlConfig%SensorID,SensorID,nSensors)

  ALLOCATE(ChannelInfo(nSensors))
  Error_Status = CRTM_Init(SensorID,ChannelInfo,File_Path=CntrlConfig%Coeff_Path)
  IF (Error_Status .ne. 0) CALL ErrHandl(ErrorType,Err_CRTMneCNTRL,'CRTM Init Failed')
  nChan=SUM(ChannelInfo%n_Channels)
  if (nChan .ne. Ym%nchan) CALL ErrHandl(ErrorType,Err_InconsNumber,'CRTM ChannelInfo and Obs nChan do not match')
  ...
  !---Initialize Atmos_K and Sfc_K structures for jacobians
  CALL CRTM_Atmosphere_Create(Atmos_K,MAXLAYS,MAXABSORB,MAXCLOUDS,MAXAEROSOL)
  CALL CRTM_Surface_Create(Sfc_K,nchan)

  !---Initialize RTSolution structures
  CALL CRTM_RTSolution_Create(RTSolution,MAXLAYS)
  CALL CRTM_RTSolution_Create(RTSolution_K,MAXLAYS)

  !CALL Init_CRTM(SensorID,Coeff_Path,ChannelInfo,Atmos,Atmos_K,Sfc, &
  ...
  mxIter     = maxval(TunParams(1:CntrlConfig%nattempts)%nIterations)  
  ...
  ALLOCATE(nGsel(CntrlConfig%nattempts))
  ...
  CALL InitHdrScene(nLev,nLay,nChan,GeophStatsT_Sfc(1)%Freq,GeophStatsT_Sfc(1)%polar,&
       ...
       Ym%nPosScan,Ym%nScanLines,CntrlConfig%AlgSN)
  ...
  IF (CntrlConfig%MonitorIterat .eq.1 .or.CntrlConfig%MonitorIterat .eq.2 ) THEN
      call WriteHdrMonitor(CntrlConfig%MonitorFile,iuMonitor,nprofiles, &
  ...
  nG0=maxval(nGsel(1:CntrlConfig%nattempts))
  ALLOCATE(Se(nChan,nChan),Fe(nChan,nChan),SaAtm(nAtm,nAtm),SaSfc(nSfc,nSfc),               &
       SaAtmTot(nAtm,nAtm,nSfcTypes,CntrlConfig%nattempts),                                 &
       SaSfcTot(nSfc,nSfc,nSfcTypes,CntrlConfig%nattempts),                                 &
       XbAtm(nAtm),XbSfc(nSfc),X1stAtm(nAtm),X1stSfc(nSfc),Xb(nG0),Sa(nG0,nG0),             &
       XbAtmTot(nAtm,nSfcTypes,CntrlConfig%nattempts),                                      & 
       XbSfcTot(nSfc,nSfcTypes,CntrlConfig%nattempts),                                      &
       ...
       UAtm(nAtm,nAtm),USfc(nSfc,nSfc),U(nG0,nG0),                                          &
       UAtmTot(nAtm,nAtm,nSfcTypes,CntrlConfig%nattempts),                                  &
       UsfcTot(nAtm,nAtm,nSfcTypes,CntrlConfig%nattempts),                                  &
       ...
       ModelErrTot(nchan,CntrlConfig%nattempts),tb(Ym%nChan),TBqc(Ym%nchan),stat=istat)
  IF (useBkgExt) ALLOCATE(XbAtm_Ext(nAtm))
  IF (istat .ne.0) CALL ErrHandl(ErrorType,Err_AllocMemPb,'(in 1dvar)')
  !------------------------------------------------------------------
  !   Load modeling errors from 'nattempts' files and do checks
  !------------------------------------------------------------------
  DO iattempt=1,CntrlConfig%nattempts
      call ReadNoise(CntrlConfig%FileModelErr(iattempt),ModelErrCFreq,ModelErr,nchanModelErr)
  ...
  iAtm=0
  iSfc=0
  iAtm_Ext=1  ! For external background, only 1 sfc type is available (background is available over all surfaces)
  DO iattempt=1,CntrlConfig%nattempts
     DO i=1,nSfcTypes
        iAtm=i
        iSfc=i
  ...
  ProfLoop: DO iprof=1,nprofiles
      ...
      IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'. Radiance file. (1dvar)')
      !---Read the external data (if applicable)
      UseorNotExterData = CntrlConfig%ExternDataAvailab
      IF (CntrlConfig%ExternDataAvailab .ne. 0) THEN
         call ReadScene(iuExtern,Scene_Ext,ierr)
         IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'(Extern file)')
         IF((ANY(Scene_Ext%qc(1:Scene_Ext%nqc) .ne. 0))) UseorNotExterData = 0
      ...
      call Read_topography(CntrlConfig%Topogr,Ym%lat,Ym%lon,xalt,stypeSfc,xcover,stypeSfcCRTM)
      ...
      call DetermOKsgnl(UseorNotExterData,Ym%tb,Ym%qc,Scene_Ext%qc,   &
           CntrlConfig%GeogrLimits,CntrlConfig%minLat,CntrlConfig%maxLat,   &
           CntrlConfig%minLon,CntrlConfig%maxLon,CntrlConfig%pass,Ym%lat,   &
           Ym%lon,Ym%node,stypeSfc,OkRetrvl)
      !---Initialization 
      call Set2DefaultQC(vec_Int=Scene%qc(1:Scene%nqc),vec_Real=Xout,singl_Int=nIterTot(iprof),singl_Real=Scene%ChiSq)
      ...
      IF (OKRetrvl) THEN
        ...
        IF (useBkgExt) CALL GetBkgAtm_Ext(XbAtm_Ext,Ym%lat,Ym%lon,Ym%julDAY,Ym%secs,iAtm_Ext,1)
        !---Start the iterative process (#of retrieval attempts)
       AttemptsLoop: DO iattempt=1,CntrlConfig%nattempts
           nAttempTot(iprof) = iAttempt
           nIterTot(iprof)   = 0
           !---Pre-classification
           TskPreclass = DEFAULT_VALUE_REAL
           IF (UseorNotExterData .ne. 0 .and. TunParams(iattempt)%EDR_ExtDataUse_sfc(iEDR_sfcT).ne.0) THEN
           ...
           IF (CntrlConfig%PrintMonitor.eq.1) THEN 
              write(*,'(a75)')  '**************************************************************************'
           ...
           !---Use alternate Atm background
           IF (useBkgExt) CALL ReplaceBkgAtm_Ext(XbAtm,XbAtm_Ext)
           ...
           IF (UseorNotExterData .ne. 0 .or. TunParams(iattempt)%EDR_ExtDataUse_sfc(iEDR_sfcp).eq.0) THEN           
          ...
          !---Disable retrieval of layers below surface level
           ...
           call FwdOper(Scene,Y(1:nchan,0),K(1:nchan,1:nGselec),ChannelInfo,    &
                ...
                TunParams(iattempt)%topSensPressWV,CntrlConfig%Logfile)
           ...
           IterLoop: DO iter=0,TunParams(iattempt)%nIterations-1
              ...
              !---Jacobians Monitoring (iteration by iteration)
              IF (CntrlConfig%MonitorIterat .eq.2) THEN
                 call WriteJacobians(iuMonitor,iprof,nEDRselec,nGselec,nIterTot(iprof),nAttempTot(iprof), &
        ...
        IF (CntrlConfig%MonitorIterat .eq.1) THEN
  ...
  call printSummary(nprofiles,nIterTot,Tim,nConvProfs,nprofsEff,  &
       nProfsOverOcean,nProfsOverSeaIce,nProfsOverLand,nProfsOverSnow,CntrlConfig%MeasurmtFile,CntrlConfig%OutputFile)

  !--------------------------------------------------------------------
  ! De-allocation of arrays
  !--------------------------------------------------------------------
  Print *,'Deallocation in progress...'
  DEALLOCATE(Se,Fe,SaAtm,SaSfc,XbAtm,XbSfc,X1stAtm,X1stSfc,Xb,Sa,X1st,Xg,Y,           &
       SaAtmTot,SaSfcTot,XbAtmTot,XbSfcTot,UAtmTot,USfcTot,                           &
       NoiseRMS,K,Xout,nIterTot,nAttempTot,ChiSq,UAtm,USfc,U,ParamIndx,ParamLength,   &
       ParamLabel,ShrkRadVec,SeStar,dY2,FeStar,Ystar,YmStar,Kstar,ChanSel,DX,DXtilda, &
       DXtildaNew,Ktilda,Ustar,Lambda,G,A,S,EDR_nEOF,EDR_cntrlRetr,EDR_ExtDataUse,    &
       iSpaceModeFlag,EmissAnalyt,dnWelling_rad,upWelling_rad,opt_lay,ModelErrCFreq,  &
       ModelErr,ModelErrTot,tb,TBqc)
  IF (useBkgExt) DEALLOCATE(XbAtm_Ext)
  ...
  ENDDO
  IF (useBkgExt) CALL Destroy_BkgAtm_Ext()
  ...
  IF (CntrlConfig%ExternDataAvailab .ne. 0) CALL DestroyScene(Scene_Ext)
