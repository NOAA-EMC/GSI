...
Program rdr2tdr_amsua
  ...
  ChanLoop: DO ichan = 1, nch_a
     ...
     OrbitLoop: DO k=1,nEffFiles
        ...
        ENDIF
        IF (nvalid .lt. 2) THEN
           CALL ErrHandl(WarningType,Warn_NoValidWarmTarget,&
           ' (in AMSUA RDR conversion). No NEDT generated, relaxing QC requirements.')
        ENDIF
     ENDDO OrbitLoop
     !---Test if we have at least one valid orbit to compute the NEDT on, otherwise relax flags and recompute
