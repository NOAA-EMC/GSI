!$Id: rdr2tdr_n18_mhs.f90 1385 2008-07-11 16:13:50Z wchen $
!===============================================================
! Name:       rdr2tdr_mhs
!
!
  ...
  ChanLoop: DO ichan = 1, nch_m
     ...
     IF (nEffFilesValid .lt. 1) THEN
        OrbitLoop2: DO k=1,nEffFiles
           ...
           ENDIF    
        ENDDO OrbitLoop2
        !---Test if we have at least one valid orbit to compute the NEDT on, otherwise write out -9.999 for channel NEDT
        IF (nEffFilesValid .ge. 1) THEN
           NEDT(ichan) =SUM(NEDTperOrbit(ichan,1:nEffFilesValid))/nEffFilesValid
        ENDIF
        IF (nEffFilesValid .lt. 1) THEN
