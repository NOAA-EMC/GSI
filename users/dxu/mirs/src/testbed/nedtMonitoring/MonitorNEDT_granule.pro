;$Id: MonitorNEDT_granule.pro 1637 2008-11-07 20:38:03Z wchen $
@../../../setup/paths_idl.pro
;===============================================================
; Name:		MonitorNEDT_granule
;
;
; Type:		IDL Subroutine
;
;
; Description: To monitor the NEDT daily granules
;
;
; Arguments:
;
...
;
; History:
;  	12/01/2011    Wanchun Chen 	Original Code
;
;===============================================================
PRO MonitorNEDT_granule,namelist=namelist

satId='npp'
ymd='2011-12-03'
nedtList='npp_NEDT_list_2011-12-03'
imgDir='./'
...
for iday=0,nList-1 do begin
    ...
    if (satId eq 'npp') then begin
        yr=float(strmid(strtrim(file_basename(listFiles(iday))),  73,4,/reverse_offset))
        mnth=float(strmid(strtrim(file_basename(listFiles(iday))),69,2,/reverse_offset))
        day=float(strmid(strtrim(file_basename(listFiles(iday))), 67,2,/reverse_offset))
	HH=fix(strmid(strtrim(file_basename(listFiles(iday))),    63,2,/reverse_offset))
	MM=fix(strmid(strtrim(file_basename(listFiles(iday))),    61,2,/reverse_offset))
	; each granule is ~32 seconds, we take middle time, so adding 16 seconds here
	SS=FIX(float(strmid(strtrim(file_basename(listFiles(iday))),  59,3,/reverse_offset)) * 0.1 ) + 16
    endif
    ;Result = JULDAY(Month, Day, Year, Hour, Minute, Second) 
    date(iday)=JULDAY(mnth,day,yr,HH, MM, SS )
...
FOR ISET = 0, 0 DO BEGIN  ; start Loop around time range
...
FOR ichan=0, nchan-1 DO BEGIN
    ...
    for isize=0, 2 do begin
	    ...
	    charsize=0.75
	    png_file = imgDir + prefix + '_ch' + strtrim(string(ichan+1),2) + '_' + yyyymmdd + '_small.png'
	endif
    	if ( isize eq 1 ) then begin
	    xticks = 6
	    device,set_resolution=[650, 500]
	    charsize=1.25
	    png_file = imgDir + prefix + '_ch' + strtrim(string(ichan+1),2) + '_' + yyyymmdd + '.png'
	endif
    	if ( isize eq 2 ) then begin
	    xticks = 18
	    device,set_resolution=[2000, 350]
	    charsize=1.25
	    png_file = imgDir + prefix + '_ch' + strtrim(string(ichan+1),2) + '_' + yyyymmdd + '_big.png'
	endif
	
	TVLCT, 0,0,0,         1 ; black
	TVLCT, 255,255,255, 255 ; white
	tit='MIRS ' + titleSat + ' NEDT Ch' + STRTRIM(ichan+1,2) + ' (' + titles(ichan) + ')' + ' ' + ymd
    	if isize eq 0 then tit = titleSat + ' NEDT Ch' + STRTRIM(ichan+1,2) + ' (' + titles(ichan) + ')' + ' ' + ymd
	...
	;xyouts, 0.02, 0.075, 'MM:SS',COLOR=1, CHARSIZE=charsize, CHARTHICK=1.0, /normal
	;xyouts, 0.02, 0.050, 'HOUR', COLOR=1, CHARSIZE=charsize, CHARTHICK=1.0, /normal
