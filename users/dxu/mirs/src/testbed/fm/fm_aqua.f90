...
!
! History:
!       09-30-2008      daisaku uesawa, JMA @ NOAA/NESDIS/STAR
!       10-27-2008      Wanchun Chen    modified to add lat/lon box option 
!===============================================================
        ...
        IF (ScanLineInROI(iscanLine) .EQ. 1 ) THEN
          ...
          FOVsLoop: Do ifov=1,nFovs
            ...
            CALL WriteMeasurmts(iu_fmsdr,nqc,qc,nchan,angleFM,tb(ifov,:),lat(ifov),lon(ifov),node,&
                 scanUTC/1000.,scanDAY,scanYear,ifov,iProcessScanLine,RelAziAngle(ifov),SolZenAngle(ifov))
          ENDDO FOVsLoop
