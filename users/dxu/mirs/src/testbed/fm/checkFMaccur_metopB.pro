...
PRO checkFMaccur_metopB, QCcheckPath, satId, date, processMode

;----Set input filename
;QCcheckPath='../../../data/TestbedData/PerfsMonitoring/metopB_amsua_mhs/'
;satId='metopB'
;date='2012-11-25'
;processMode=1
...
if ( satId eq 'metopB' ) then satName='MetOp-B'
...
imp:
	...
	cend='as'
        ...
        tit     = satName +' TB Difference @ 89 GHz (AMSUA-MHS) ' + date + ' Asc'
	...
	cend='ds'
        ...
        tit     = satName +' TB Difference @ 89 GHz (AMSUA-MHS) ' + date + ' Des'
        mapPlot,minlat,maxlat,minlon,maxlon,xlat,xlon,ind,tit,minY,maxY,xtab,'[K]',0.6,8,1,0,'(f7.2)'
        map_continents,/continents,/noborder,/hires,fill_continents=0,color=18
	thisImage = TVRD()
	png_file = QCcheckDir+"/"+prefix + '_delta_' + cend + '.png'
	...
	cend='as'
        plot,x,Err(0,*,1),xtitle='MHS Scan Position Index',ytitle='Stdev of DeltaTB [AMSUA-MHS] @ 89 GHz [K]',$
          yrange=[ymin,ymax],ystyle=1,xstyle=1,psym=-8,symsize=0.5,charsize=1.5, title=satName + ' ' + date + ' Asc', /noData
        FOR ifile=0,nList-1 DO BEGIN
            oplot,x,Err_as(ifile,*,1),psym=-8,symsize=0.5, color=(ifile+1)*18
	    xyouts, 0.01, 0.20 + ifile  * 0.05, '_orbit ' + strtrim(string(ifile+1),2), color=(ifile+1)*18, CHARSIZE=1.0, CHARTHICK=0.8, /normal
        ENDFOR
	thisImage = TVRD()
	...
	cend='ds'
        plot,x,Err(0,*,1),xtitle='MHS Scan Position Index',ytitle='Stdev of DeltaTB [AMSUA-MHS] @ 89 GHz [K]',$
          yrange=[ymin,ymax],ystyle=1,xstyle=1,psym=-8,symsize=0.5,charsize=1.5, title=satName + ' ' + date + ' Des', /noData
        FOR ifile=0,nList-1 DO BEGIN
            oplot,x,Err_ds(ifile,*,1),psym=-8,symsize=0.5, color=(ifile+1)*18
	    xyouts, 0.01, 0.20 + ifile  * 0.05, '_orbit ' + strtrim(string(ifile+1),2), color=(ifile+1)*18, CHARSIZE=1.0, CHARTHICK=0.8, /normal
        ENDFOR
	thisImage = TVRD()
	png_file = QCcheckDir+"/"+prefix + '_stdv_' + cend + '.png'
	...
	cend='as'
        plot,x,Err(0,*,0),xtitle='MHS Scan Position Index',ytitle='Mean TB Diffeence [AMSUA-MHS] @ 89 GHz [K]',$
          yrange=[ymin,ymax],ystyle=1,xstyle=1,psym=-8,symsize=0.5,charsize=1.5, title=satName + ' ' + date + ' Asc', /noData
        FOR ifile=0,nList-1 DO BEGIN
            oplot,x,Err_as(ifile,*,0),psym=-8,symsize=0.5, color=(ifile+1)*18
	    xyouts, 0.01, 0.20 + ifile  * 0.05, '_orbit ' + strtrim(string(ifile+1),2), color=(ifile+1)*18, CHARSIZE=1.0, CHARTHICK=0.8, /normal
        ENDFOR
	thisImage = TVRD()
	...
	cend='ds'
        plot,x,Err(0,*,0),xtitle='MHS Scan Position Index',ytitle='Mean TB Diffeence [AMSUA-MHS] @ 89 GHz [K]',$
          yrange=[ymin,ymax],ystyle=1,xstyle=1,psym=-8,symsize=0.5,charsize=1.5, title=satName + ' ' + date + ' Des', /noData
        FOR ifile=0,nList-1 DO BEGIN
            oplot,x,Err_ds(ifile,*,0),psym=-8,symsize=0.5, color=(ifile+1)*18
	    xyouts, 0.01, 0.20 + ifile  * 0.05, '_orbit ' + strtrim(string(ifile+1),2), color=(ifile+1)*18, CHARSIZE=1.0, CHARTHICK=0.8, /normal
        ENDFOR
	thisImage = TVRD()
	png_file = QCcheckDir+"/"+prefix + '_mean_' + cend + '.png'
