...
int f_spectral_bands(ARG0) {
    ...
    int code1, code2, instrument, scale_factor;
    float scaled_val, value;
	...
	for (i = 0; i < nb; i++) {
	    ...
	    scale_factor = int2(sec[4]+19+11*i);
	    scaled_val = ieee2flt(sec[4]+20+11*i);
	    value = scaled2flt(scale_factor, scaled_val);
	    sprintf("%sband %d %d instrument %d central wave no %lf (m-1)",
