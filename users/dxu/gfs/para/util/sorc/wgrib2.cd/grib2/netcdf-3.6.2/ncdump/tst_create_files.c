/* This is part of the netCDF package.  
   ...
   This program creates some test files which ncdump will read.

   $Id: tst_create_files.c,v 1.7 2007/02/16 15:16:22 ed Exp $
*/
...

   {
	 ...
	 favs[fav].v = V_VALUE;
      ...
      if (nc_def_compound(ncid, sizeof(struct wind_vector), COMPOUND_NAME, 
			  &typeid)) ERR;
      if (nc_insert_compound(ncid, typeid, "u", HOFFSET(struct wind_vector, u), 
			     NC_FLOAT)) ERR;
      if (nc_insert_compound(ncid, typeid, "v", HOFFSET(struct wind_vector, v), 
			     NC_FLOAT)) ERR;
