...
C
   ...
   15 FORMAT (' IALL TYPEGRID = ',I1,I4)
      PRINT *,'INPUT AWIPS OR CRAY FILE = ',NFILE
      KKK = KKK + 1
C
...
C
      ...
      PRINT *,'OUTPUT FILE FOR PACKED GRIB MESSAGES = ',OFILE
c
...
C       PRINT *,'NN,I,JSTART,RCBYTE(NN) = ',NN,I,JSTART,RCBYTE(NN)
  101 CONTINUE
C
      PRINT *,'NUMBER OF GRIB RECORDS IN FILE = ',NN
C
C
      NUMREC = NN
C
...
C
   50 CONTINUE
      PRINT *,' '
      IF (NREC.NE.0) THEN
         PRINT *,'WROTE ',NREC,' GRIB RECORDS IN FILE ',OFILE(1:30)
      ENDIF
      PRINT *,' '
c
      CLOSE (UNIT=11)
