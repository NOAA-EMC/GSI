...
#                   defaults to ${FIXGLOBAL}/global_soilmgldas.t${JCAP}.${LONB}.${LATB}.grb
...
export SFCHDR=${SFCHDR:-$NWPROD/global_sfchdr$XC}
...
export FNOROG=${FNOROG:-${FIXGLOBAL}/global_orography.t$JCAP.$LONB.$LATB.grb}
...
export FNMASK=${FNMASK:-${FIXGLOBAL}/global_slmask.t$JCAP.$LONB.$LATB.grb}
...
#---------------
if [ $machine = IBMP6 -a $CYCLEXEC = "/nwprod/exec/global_cycle" ]; then
#---------------
eval $CYCLEXEC <<EOF $REDOUT$PGMOUT $REDERR$PGMERR
 &NAMCYC
  idim=$LONB, jdim=$LATB, lsoil=$LSOIL,
  iy=$iy, im=$im, id=$id, ih=$ih, fh=$FHOUR,
  DELTSFC=$DELTSFC,ialb=$IALB,
 /
 &NAMSFCD
  FNBGSI="$SFCGES",
  FNBGSO="$SFCANL",
