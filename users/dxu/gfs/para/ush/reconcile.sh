...
#if [ $(echo $node | cut -c1-4) = c1ms ] ; then mac=g ; fi
#if [ $(echo $node | cut -c1-2) = c2 ]   ; then mac=g ; fi
#if [ $(echo $node | cut -c1) = f    ]   ; then mac=z ; fi
#if [ $(echo $node | cut -c1) = r    ]   ; then mac=z ; fi
#if [ $mac = c -o $mac = s ] ; then
mac=$(hostname | cut -c1-1)
mac2=$(hostname | cut -c1-2)
mac3=$(hostname | cut -c3-3)
if [ $mac = c -o $mac = s -a $mac3 = n ] ; then        # For cirrus and stratus
                                                       # ---------------------------
  rdir=/global
  DMPDIR=${DMPDIR:-$rdir/shared}
# yzdir=${yzdir:-$rdir/save/wx20yz}
  yzdir=${yzdir:-$rdir/save/wx23sm/wx20yz}
  cdasdir=${cdasdir:-/global/shared/stat/cdas}
  machine=IBMP6
  memory_node=${memory_node:-105000}
else
  if [ $mac2 = ga -o $mac2 = c1 -o $mac2 = c2 ] ; then # For gaea
                                                       # ---------
    rdir=/lustre/fs/scratch
    lfsdir=$rdir/$LOGNAME
    DMPDIR=${DMPDIR:-$rdir/shared/glopara}
    yzdir=${yzdir:-$rdir/Shrinivas.Moorthi/para/wx20yz}
    cdasdir=${cdasdir:-$yzdir/gvrfy/data/cdas}
    memory_node=${memory_node:-64000}
    machine=GAEA
    USE_HPSS=NO
    POE=NO
  elif [ $mac = z -o $mac = f -o $mac = r -o $mac2 = bm ] ; then      # For zeus
                                                                      # ---------
    rdir=/scratch2/portfolios/NCEPDEV/global
    PREPTMP=/scratch2/portfolios/NCEPDEV
    PTMP=${PTMP:-/scratch2/portfolios/NCEPDEV/ptmp}
    STMP=${STMP:-/scratch2/portfolios/NCEPDEV/stmp}
    lfsdir=$rdir/save/$LOGNAME
    DMPDIR=${DMPDIR:-$rdir/shared/glopara}
    yzdir=${yzdir:-$rdir/Shrinivas.Moorthi/para/wx20yz}
    cdasdir=${cdasdir:-$yzdir/gvrfy/data/cdas}
    memory_node=${memory_node:-24000}
    machine=ZEUS
    USE_HPSS=YES
    POE=NO
    GLOPARABIN=${GLOPARABIN:-/home/glopara/bin}
    pe_node=${pe_node:-12}
  elif [ $mac = g -o $mac = t ] ; then                  # For wcoss
                                                        # ---------
    rdir=/global
    lfsdir=$rdir/save/$LOGNAME
    DMPDIR=${DMPDIR:-$rdir/shared/glopara}
...
#   USE_HPSS=YES
    USE_HPSS=NO
    POE=NO
    GLOPARABIN=${GLOPARABIN:-/u/emc.glopara/bin}
    PTMP=${PTMP:-/ptmpp1}
    STMP=${STMP:-/stmpp1}
    pe_node=${pe_node:-16}
  fi
...
mkdir -p $EXPDIR
#
HOMEDIR=${HOMEDIR:-$NWPROD}
SHDIR=${SHDIR:-$HOMEDIR/bin}
JOBSDIR=${JOBSDIR:-$HOMEDIR/jobs}
USHDIR=${USHDIR:-$HOMEDIR/ush}
USHPREV=${USHPREV:-$USHDIR}
USHSYND=${USHSYND:-$USHDIR}
SCRDIR=${SCRDIR:-$HOMEDIR/scripts}
#
...
if [ $USE_HPSS = YES ] ; then
# HPSSTAR=${HPSSTAR:-$HOMEDIR/ush/hpsstar}
...
FIXGLOBAL=${FIXGLOBAL:-$HOMEDIR/fix/fix_am}
...
##DUMPROCESSSH=${DUMPROCESSSH:-$HOMEDIR/ush/get_opr_satwnd.sh}
...
GSIEXEC=${GSIEXEC:-$HOMEDIR/exec/global_gsi}
...
FCSTEXECTMP=${FCSTEXECTMP:-$HOMEDIR/exec/global_fcst}
EXEC_AMD=${EXEC_AMD:-$HOMEDIR/exec}
...
GENPSICHIEXE=${GENPSICHIEXE:-$HOMEDIR/exec/genpsiandchi}
OVERPARMEXEC=${OVERPARMEXEC:-$HOMEDIR/exec/overparm_grib}
...
#
CHGRESEXEC=${CHGRESEXEC:-$HOMEDIR/exec/global_chgres}
CYCLEXEC=${CYCLEXEC:-$HOMEDIR/exec/global_cycle}
...
#
# CHGRESVARS="IDVC=$idvc,IVSSFC=$ivssfc,NVCOORD=$nvcoord"
#
FNTSFC=${FNTSFC:-$FIXGLOBAL/cfs_oi2sst1x1monclim19822001.grb}
FNAISC=${FNAISC:-$FIXGLOBAL/cfs_ice1x1monclim19822001.grb}
FNVEGC=${FNVEGC:-$FIXGLOBAL/global_vegfrac.0.144.decpercent.grb}
FNVETC=${FNVETC:-$FIXGLOBAL/global_vegtype.1x1.grb}
IALB=${IALB:-${ialb:-0}}
#
MTNDIR=${MTNDIR:-$FIXGLOBAL}
#
...
if [ $newoz_nrl = YES ] ; then
 O3FORC=${O3FORC:-$FIXGLOBAL/global_o3prdlos.f77}
 O3CLIM=${O3CLIM:-$FIXGLOBAL/ozone.clim}
fi
...
#
 FIX_OM=${FIX_OM:-$HOMEDIR/fix/fix_om}
 FIX_OCN=${FIX_OCN:-$FIX_OM}
 PARM_OM=${PARM_OM:-$HOMEDIR/parms/parm_om}
 SNOWNC=${SNOWNC:-$FIX_OCN/SNOW.nc}
 SSTICECLIM=${SSTICECLIM:-$FIX_OCN/sst_ice_clim.nc}
#
 if [[ $CKSH = oanl ]] ; then
  PARM_GODAS=${PARM_GODAS:-$HOMEDIR/parms/parm_om/tbl_nml}
...
#                                     ------------------------------------
 EXEC_OMD=${EXEC_OMD:-$HOMEDIR/exec}
 OM_EXEC=${OM_EXEC:-EXEC_OMD/fms_mom4icie2.x}
#
#                                     Coupler directory and executable
#                                     --------------------------------
 EXEC_CD=${EXEC_CD:-$HOMEDIR/exec}
 C_EXEC=${C_EXEC:-$EXEC_CD/mlc_cfs4_coupler}
#
#
#                                     GODAS and related executables
#                                     --------------------------------
 GODASEXEC=${GODASEXEC:-$HOMEDIR/exec/fms_gdsSOLO.x}
 cmbDysPrf4=${cmbDysPrf4:-$HOMEDIR/exec/cmbDysPrf4}
 cmbDysPrfs4=${cmbDysPrfs4:-$HOMEDIR/exec/cmbDysPrfs4}
 mkEvNc4r=${mkEvNc4r:-$HOMEDIR/exec/mkEvNc4r}
 GODASSH=${GODASSH:-$HOMEDIR/ush/godasM4.sh}
#
...
# For GLDAS
if [[ $CKSH = lanl ]] ; then
 PARM_LM=${PARM_LM:-$HOMEDIR/parms/parm_lm}
fi
#                                     --------------------------------
...
LISEXEC=${LISEXEC:-$HOMEDIR/exec/cfs_cdas_gldas_LIS}
#
...
if [ $mac = z -o $mac = f -o $mac = r -o $mac2 = bm ] ; then
 export CHGRP_CMD=${CHGRP_CMD:-tag_rstprod}
else
 ...
 if [ ${TASKGM:-NO} = YES ] ; then
  if [ $COUP_FCST = NO ] ; then
   if [ $CKSH = fcst -a $liope = .true. -a $JCAP -ge 1500 ] ; then
    ...
    TASK_GEOMETRY=$(/usrx/local/bin/mktgs  $((tasks-1))/$((npe_node/NUMTHRD)) 1/1)
   fi
  else
