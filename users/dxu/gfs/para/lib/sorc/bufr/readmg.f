...
C         If the backspace fails, assume end-of-file.

          BACKSPACE (LUNIT,IOSTAT=IERBSP)
          IF(IERBSP.NE.0) THEN
           IF(IPRT.GE.0) THEN
      CALL ERRWRT('++++++++++++++BUFR ARCHIVE LIBRARY+++++++++++++++++')
      ERRSTR = 'BUFRLIB: READMG - COULD NOT BACKSPACE FILE TO READ'//
     . ' MESSAGES AS ARRAYS OF BYTES; WILL ASSUME END-OF-FILE'
      CALL ERRWRT(ERRSTR)
      CALL ERRWRT('++++++++++++++BUFR ARCHIVE LIBRARY+++++++++++++++++')
      CALL ERRWRT(' ')
           ENDIF
...
C	   Backspace the file pointer and then use subroutine
C	   RDBFDX to read in all such dictionary messages
C	   (they should be stored consecutively!) and reset
C	   the internal tables.
