...
while [[ $imem -le $ENSEND ]]; do
   ...
   export FCSTVARS_TMP=${FCSTVARS_ENKF:-" "}
   ...
   if [[ "$SET_STP_SEED" = "YES" ]] ; then
      ...
      export FCSTVARS_TMP="ISEED_SPPT=$ISEED_SPPT,ISEED_SKEB=$ISEED_SKEB,ISEED_VC=$ISEED_VC,ISEED_SHUM=$ISEED_SHUM,$FCSTVARS_TMP"
   fi
   ...
   if [ $DOIAU = YES ]; then
     ...
     export FCSTVARS_TMP="iau=.true.,iau_delthrs=6,iaufhrs=6,iaufiles_fg=$iaufiles_fg,iaufiles_anl=$iaufiles_anl,$FCSTVARS_TMP"
