...
! ----------------------------------------------------------------------
real(r_quad) function dplevs0(nlevs,dx,dy)
...
!   2008-06-04  safford - rm unused uses and vars
...
! Declar passed variables
  integer(i_kind),intent(in)::nlevs
  real(r_kind),dimension(lat2,lon2,nlevs),intent(in)::dx,dy
...
! This would result in slightly more accurate results
  ...
  dplevs0=zero_quad
  do k=1,iglobal
    dplevs0=dplevs0+work1(k)
  end do

return
end function dplevs0

real(r_quad) function dplev1(nlevs,dx,dy)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    dplev   calculates dot product for data on subdomain
!   prgmmr: derber           org: np23                date: 2004-05-13
...
! Declare local variables
  ...
  do k=1,nlevs
     do j=1,lon1
        jp=j+1
        do i=1,lat1
           dplev1=dplev1+dx(i+1,jp,k)*dy(i+1,jp,k)
        end do
     end do
  end do
