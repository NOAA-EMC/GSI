...
fixgsi=$GSIDir/fix
#fixcrtm=/scratch1/NCEPDEV/da/Emily.Liu/RTM/CRTM/releases/REL-2.3.0/fix_crtm
USHDir=$GSIDir/ush/
...
export JCAP=766        # orig: 382
export JCAP_B=1534     # orig: 382
export LEVS=64
...
NDATE=/scratch1/NCEPDEV/global/glopara/git/NCEPLIBS-prod_util/v1.1.0/exec/ndate
ncpc=/bin/cp
ncpl="ln -fs"
...
datobs=$ObsDir/${dumpobs}.$PDYa/$cyca
datobsnr=$ObsDir/${dumpobs_nr}.$PDYa/$cyca
datges=$GuessDir/gdas.$PDYg/$cycg
...
satinfo=$fixgsi/global_satinfo_nrt.txt 
...
#$ncpl /scratch2/NCEPDEV/stmp1/Emily.Liu/AOP75-NRT-work/Data/gdas.${gdate}.abias_air ./aircftbias_in 
...
env
/apps/slurm/default/bin/srun ./gsi.x > stdout
...
mkdir -p ${OutDir_obs}
mv diag_* ${OutDir_obs}
cp satbias_in ${OutDir_obs}/satbias.$gdate
cp satbias_pc ${OutDir_obs}/satbias_pc.$gdate
cp aircftbias_in ${OutDir_obs}/aircftbias.$gdate
cp gsistat ${OutDir_obs}/gsistat.ops.$adate
cp gsistat.out ${OutDir_obs}/gsistat.$adate
cp stdout ${OutDir_obs}/stdout.$adate

cd $OutDir_obs
cd ../
tar -cvf ${adate}_obs.tar ./${adate}_obs

# Now processing ioda-converters in JEDI-rapids
...
#sbatch $USHDir/JEDI/convert_gsi_diags.sh $USHDir $OutDir $adate
