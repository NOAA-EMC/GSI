...
!$$$
    ...
    jpch_oz = 53                ! (12 levels + 1 total column) * 4 satellites = 52
                                ! plus 1 for OMI ozone data = 53 total entries
                                ! jpch_oz is total number of enteries in the
...
!   All mpi tasks open and read ozone information file.
    ...
    do j=1,jpch_oz
       read(lunin,*,err=200,end=210) nusis_oz(j),&
            nulev(j),iuse_oz(j),pob_oz(j),gross_oz(j),error_oz(j), &
            b_oz(j),pg_oz(j)
    end do
    close(lunin)

!   Echo information just read in to standard out.  Do this from
!   a single mpi task, mype_oz
    if (mype==mype_oz) then
       open(iout_oz)
       do j=1,jpch_oz
          write(iout_oz,110) j,nusis_oz(j),nulev(j),&
...
110 format(i3,1x,a20,' lev = ',i4,' use = ',i2,' pob = ',f9.3,&
         ' gross = ',f7.3,' error = ',f7.3,' b_oz = ',f7.3,' pg_oz = ',f7.3)

!   Successful read, return to calling routine
    return

!   Handle various error conditions below
!   1) Encountered a fatal read error
200 continue
...
210 continue
    write(6,*)'OZINFO_READ:  ***ERROR*** fewer entries than jpch_oz=',jpch_oz
