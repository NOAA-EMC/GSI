...
!?????????????????????later refine to be area weighted
    ...
    call mpi_allgather(count,1,mpi_rtype,count0,1,mpi_rtype,mpi_comm_world,ierror)
    count=zero
    do i=0,npe-1
      count=count+count0(i)
    end do
    count=one/count
...
!   psbar:
    ...
    call mpi_allgather(psbar,1,mpi_rtype,psbar0,1,mpi_rtype,mpi_comm_world,ierror)
    psbar=zero
    do i=0,npe-1
      psbar=psbar+psbar0(i)
    end do
...
!   tbar:
    do k=1,nsig
      ...
      do j=2,lon2-1
        do i=2,lat2-1
          tbar(k)=tbar(k)+ges_tv(i,j,k,ntguessig)
        end do
      end do
    end do
    call mpi_allgather(tbar,nsig,mpi_rtype,tbar0,nsig,mpi_rtype,mpi_comm_world,ierror)
    do k=1,nsig
      tbar(k)=zero
    end do
    do i=0,npe-1
      do k=1,nsig
        tbar(k)=tbar(k)+tbar0(k,i)
      end do
    end do
    do k=1,nsig
      tbar(k)=count*tbar(k)
...
!   hmat:
    do k=1,nsig
      hmat(k)= rd*tbar(k)*(bhat(k)+bhat(k+1))/(pbar(k)+pbar(k+1)) + &
           two*rd*tbar(k)*(bhat(k)*pbar(k+1)-bhat(k+1)*pbar(k))/(pbar(k)+pbar(k+1))**2
    end do
    do k=2,nsig
      do i=1,k-1
        hmat(k)=hmat(k)+ &
         four*rd*tbar(i)*(bhat(i)*pbar(i+1)-bhat(i+1)*pbar(i))/(pbar(i)+pbar(i+1))**2
      end do
    end do

!   amat:              ???????????later add chat terms to amat to complete generality
    amat=zero
    do i=1,nsig
      factor=rd*(pbar(i)-pbar(i+1))/(pbar(i)+pbar(i+1))
      amat(i,i)=factor
      if(i.eq.nsig) exit
      factor=two*factor
      do k=i+1,nsig
...
!   bmat:
    bmat=zero
    do k=1,nsig
      factor=rd_over_cp*tbar(k)/(pbar(k)+pbar(k+1))
      bmat(k,k)=factor*(pbar(k)-pbar(k+1))
      factor=two*factor
      if(k.eq.nsig) exit
      do i=k+1,nsig
        bmat(k,i)=factor*(pbar(i)-pbar(i+1))
      end do
    end do

!   smat:
    do k=1,nsig
      smat(k)=pbar(k)-pbar(k+1)
    end do
