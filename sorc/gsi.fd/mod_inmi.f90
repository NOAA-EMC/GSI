...
!   def nstrong_end      - number of "clean up" iterations of strong constraint initialization
!                          at end of inner loop
!   def update_end       - if .false., then run "clean up" iterations for diagnostic information
!                          on balance of current outer loop increment, but don't update increment
...
implicit none
  ...
  integer(i_kind) nstrong_end
  logical update_end, jcstrong
  integer(i_kind) mmax
  character(1) scheme
  integer(i_kind) m
  real(r_kind) gspeed,period_max,period_width
...
!         unscale:    vort_hat_g, div_hat_g, phi_hat_g --> vort_g, div_g, phi_g
    ...
    real(r_kind) b(m:mmax),c(m:mmax),f(m:mmax),pmask(m:mmax)
