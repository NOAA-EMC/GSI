...
!$$$
  ...
  use obsmod, only: iout_sst,iout_pw,iout_t,iout_rw,iout_dw,&
       iout_srw,iout_uv,iout_gps,iout_ps,iout_q,&
       mype_dw,mype_rw,mype_srw,mype_sst,mype_gps,mype_uv,mype_ps,&
       mype_t,mype_pw,mype_q,ndat,dtype,ditype
...
! Declare passed variables
  integer(i_kind),intent(in):: mype,i_ps,i_uv,&
       i_srw,i_t,i_q,i_pw,i_rw,i_dw,i_gps,i_sst
  real(r_kind),dimension(7*nsig+100,10),intent(in)::awork
...
! Initialize constants and variables.
  ...
  ptopall(1)=zero; pbotall(1)=2000.0
...
! Summary report for gps 
  if (mype==mype_gps)then
     ...
     gpsmplty=zero; gpsqcplty=0 ; ntot=0
     ...
     if(nkeep > izero)then
      ...
      write(iout_gps,*)' number of gps obs failed in Northern Hemisphere =',numfail1_gps
      write(iout_gps,*)' number of gps obs failed in Southern Hemisphere =',numfail2_gps
      write(iout_gps,*)' number of gps obs failed in Tropics =',numfail3_gps
