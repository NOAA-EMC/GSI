...
!$$$
  ...
  use berror, only: as,dssvl,dssv2,&
  ...
  use guess_grids, only: ntguessfc,isli,ozmz,ges_prslavg,ges_psfcavg
...
! background error arrays for sfp,land skint, and ice t
  do j=llmin,llmax
     dssvl(j,1)=corp(j)*as(3)
     dssvl(j,2)=as(7)*one
     dssvl(j,3)=as(7)*one
  end do
! background error array for SST
  do j=1,lon2
     do i=1,lat2
        dssv2(i,j)=as(7)*zero_3
     end do
  end do
...
! sli in scale  unit (can add in sea-land mask)
  ...
  do i=1,nx
     do j=1,ny
        ...
        sli(j,i,3,1)=region_dy(j,i)*fact
     enddo
  enddo
