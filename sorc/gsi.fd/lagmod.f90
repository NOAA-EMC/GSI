...
DO i=1,n
   ...
   DO j=1,n
      IF(j /= i) THEN
       qin(i)=q(i)
       q(i)=qin(i)/(x(i)-x(j))
       q_TL(i)=(q_TL(i)/(x(i)-x(j)))-&
               (qin(i)*(x_TL(i)-x_TL(j))/(x(i)-x(j))**2)
      ENDIF
   ENDDO
ENDDO
...
!============================================================================
subroutine slagdw_TL(x,x_TL,xt,aq,aq_TL,bq,bq_TL,w_TL,dw,dw_TL,n)
!============================================================================
IMPLICIT NONE
INTEGER(i_kind),            INTENT(IN ) :: n
REAL(r_kind),               INTENT(IN ) :: xt
...
REAL(r_kind),DIMENSION(n),  INTENT(OUT) :: dw_TL,w_TL,dw
!-----------------------------------------------------------------------------
REAL(r_kind),DIMENSION(n)               :: aw,bw,daw,dbw
