subroutine setupq(lunin,mype,bwork,awork,nele,nobs,conv_diagsave,perturb)
...
!$$$
  ...
  use constants, only: rad2deg,huge_single,wgtlim
...
! Declare passed variables
  ...
  real(r_kind),dimension(nobs),intent(in):: perturb
...
! Declare local variables  
  ...
  integer(i_kind) jlat,jlon,jsig,itype,k,nn,ikxx
...
!   array perturb=0.0 and observation is unchanged.
    ...
    qob = data(iqob,i) + perturb(i)*obserror*qsges
...
!   amount of extrapolation.
    ...
    ratio_errors=error*qsges/((errorx+1.0e6_r_kind*rhgh+r8*rlow)*sqrt(dup(i)))
