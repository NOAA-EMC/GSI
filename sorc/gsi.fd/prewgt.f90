...
! Declare local variables
  ...
  real(r_kind),dimension(0:nlat+1,nsig,4):: hwll
  ...
  real(r_single),dimension(nlat,nsig):: corz,cord,corh,corq,corc,corq2
  real(r_single),dimension(nlat):: corp
  real(r_single),dimension(nlat,nlon):: corsst
  real(r_single) hsstmin
  real(r_kind) minhsst,fact,factoz,corq2x
  real(r_kind),allocatable:: randfct(:)

  real(r_single),dimension(nlat,nsig*5+1):: hwllin
  real(r_single),dimension(nlat,nlon):: hsst
  real(r_single),dimension(nlat,nsig*5):: vscalesin
...
! load vertical length scales
  ...
  voz=eight_tenths   ! for now use fixed value
...
!   load variances onto subdomains
    do k=1,nsig
       factoz = max(ozmz(k),0.0002_r_kind)*r25
       do i=1,lon2
          dssv(1,j,i,k)=dssv(1,j,i,k)*corz(jx,k)*as(1)      ! streamfunction
          dssv(2,j,i,k)=dssv(2,j,i,k)*cord(jx,k)*as(2)      ! velocity potential
          dssv(3,j,i,k)=dssv(3,j,i,k)*corh(jx,k)*as(4)      ! temperature
       ...
       enddo
    enddo
    dssvl(j,1)= corp(jx)*as(3)                      ! surface pressure
    dssvl(j,2)= one                                 ! land sfc tmp
    dssvl(j,3)= one                                 ! ice sfc tmp
  end do
  ...
  do j=1,lon2
     ...
     do i=1,lat2
        ...
        ix=max(ix,2)
        ix=min(nlat-1,ix)
        dssv2(i,j) = corsst(ix,jx)*as(7)        ! SST
     end do
  end do
