...
!$$$
  ...
  use gridmod, only: lat2,lon2,nsig,sigsum
  ...
  real(r_kind),intent(in):: p(lat2,lon2)  
     ...
     do k=1,nsig
       do j=1,lon2
          do i=1,lat2
             q(i,j,k) = dqdrh(i,j,k)*rhnorm(i,j,k) &
                  +dqdt(i,j,k)*t(i,j,k) &
                  -dqdp(i,j,k)*half*p(i,j)*sigsum(k)
          end do
       end do
     end do
  end if
...
!$$$
  ...
  real(r_kind),intent(inout):: p(lat2,lon2)
     ...
     do k=1,nsig
        do j=1,lon2
           do i=1,lat2
              ...
              p(i,j) = p(i,j) - half*dqdp(i,j,k)*q(i,j,k)*sigsum(k)
              q(i,j,k) = zero
           end do
        end do
     end do
