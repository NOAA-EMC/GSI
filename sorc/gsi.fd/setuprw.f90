...
!$$$
  ...
  use guess_grids, only: ges_ps,nfldsfc,hrdifsig,geop_hgtl,ges_z,nfldsig,&
       ntguessig,hrdifsfc,fact10,ges_lnprsl,ges_u,ges_v
  use gridmod, only: nsig,nlat,nlon,get_ijk
  use constants, only: flattening,semi_major_axis,grav_ratio,zero,grav,wgtlim,&
       half,one,two,grav_equator,eccentricity,somigliana,rad2deg,deg2rad,r1000
...
! Declare local variables
  ...
  real(r_kind) ugesin,vgesin,factw,presrw
  ...
  integer(i_kind) mm1,jj,k2,itype
...
!    index information for data array (see reading routine)
  ...
  irange=15   ! index of range in km of obs from radar
  izsges=16   ! index of model (guess) elevation for radar associated with vad wind
  ier2=17     ! index of original-original obs error
  numequal=0
  numnotequal=0
...
!    Interpolate log(surface pressure), 10m wind factor, 
