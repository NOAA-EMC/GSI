...
! Declare local variables  
  ...
  real(r_kind) dlon,dlat,time,timedif,rflag,sstx
  ...
  do while (ireadmg(lnbufr,subset,idate) == 0)
     ...
     read_loop:do while (ireadsb(lnbufr) == 0)
       ...
       &            'YEAR MNTH DAYS HOUR MINU SECO CLAT CLON SAID')
        ...
        call ufbint(lnbufr,bufrf(14),5,1,iret,'SAZA SOZA SSTYPE BEARAZ SOLAZI')
        call ufbint(lnbufr,bufrf(20),1,1,iret,'SST1')
        call ufbrep(lnbufr,bufrf(23),1,5,iret,'TMBR')

        do k=1,nchanl
           if(bufrf(24+k) < tbmin .or. bufrf(24+k) > tbmax) then
              iskip=iskip+1
           else
              nread=nread+1
           end if
...
!       Transfer information to work array
        ...
        data_all(1, iout) = bufrf(9)            ! satellite id (16 = NOAA-16, 17 = NOAA-17)
        data_all(2, iout) = tdiff               ! time (hours)
        data_all(3, iout) = dlon                ! grid relative longitude
        data_all(4, iout) = dlat                ! grid relative latitude
        data_all(5, iout) = bufrf(14)*deg2rad   ! satellite zenith angle (radians)
        data_all(6, iout) = bufrf(17)           ! satellite azimuth angle 
        data_all(7, iout) = bufrf(20)           ! Navy SST retrieval
        data_all(8, iout) = zero                ! scan position (unavailable)
        data_all(9, iout) = bufrf(15)           ! solar zenith angle (radians)
        data_all(10,iout) = bufrf(18)           ! solar azimuth angle (radians)
        data_all(11,iout) = isflg               ! land sea ice snow flag
        data_all(12,iout) = sfcpct(0)           ! sea percentage of
        data_all(13,iout) = sfcpct(1)           ! land percentage
        data_all(14,iout) = sfcpct(2)           ! sea ice percentage
        data_all(15,iout) = sfcpct(3)           ! snow percentage
        ...
        data_all(18,iout) = bufrf(16)           ! Data type: 151=day,152=night,159=cloud problem
        data_all(19,iout) = sst_hires           ! interpolated hires SST (deg K)

          
        data_all(20,iout) = val_avhrr
        data_all(21,iout) = itt
        do k=1,nchanl
           data_all(k+maxinfo,iout)= bufrf(24+k)  ! Tb for avhrr ch-3, ch-4 and ch-5
        end do
