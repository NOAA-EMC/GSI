...
!$$$
  ...
  use guess_grids, only: fact10,ges_lnprsl,ges_ps,geop_hgtl,nfldsfc,&
       hrdifsfc,hrdifsig,nfldsig,ges_u,ges_v
  ...
  use constants, only: huge_single
...
! Declare local variables
  ...
  integer(i_kind) itype,itype2
  integer(i_kind) i,nchar,nreal,k,j,it,ii,l,jj,k1,k2
...
!    index information for data array (see reading routine)
  ...
  irange=15   ! index of mean range of superob from radar (meters)
...
! Interpolate log(surface pressure), model terrain, 10m wind
! factor and log(pres) at mid-layers to observation location.
...
! account for representativeness error.
    ...
    ratio_errors = error/(abs(error + 1.0e6*rhgh + r8*rlow)*sqrt(dup(i)))
    error = one/error
