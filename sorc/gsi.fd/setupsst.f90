...
!$$$
  ...
  use guess_grids, only: hrdifsig,nfldsig,sfct,isli,ntguessfc
  ...
  use constants, only: zero,tiny_r_kind,one,one_tenth,half,wgtlim, &
            two,cg_term,three,pi,izero,rad2deg,huge_single,r1000
...
! Declare local variables
  ...
  integer(i_kind) i,nchar,nreal,j,k,ii,ikxx,nn
...
! Interpolate to get sst at obs location/time
    call tintrp2a(sfct,sstges,dlat,dlon,dtime,hrdifsig,&
       1,1,mype,nfldsig)
...
!   Check to ensure point surrounded by water
    ...
    if(isli(ix,iy ,ntguessfc) > izero .or. isli(ixp,iy ,ntguessfc) > izero .or. &
       isli(ix,iyp,ntguessfc) > izero .or. isli(ixp,iyp,ntguessfc) > izero)then
        error = zero
    end if
