...
! Number of channels for sensors in BUFR
  ...
  integer(i_kind),parameter :: maxinfo    =  19
...
! Input variables
  ...
  real(r_kind)     ,intent(in) :: val_amsre
...
! Other work variables
  ...
  real(r_kind)     :: dx, dy, dx1, dy1, sstx, dlon, dlat
...
!                 (3) - snow percentage
           ...
           if (amsre_low) then
             call deter_sfc_amsre_low(dlat,dlon,isflg,sfcpct,sstx)
           else if(amsre_mid .or. amsre_hig) then
...
!            data_all(8,iobsout) = ifov                    ! fov number    1-196
             ...
             data_all(15,iobsout)= dlon_earth*rad2deg      ! earth relative longitude (degrees)
             data_all(16,iobsout)= dlat_earth*rad2deg      ! earth relative latitude (degrees)
             data_all(17,iobsout)= sun_gangle              ! sun glint angle

             data_all(18,iobsout)= val_amsre
             data_all(19,iobsout)= itt
...
subroutine deter_sfc_amsre_low(dlat,dlon,isflg,sfcpct,sstx)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
! Mean sst in FOV
!     sstx=sst_full(klat1,klon1 ) + sst_full(klatp1,klon1 ) + &
!          sst_full(klat1,klonp1) + sst_full(klatp1,klonp1) + &
!          sst_full(klatp2,klon1) + sst_full(klatp2,klonp1) + &
!          sst_full(klatp1,klon2) + sst_full(klatp1,klonp2) + &
!          sst_full(klat1,klon2) + sst_full(klat1,klonp2) + &
!          sst_full(klat2,klon1) + sst_full(klat2,klonp1)
!     sstx = sstx / 12.0
...
!    Set surface type flag.  Begin by assuming obs over ice-free water
     ...
     if(sno_full(klat1 ,klon1 ) > one .and. jsli == 1)then
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp1 ,klon1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klat1 ,klonp1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp1 ,klonp1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp2 ,klon1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp2 ,klonp1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp1 ,klon2 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klatp1 ,klonp2 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klat1 ,klon2 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klat1 ,klonp2 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klat2 ,klon1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
     ...
     sfcpct(jsli)=sfcpct(jsli)+one
     if(sno_full(klat2 ,klonp1 ) > one .and. jsli == 1)then
        sfcpct(3)=sfcpct(3)+one
        sfcpct(jsli)=sfcpct(jsli)-one
     endif
