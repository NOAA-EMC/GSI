...
!          begin read of mub while doing rest of offset setup
  ...
  do k=1,lm
     ...
     length(i)=(im+1)*jm
           if(mype == 0.and.k==1) write(6,*)' u i,igtype,offset,kdim(i) = ',i,igtype(i),offset(i),kdim(i)
  end do
  ...
  do k=1,lm
     ...
     offset(i)=n_position ; length(i)=im*(jm+1) ; igtype(i)=3 ; kdim(i)=lm
           if(mype == 0.and.k==1) write(6,*)' v i,igtype,offset,kdim(i) = ',i,igtype(i),offset(i),kdim(i)
  end do
...
! Load updated skin temperature array if writing out to analysis file
  if (update_regsfc) then
     do i=1,lon1
        ...
        do j=1,lat1
           jp1=j+1
           all_loc(j,i,i_sst)=sfct(jp1,ip1,ntguessfc)
           all_loc(j,i,i_tsk)=sfct(jp1,ip1,ntguessfc)
        end do
     end do
  end if
...
!---------------------- write surface files last
  do k=kbegin(mype),kend(mype)
     if(kdim(k).eq.1) then
        call contract_ibuf(ibuf(1,k),im,jm)
...
subroutine contract_ibuf(ibuf,im,jm)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    contract_ibuf    contract array in place
!   prgmmr: parrish          org: np22                date: 2004-11-29
!
! abstract: contract array in place from im+1,jm+1 to im,jm
!
! program history log:
!   2004-11-29  parrish
...
!
!   input argument list:
!     ibuf     - input grid values in im+1,jm+1
!     im       - first grid index
...
!   field of dim (im+1)*(jm+1) needs to be converted to field of im*jm
