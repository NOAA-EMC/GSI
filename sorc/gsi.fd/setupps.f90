subroutine setupps(lunin,mype,bwork,awork,nele,nobs,conv_diagsave,perturb)
...
!$$$
  ...
  use obsmod, only: rmiss_single,pstail,pshead
...
!   array perturb=0.0 and observation is unchanged.  Factor
!   of one_tenth converts cermin and cermax limits from mb
!   to cb
    obserror = max(cermin(ikx)*one_tenth,&
         min(cermax(ikx)*one_tenth,data(ier,i)))
    pob = pob + perturb(i)*obserror
