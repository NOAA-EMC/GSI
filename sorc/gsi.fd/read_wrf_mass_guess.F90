...
!$$$
  ...
  use jfunc, only: qsatg,qsinv2,jiter,jiterstart,qgues,&
...
! other internal variables
  ...
  integer(i_kind),allocatable::igtype(:),kdim(:)
...
!    Following is for convenient WRF MASS input
     ...
     allocate(igtype(num_mass_fields),kdim(num_mass_fields))
...
!       used as individual file pointers by mpi_file_read_at
     ...
     do it=1,nfldsig
        ...
        do k=1,lm+1
           i=i+1
           offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=lm+1
           if(mype == 0.and.k==1) write(6,*)' sfc geopot i,igtype(i),offset(i) = ',i,igtype(i),offset(i)
        end do
        ...
        do k=1,lm
           ...
           offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=lm
        ...
        do k=1,lm
           i=i+1                                                       ! v(k)
           offset(i)=n_position ; length(i)=im*(jm+1) ; igtype(i)=3 ; kdim(i)=lm
        ...
        read(lendian_in) n_position,ksize
        offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=ksize
        if(mype == 0) write(6,*)' smois i,igtype(i),offset(i) = ',i,igtype(i),offset(i)
        do k=2,ksize
           i=i+1
           offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=ksize
        end do
        ...
        read(lendian_in) n_position,ksize
        offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=ksize
        if(mype == 0) write(6,*)' tslb i,igtype(i),offset(i) = ',i,igtype(i),offset(i)
        do k=2,ksize
           i=i+1
           offset(i)=n_position ; length(i)=im*jm ; igtype(i)=1 ; kdim(i)=ksize
...
!                                    read geopotential
        ...
        call mpi_file_read_at(mfcst,this_offset,jbuf(1,1,jbegin(mype)),this_length,mpi_integer,status,ierror)
...
!---------------------- read surface files last
        do k=kbegin(mype),kend(mype)
           if(kdim(k).eq.1) then
              ...
              call expand_ibuf(ibuf(1,k),im,jm)
           end if
        end do
...
!                Convert guess mixing ration to specific humidity
...
!             For now, replace missing skin temps with 1st sigma level temp
     ...
     deallocate(all_loc,igtype,kdim)
...
subroutine expand_ibuf(ibuf,im,jm)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    expand_ibuf    expand array in place
!   prgmmr: parrish          org: np22                date: 2004-11-29
!
! abstract: expand array in place from im,jm to im+1,jm+1
!
! program history log:
!   2004-11-29  parrish
!
...
!   field of dim im*jm read into array of dim (im+1)*(jm+1)--need to readjust

  use kinds, only: i_long,i_kind
  implicit none
  
  integer(i_kind),intent(in):: im,jm
  integer(i_long),intent(inout):: ibuf((im+1)*(jm+1))
  
  integer(i_long) itemp(im+1,jm+1)
