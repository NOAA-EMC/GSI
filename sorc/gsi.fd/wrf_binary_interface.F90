...
!                   PHB                  
     ...
     if(index.lt.0) stop
      n_position=file_offset(index+1)
      write(6,*)'  byte offset for PHB(',k,') = ',n_position
      write(lendian_out)n_position    ! offset for PHB (zsfc*g is 1st level of this 3d field)
...
!                   T                  
     ...
     if(index.lt.0) stop
        n_position=file_offset(index+1)
        write(6,*)'  byte offset for T(',k,') = ',n_position
        write(lendian_out)n_position    ! offset for T(k)    ! POT TEMP (sensible)
...
!                   QVAPOR
     ...
     if(index.lt.0) stop
        n_position=file_offset(index+1)
        write(6,*)'  byte offset for QVAPOR(',k,' = ',n_position
        write(lendian_out)n_position    ! offset for QVAPOR(k)
  
!                   U                  
     call retrieve_index(index,'U',varname_all,nrecs)
     if(index.lt.0) stop
        n_position=file_offset(index+1)
        write(6,*)'  byte offset for U(',k,' = ',n_position
        write(lendian_out)n_position    ! offset for U(k)

!                   V                  
     call retrieve_index(index,'V',varname_all,nrecs)
     if(index.lt.0) stop
        n_position=file_offset(index+1)
        write(6,*)'  byte offset for V(',k,' = ',n_position
        write(lendian_out)n_position    ! offset for V(k)
...
!                   SMOIS              
     ...
     if(index.lt.0) stop
       ...
       n_position=file_offset(index+1)
       write(6,*)'  byte offset, ksize for SMOIS(',k,') = ',n_position,ksize
       write(lendian_out)n_position,ksize     !  SMOIS
...
!                   TSLB               
     ...
     if(index.lt.0) stop
           ksize=domainend_all(2,index)
       n_position=file_offset(index+1)
       write(6,*)'  byte offset, ksize for TSLB(',k,') = ',n_position,ksize
       write(lendian_out)n_position,ksize     !  TSLB
...
!                   PINT               
     ...
     if(index.gt.0) then
       update_pint=.true.
       n_position=file_offset(index+1)
       write(6,*)'  byte offset for PINT = ',n_position
       write(lendian_out)n_position    ! offset for PINT !
     end if
...
!                   T                  
     ...
     if(index.lt.0) stop
     n_position=file_offset(index+1)
     write(6,*)'  byte offset for T = ',n_position
     write(lendian_out)n_position    ! offset for T    !
  
!                   Q                  
     call retrieve_index(index,'Q',varname_all,nrecs)
     if(index.lt.0) stop
     n_position=file_offset(index+1)
     write(6,*)'  byte offset for Q = ',n_position
     write(lendian_out)n_position    ! offset for Q    !

!                   U                  
     call retrieve_index(index,'U',varname_all,nrecs)
     if(index.lt.0) stop
     n_position=file_offset(index+1)
     write(6,*)'  byte offset for U = ',n_position
     write(lendian_out)n_position    ! offset for U    !

!                   V                  
     call retrieve_index(index,'V',varname_all,nrecs)
     if(index.lt.0) stop
     n_position=file_offset(index+1)
     write(6,*)'  byte offset for V = ',n_position
     write(lendian_out)n_position    ! offset for V    !
...
!                   SMC                
     ...
     n_position=file_offset(index+1)
           ksize=domainend_all(2,index)
     write(6,*)'  byte offset, ksize for SMC = ',n_position,ksize
     write(lendian_out)n_position,ksize    ! offset for SMC   !
...
!                   STC                
     ...
     n_position=file_offset(index+1)
           ksize=domainend_all(2,index)
     write(6,*)'  byte offset, ksize for STC = ',n_position,ksize
     write(lendian_out)n_position,ksize    ! offset for STC   !
