...
! Include data type module
  ...
  use pcpinfo, only: ibias,iusep,varchp,dtphys,deltim,nupcp,jtype
  use obsmod, only: dtype,ndat,iout_pcp,dplat,ditype
  use jfunc, only: jiter,first
  use constants, only: zero,half,one,two,r1000
  implicit none
...
! On first outer iteration, write constant parameters
  if (first ) then
     ...
     do j=1,jtype
        write(iout_pcp,1005) nupcp(j),iusep(j),ibias(j),varchp(j)
     end do
1000 format(1X,5I5,2E18.10)
1005 format(' ipcp  = ',a16,  &
          ' iusep = ',i4,   ' ibias = ',i4, &
          ' var   = ',e10.4)
  endif
...
3000 format('pcp total   penalty_all  =',g18.12)
  write(iout_pcp,3001)qcpenalty_all
3001 format('pcp total qcpenalty_all  =',g18.12)
