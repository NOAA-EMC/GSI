...
! Define parameters
  integer(i_kind),parameter,private:: lnbufr = 15
...
99  format('.',I4.4)
    ...
    call openbf (lnbufr,'OUT',lendian_in)
...
!
  ...
  subroutine avhrr_sst_retrieval(obstype,csatid,nchanl,&
       ...
       rad_diagsave,nadir,mype,ireal,ipchan,duse)
...
!   Declare local parameters
    ...
    real(r_kind),parameter:: amiss=-999.0_r_kind
...
!**********************************
    ...
    if (duse) then
     ...
     dsst = amiss; dta = amiss; dqa = amiss
...
!   write(*,'(I4,15f8.2)') csatid,obslon,obslat,sstph,sstnv,ts5, &
!                       tb_obs(ch3),bt3,tb_obs(ch4),bt4,tb_obs(ch5),bt5, &
!                       sstph-ts5,dta,dqa,pangs
...
! ------------------------------------------------------------------------------
        ...
        if(csatid == 'g15') rsat=106.0_r_kind
        if(csatid == 'g16') rsat=107.0_r_kind
        if(csatid == 'g17') rsat=108.0_r_kind
        if(csatid == 'g18') rsat=109.0_r_kind
...
!          file on first outer iteration.
           ...
           diagbuf(11) = rmiss_single                   ! fractional coverage by water
           diagbuf(12) = rmiss_single                   ! fractional coverage by land
           diagbuf(13) = rmiss_single                   ! fractional coverage by ice
           diagbuf(14) = rmiss_single                   ! fractional coverage by snow
...
!          Note:  The following quantities are not computed for all sensors
           ...
           do i=1,nchanl
              ...
              diagbufchan(5,i)=rmiss_single    ! quality control mark or event
              diagbufchan(6,i)=emissivity(i)   ! surface emissivity
              diagbufchan(7,i)=tlapchn(i)      ! stability index
              
              if ( nn == 1 .and. varinv(nn) == zero ) then
                 diagbufchan(1,nn) = amiss                    ! Tb (obs) (K)
                 diagbufchan(2,nn) = amiss                    ! dTb (cal (bc) - obs) (K)
                 diagbufchan(3,nn) = amiss                    ! dTb (cal (nobc) - obs) (K)
                 diagbufchan(4,nn) = amiss                    ! d(Tb)/d(Ts)
                 diagbufchan(5,nn) = amiss                    ! d(Tb)/d(Ta)
                 diagbufchan(6,nn) = amiss                    ! d(Tb)/d(Qa)
              endif
