...
!$$$
  ...
  use jfunc, only: qsatg,qsinv2,jiter,jiterstart,&
       qgues,dqdt,dqdrh,dqdp,qoption,switch_on_derivatives,tendsflag
...
!    Following is for convenient NMM/WRF NMM input
     ...
     allocate(igtype(num_nmm_fields),kdim(num_nmm_fields))
...
!       used as individual file pointers by mpi_file_read_at
     ...
     num_doubtful_sfct=0
        ...
        do k=1,lm
           i=i+1                                                       ! u(k)
           offset(i)=n_position ; length(i)=im*jm ; igtype(i)=2 ; kdim(i)=lm
...
!             For now, replace missing skin temps with 1st sigma level temp
     ...
     deallocate(all_loc)
     deallocate(temp1,igtype,kdim,offset,length)
...
!$$$
  ...
  use jfunc, only: qsatg,qsinv2,jiter,jiterstart,&
       qgues,dqdt,dqdrh,dqdp,qoption,switch_on_derivatives
  use guess_grids, only: ges_z,ges_ps,ges_pint,ges_pd,ges_tv,ges_q,ges_u,ges_v,&
