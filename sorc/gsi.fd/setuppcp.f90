...
subroutine setuppcp(lunin,mype,aivals,nele,nobs,&
     obstype,isis,is,pcp_diagsave,perturb)
...
! !USES:
  ...
  use guess_grids, only: hrdifsig, hrdifsfc
  use guess_grids, only: ntguessfc
  use guess_grids, only: nfldsig, nfldsfc
  use guess_grids, only: ges_ps
  use guess_grids, only: ges_ps_lon
  use guess_grids, only: ges_ps_lat
  use guess_grids, only: ges_prsl,ges_prsi
  use guess_grids, only: ges_div
  use guess_grids, only: ges_vor
  use guess_grids, only: ges_cwmr
  use guess_grids, only: ges_tsen
  use guess_grids, only: ges_u
  use guess_grids, only: ges_v
  use guess_grids, only: ges_q
  use guess_grids, only: isli
  use guess_grids, only: ges_tv_ten,ges_q_ten,ges_prs_ten
  ...
  use constants, only: rd,cp,pi,grav,deg2rad,zero,one_tenth,quarter,&
       half,one,two,tiny_r_kind,one_tenth,izero,cg_term,r1000,wgtlim,fv
...
!   2005-08-05  treadon - increase size of diag_pcp_file character variable;
!                         remove mype and add idate to diag_pcp_file header
...
! Declare local variables
  ...
  real(r_kind) errf,varinv,error0,obstime,slats,slons,satpcp,pcpsdv, &
       ...
       plon0,plat0,pcplrg,rn1_ad,rn4,psexp,cldwrk,cldwrk4,xkt2,ts0
...
!    Extract satellite id
     ...
     if (kx<=0) write(6,*) &
        ...
        ksatid,kx,isis,(nupcp(i),i=1,jtype)
!
...
!    Perturb observation.  Note:  if perturb_obs=.false.,
!    the array perturb=0.0 and observation is unchanged.
     satpcp = satpcp + perturb(n)*error
...
!    Get skin temperature at observation location.
     ...
     call tintrp2a(sfct,ts0,slats,slons,obstime, &
       hrdifsfc,1,1,mype,nfldsfc)
