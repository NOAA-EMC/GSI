...
module fv3_interface
  ...
  type(ncio_meta)   :: meta_ncio !! ncio metadata for the current file
...
100 format(A,': ',A)
    ...
    ! Read the ncio header
    call gfs_ncio_initialize(meta_ncio, firstguess_filename)
    call gfs_grid_initialize(grid, meta_ncio)
...
200 format('error in: ',A,': ',A) ! context was not supplied
  ...
  subroutine fv3_analysis_read_var(grid,varname)
    ...
    integer :: k ! Vertical index loop


    ! Read the ncio file header
    call gfs_ncio_initialize(meta_ncio,filename=grid%filename)

    ! Allocate our local work array
    allocate(workgrid(grid%nx,grid%ny,grid%nz))
  ...
  subroutine fv3_analysis_read_or_calc_dpres(grid)
    ...
    allocate(workgrid(meta_ncio%dimx,meta_ncio%dimy,meta_ncio%dimz))
  ...
  subroutine fv3_analysis_read_non_inc_vars(grid)
    ...
    grid%nx=meta_ncio%dimx
    grid%ny=meta_ncio%dimy
    grid%nz=meta_ncio%dimz

    ! Determine ordering of latitudes:
    if (debug) then
       print *,'lats',meta_ncio%lat(1), meta_ncio%lat( &
                meta_ncio%dimx*meta_ncio%dimy)
    endif
    if (meta_ncio%lat(1) > meta_ncio%lat(meta_ncio%dimx*meta_ncio%dimy)) then
       grid%flip_lats = .true.
    else
  ...
  subroutine fv3_grid_allocate(an_grid,fg_grid)
    ...
    call gfs_ncio_initialize(meta_ncio,filename=analysis_filename)
    an_grid%nx   = meta_ncio%dimx
    an_grid%ny   = meta_ncio%dimy
    an_grid%nz   = meta_ncio%dimz
