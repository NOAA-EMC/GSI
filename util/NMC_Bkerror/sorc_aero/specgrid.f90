...
!>swei
! Try to port sptranfv to here similar to sptranf to sptranf_s
...
! Call spectral <--> grid transform
    call sptranfv_s(waved,wavez,gridu,gridu,gridv,gridv,idir)
  ...
  subroutine sptranfv_s(waved,wavez,gridun,gridus,gridvn,gridvs,idir)
    use kinds, only: r_kind
    implicit none
...
! Declare passed variables
    integer,intent(in):: idir
    real(r_kind),dimension(nc),intent(inout):: waved,wavez
    real(r_kind),dimension(ijmax),intent(inout):: gridun,gridvn
    real(r_kind),dimension(ijmax),intent(inout):: gridus,gridvs

! Declare local variables
    integer i,j,jj,ij,ijn,ijs,mp
    real(r_kind),dimension(2*(jcap+1)):: utop,vtop
    real(r_kind),dimension(nc) :: u,v
    real(r_kind),dimension(imax,2):: ug,vg
      ...
      do j=jb,je
        if(wlat(j).gt.0.) then
          do i=1,imax
             ...
             ijn = i + jj*jn
             ijs = i + jj*js + ioffset
             ug(i,1)=gridun(ijn)/clat(j)**2
             ug(i,2)=gridus(ijs)/clat(j)**2
             vg(i,1)=gridvn(ijn)/clat(j)**2
             vg(i,2)=gridvs(ijs)/clat(j)**2
          enddo
        endif
        write(6,*) 'tranf1'
        call sptranf1(iromb,jcap,idrt,imax,jmax,j,j, &
