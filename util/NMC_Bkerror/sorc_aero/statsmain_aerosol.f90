...
!   smoothdeg - degree of horizontal smoothing to apply in latitudinal direction
  ...
  namelist/namstat/jcap,jcapin,jcapsmooth,nsig,nlat,nlon,maxcases, &
                   hybrid,smoothdeg,biasrm,vertavg,use_nemsio,modelname !swei add use_nemsio
...
!
  ...
  call create_mapping
!>swei
! This subroutine initialize all variables for spectrum transform
...
! Make call to see how many available files there are
  ...
  call getcases_nems(numcases,mype)
  call readpairs_aerosol(npe,mype,numcases)
...
! Get lat-dependend variances
  call variances_aerosol(numcases,mype)

! Vertical length scales
  call vertsc_aerosol(numcases,mype)

! Horizontal length scales
  call horizsc_aerosol(numcases,mype)

! post Processing
  if (mype==0) then
    write(6,*) 'WRITE OUT BACKGROUND ERROR STATISTICS'
    call writefiles_aerosol
  end if
