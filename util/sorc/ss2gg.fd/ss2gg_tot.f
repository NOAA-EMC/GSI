...
subroutine ss2gg1(luggg,idrt,imax,jmax,head,data)
  ...
  real(4) pm(imax*jmax,head%levs),pd(imax*jmax,head%levs)
  integer k,n,ij
  ...
  call modpr(imax*jmax,imax*jmax,head%levs,head%idvc,head%idsl,&
             head%si,head%ak,head%bk,f1,pm,pd)
  call wryte(luggg,4*imax*jmax*head%levs,pm)
  call wryte(luggg,4*imax*jmax*head%levs,pd)
  ...
  do n=2,head%ntrac
    ...
    call wryte(luggg,4*imax*jmax*head%levs,g1)
  enddo
! Compute total column for tracers
  do n=1,head%ntrac
     call sptezm(0,head%jcap,idrt,imax,jmax,head%levs,data%q(1,1,n),g1,1)
     f1=0.0
     do k=1,head%levs
        do ij=1,imax*jmax
           f1(ij)=f1(ij)+g1(ij,k)*pd(ij,k)
        end do
     end do
     if (n==1 .or. n==3) f1=max(f1,0.0)/con_g
     if (n==2) f1=max(f1,0.0)/(con_g*2.14e-5)
     call wryte(luggg,4*imax*jmax,f1)
  end do
end subroutine
subroutine ss2gg2(luctl,idrt,imax,jmax,nsig,head,cfggg)
  ...
  write(luctl,'("vars",i6)') 11+2*head(1)%ntrac
  ...
  write(luctl,'("TQ  ",i3," 99 specific humidity (kg/kg)")') 1
  do n=2,min(head(1)%ntrac,9)
    write(luctl,'("TQ",i1,1x,i3," 99 total column tracer ",i1," (kg/kg)")') n,1,n
  enddo
  do n=10,head(1)%ntrac
    write(luctl,'("TQ",i2,1x,i3," 99 total column tracer ",i2," (kg/kg)")') n,1,n
  enddo
