               ...
               do c=1,nch_active
                  ...
                  do j=1,n_pair
                     if ((anluse(r)>zero).and.(gesuse(obs_pairs(j),c,i)>zero)) then
                        ...
                        babo_sum=babo_sum+((ges(obs_pairs(j),c,i)-anl(r))*(ges(obs_pairs(j),c,i)))
                        bobo_sum=bobo_sum+(ges(obs_pairs(j),c,i)*ges(obs_pairs(j),c,i))
                        anl_sum=anl_sum+anl(r)
                        ges_sum=ges_sum+ges(obs_pairs(j),c,i)
                        ba_sum=ba_sum+(ges(obs_pairs(j),c,i)-anl(r) )
                        div=div+1
                     end if  
                  end do
...
Rcov=Rcov/two
Rcorr=Rcorr/two
Edbadbo=Edbadbo+TRANSPOSE(Edbadbo)
Edbodbo=Edbodbo+TRANSPOSE(Edbodbo)
Edbadbo=Edbadbo/two
Edbodbo=Edbodbo/two
invE=Edbadbo+Rcov
...
deallocate(ba_ave)
deallocate(Pmult, invE)
deallocate(Edbadbo, Edbodbo)
