...
module read_diag
  ...
  use ckinds, only:  i_kind,r_radstat,r_kind
...
! Declare structures for radiance diagnostic file information
  ...
  type diag_header_chan_list
     real(r_radstat) :: freq              ! frequency (Hz)
     real(r_radstat) :: polar             ! polarization
     real(r_radstat) :: wave              ! wave number (cm^-1)
     real(r_radstat) :: varch             ! error variance (or SD error?)
     real(r_radstat) :: tlapmean          ! mean lapse rate
  ...
  type diag_data_fix_list
     real(r_radstat) :: lat               ! latitude (deg)
     real(r_radstat) :: lon               ! longitude (deg)
     real(r_radstat) :: zsges             ! guess elevation at obs location (m)
     real(r_radstat) :: obstime           ! observation time relative to analysis
     real(r_radstat) :: senscn_pos        ! sensor scan position (integer(i_kind))
     real(r_radstat) :: satzen_ang        ! satellite zenith angle (deg)
     real(r_radstat) :: satazm_ang        ! satellite azimuth angle (deg)
     real(r_radstat) :: solzen_ang        ! solar zenith angle (deg)
     real(r_radstat) :: solazm_ang        ! solar azimumth angle (deg)
     real(r_radstat) :: sungln_ang        ! sun glint angle (deg)
     real(r_radstat) :: water_frac        ! fractional coverage by water
     real(r_radstat) :: land_frac         ! fractional coverage by land
     real(r_radstat) :: ice_frac          ! fractional coverage by ice
     real(r_radstat) :: snow_frac         ! fractional coverage by snow
     real(r_radstat) :: water_temp        ! surface temperature over water (K)
     real(r_radstat) :: land_temp         ! surface temperature over land (K)
     real(r_radstat) :: ice_temp          ! surface temperature over ice (K)
     real(r_radstat) :: snow_temp         ! surface temperature over snow (K)
     real(r_radstat) :: soil_temp         ! soil temperature (K)
     real(r_radstat) :: soil_mois         ! soil moisture 
     real(r_radstat) :: land_type         ! land type (integer(i_kind))
     real(r_radstat) :: veg_frac          ! vegetation fraction
     real(r_radstat) :: snow_depth        ! snow depth
     real(r_radstat) :: sfc_wndspd        ! surface wind speed
     real(r_radstat) :: qcdiag1           ! ir=cloud fraction, mw=cloud liquid water
     real(r_radstat) :: qcdiag2           ! ir=cloud top pressure, mw=total column water
     real(r_radstat) :: tref              ! reference temperature (Tr) in NSST
     real(r_radstat) :: dtw               ! dt_warm at zob
     real(r_radstat) :: dtc               ! dt_cool at zob
     real(r_radstat) :: tz_tr             ! d(Tz)/d(Tr)
  end type diag_data_fix_list

  type diag_data_chan_list
     real(r_radstat) :: tbobs              ! Tb (obs) (K)
     real(r_radstat) :: omgbc              ! Tb_(obs) - Tb_(simulated w/ bc)  (K)
     real(r_radstat) :: omgnbc             ! Tb_(obs) - Tb_(simulated_w/o bc) (K)
     real(r_radstat) :: sprd               ! ensemble spread
     real(r_radstat) :: errinv             ! inverse error (K**(-1))
     real(r_radstat) :: qcmark             ! quality control mark
     real(r_radstat) :: emiss              ! surface emissivity
     real(r_radstat) :: tlap               ! temperature lapse rate
     real(r_radstat) :: tb_tz              ! d(Tb)/d(Tz)
     real(r_radstat) :: bicons             ! constant bias correction term
     real(r_radstat) :: biang              ! scan angle bias correction term
     real(r_radstat) :: biclw              ! CLW bias correction term
     real(r_radstat) :: bilap2             ! square lapse rate bias correction term
     real(r_radstat) :: bilap              ! lapse rate bias correction term
     real(r_radstat) :: bicos              ! node*cos(lat) bias correction term
     real(r_radstat) :: bisin              ! sin(lat) bias correction term
     real(r_radstat) :: biemis             ! emissivity sensitivity bias correction term
     real(r_radstat),dimension(:),allocatable :: bifix          ! angle dependent bias
     real(r_radstat) :: bisst              ! SST bias correction term
  end type diag_data_chan_list

  type diag_data_extra_list
     real(r_radstat) :: extra              ! extra information
  ...
  real(r_radstat),parameter::  rmiss_radiag    = -9.9e11_r_radstat
...
subroutine open_radiag(filename, ftin, istatus)
   ...
   if (netcdf) then
      ...
      open(ftin,form="unformatted",file=filename,access="sequential",convert="big_endian",iostat=istatus)
      rewind(ftin)
   endif
...
!  Declare local variables
  ...
  real(r_radstat) :: freq_tmp,polar_tmp,wave_tmp,varch_tmp,tlapmean_tmp
  logical loutall
...
! Declare local variables
  ...
  real(r_radstat), allocatable, dimension(:)  :: Latitude, Longitude, Elevation, Obs_Time, Scan_Position, &
  ...
  real(r_radstat), allocatable, dimension(:,:)   :: BC_angord ! (nobs, BC_angord_arr_dim) ;
  real(r_radstat), allocatable, dimension(:,:) :: Observation_Operator_Jacobian
...
! Declare passed arguments
  ...
  real(r_radstat),dimension(:,:),allocatable :: data_tmp
  real(r_radstat),dimension(:),allocatable   :: fix_tmp
  real(r_radstat),dimension(:,:),allocatable :: extra_tmp
