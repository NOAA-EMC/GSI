...
>$cmdfile
   ...
   $SUB -q $JOB_QUEUE -P $PROJECT -R affinity[core] -o ${logfile} -W 0:45 -J ${jobname} $cmdfile
...
>$cmdfile
   ...
   if [[ $MY_MACHINE = "wcoss" ]]; then
      $SUB -q $JOB_QUEUE -P $PROJECT -R affinity[core] -o ${logfile} -W 2:45 -J ${jobname} $cmdfile
   else
