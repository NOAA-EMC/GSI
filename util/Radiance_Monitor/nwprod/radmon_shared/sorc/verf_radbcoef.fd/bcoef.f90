...
! Namelist with defaults
  ...
  namelist /input/ satname,npredr,nchanl,iyy,imm,idd,ihh,idhh,&
       incr,suffix,retrieval,gesanl,little_endian,netcdf
...
100 format('.',i4.4,3i2.2)
  ...
  if ( trim(gesanl) == 'ges' ) then
     ...
     data_file= 'bcoef.' // trim(satname) // trim(stringd) // '.ieee_d'
     ctl_file = 'bcoef.' // trim(satname) // '.ctl'
  else
     diag_rad = trim(satname) // '_anl'
     data_file= 'bcoef.' // trim(satname) // '_anl' // trim(stringd) // '.ieee_d'
     ctl_file = 'bcoef.' // trim(satname) // '_anl.ctl'
  endif 
