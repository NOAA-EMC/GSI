...
! Namelist with defaults
  ...
  namelist /input/ satname,iyy,imm,idd,ihh,idhh,incr,nchanl,&
       suffix,retrieval,gesanl,little_endian,&
       rad_area,netcdf
...
100 format('.',i4.4,3i2.2)
  ...
  if ( trim(gesanl) == 'ges' ) then
     ...
     data_file= 'angle.' // trim(satname) // trim(stringd) // '.ieee_d'
     ctl_file = 'angle.' // trim(satname) // '.ctl'
  else 
     diag_rad = trim(satname) // '_anl'
     data_file= 'angle.' // trim(satname) // '_anl' // trim(stringd) // '.ieee_d'
...
! Deallocate arrays
  ...
  if(allocated(timang))          deallocate (timang)
  if(allocated(tot_cor))         deallocate (tot_cor)
  if(allocated(omg_nbc))         deallocate (omg_nbc)
  if(allocated(omg_bc))          deallocate (omg_bc)
  ...
  if(allocated(cos_cor))         deallocate (cos_cor)
  if(allocated(sin_cor))         deallocate (sin_cor)
