...
! Namelist with defaults
  ...
  namelist /input/ satname,iyy,imm,idd,ihh,idhh,incr,nchanl,&
       suffix,retrieval,gesanl,little_endian,rad_area,netcdf
...
100 format('.',i4.4,3i2.2)
  ...
  if ( trim(gesanl) == 'ges' ) then
     ...
     data_file= 'bcor.' // trim(satname) // trim(stringd) // '.ieee_d'
     ctl_file = 'bcor.' // trim(satname) // '.ctl'
  else
     diag_rad = trim(satname) // '_anl'
     data_file= 'bcor.' // trim(satname) // '_anl' // trim(stringd) // '.ieee_d'
...
!       Determine into which subdomains the observation falls.
