...
!    End of loop over diagnostic file
     ...
     do j=1,n_chan
        if( count(j,1) > 0.0 ) then 
           write(6,*) 'omg_bc(j,1,1) / count(j,1) =', omg_bc(j,1,1), count(j,1), omg_bc(j,1,1) / count(j,1)
        else
           write(6,*) 'omg_bc(j,1,1), count(j,1) =', omg_bc(j,1,1), count(j,1)
        endif
     enddo
