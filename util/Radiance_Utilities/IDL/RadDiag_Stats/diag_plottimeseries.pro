...
;-
  ...
  FOR n = 0, RadDiag_Stats.nVariables - 1 DO BEGIN
    ...
    ; Get limits
    minY = MIN(RadDiag_Stats.Time_Mean[n,Channel-1,*])
    maxY = MAX(RadDiag_Stats.Time_Mean[n,Channel-1,*])
    IF ( d2_Present EQ 1 ) THEN BEGIN
      minY = minY < MIN(RadDiag_Stats2.Time_Mean[n,Channel-1,*])
      maxY = maxY > MAX(RadDiag_Stats2.Time_Mean[n,Channel-1,*])
    ENDIF

    ; Plot the data
    PLOT, xJD, RadDiag_Stats.Time_Mean[n,Channel-1,*], $
          YRANGE=[minY,maxY], $
    ...
    OPLOT, xJD, RadDiag_Stats.Time_Mean[n,Channel-1,*], COLOR=DCOLOR

    IF ( d2_Present EQ 1 ) THEN BEGIN
      OPLOT, xJD2, RadDiag_Stats2.Time_Mean[n,Channel-1,*], $
             COLOR=D2COLOR
    ENDIF
